...
class RpcPeer {
  constructor(child, handlers, log) { this.child = child; this.handlers = handlers; this.log = log; this.nextId = 0; this.pending = new Map(); this.buffer = ""; child.stdout.setEncoding("utf8"); child.stdout.on("data", (chunk) => this.accept(chunk)); child.stderr.on("data", (chunk) => log.append(chunk.toString())); child.on("exit", (code) => this.close(new Error(`Workstrand ACP exited with ${code ?? "signal"}.`))); }
...
class KestrelEditorClient {
  ...
  async start(prompt) {
    ...
    const child = spawn(executable, args, { stdio: ["pipe", "pipe", "pipe"], shell: false, cwd: this.root, env: process.env }); this.peer = new RpcPeer(child, this.handlers(), this.output); this.status.text = "$(sync~spin) Workstrand"; this.status.show();
    await this.peer.request("initialize", { protocolVersion: 1, clientInfo: { name: "Workstrand VS Code", version: "0.1.0" }, clientCapabilities: { fs: { readTextFile: true, writeTextFile: true }, terminal: true } });
    const session = await this.peer.request("session/new", { cwd: this.root, mcpServers: config.get("mcpServers", []) }); this.sessionId = session.sessionId; this.output.show(true); this.output.appendLine(`Workstrand task ${this.sessionId}`); this.output.appendLine(`You: ${prompt}`);
    const result = await this.peer.request("session/prompt", { sessionId: this.sessionId, prompt: [{ type: "text", text: prompt }] }); this.output.appendLine(`\n[${result.stopReason}]`); this.status.text = "$(check) Workstrand";
  }
  handlers() { return {
...
function activate(context) { const output = vscode.window.createOutputChannel("Workstrand"); const status = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 20); let active; context.subscriptions.push(output, status, vscode.commands.registerCommand("kestrel.startTask", async () => { const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath; if (!root) return void vscode.window.showErrorMessage("Open a workspace before starting Workstrand."); const prompt = await vscode.window.showInputBox({ title: "Workstrand task", prompt: "What should Workstrand do?", ignoreFocusOut: true }); if (!prompt) return; active?.dispose(); active = new KestrelEditorClient(root, output, status); try { await active.start(prompt); } catch (error) { status.text = "$(error) Workstrand"; output.appendLine(`\nError: ${error instanceof Error ? error.message : error}`); output.show(); } }), vscode.commands.registerCommand("kestrel.cancelTask", () => active?.cancel()), { dispose: () => active?.dispose() }); }
function deactivate() {}
