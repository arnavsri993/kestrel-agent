...
final class AcpPeer implements AutoCloseable {
    ...
    void run(String prompt) throws Exception {
        JsonObject initialize = new JsonObject(); initialize.addProperty("protocolVersion", 1); initialize.add("clientInfo", gson.toJsonTree(Map.of("name", "Workstrand JetBrains", "version", "0.1.0"))); initialize.add("clientCapabilities", gson.toJsonTree(Map.of("fs", Map.of("readTextFile", true, "writeTextFile", true), "terminal", true)));
    ...
    @Override public void close() { cancel(); terminals.values().forEach(item -> item.process.destroy()); if (process != null) process.destroy(); pending.values().forEach(future -> future.completeExceptionally(new IOException("Workstrand editor client closed."))); pending.clear(); }
}
