...
- Primary composition: a persistent five-step horizontal progress rail—Welcome, Before you begin, Choose a model, Model setup, Ready—keeps the complete journey above the task plane. The current stage receives one restrained contained surface; completed stages remain available for backward navigation, while future stages stay inert. Model setup opens with one three-route source chooser: external providers, a private local agent, and a reviewed open-access directory. The selected source expands below without duplicating the choice into another tab bar. The local pane has one dominant automatic action, one manual fallback, visible download/proof progress, and the full model library below.
...
- Native-quality reset: setup uses a quiet macOS assistant composition rather than a provider console. Progress remains one compact row of full, readable stage names. Source choices use readable labels, one restrained symbol, one honest status, and one directional affordance each; selected detail is disclosed below, and exhaustive compatibility data stays behind a native disclosure control. Body text is never miniaturized to make the catalog fit, and compact windows keep the rail on one row.
...
- Modernized-step captures: `artifacts/screenshots/desktop/setup-modernized/` covers the five-stage journey, expanded boundary detail, all three model routes, and compact provider setup. The rail stays one row at 640×760 with no page-level horizontal overflow.
- Modernization refinements retained: model-route rows now end in a restrained directional affordance, and step changes move programmatic focus to the new page heading after the state transition while initial load remains undisturbed.
- Rejected after live visual review: a thin connected dot-and-line stepper with shortened labels. It read as a generic web onboarding pattern, reduced label clarity, weakened the current-stage hierarchy, and was removed rather than iterated.
- App-wide copy hierarchy: persistent navigation names destinations; page headings describe the task or outcome instead of echoing that destination. Setup uses numeric progress beneath the product mark instead of repeating the active rail label. Browser history, downloads, command center, Life, Research, Artifacts, Work, Activity, Opportunities, Extensions, and Settings follow the same rule. Repeated control names remain only where consistency or an accessible name is required, and safety, privacy, cost, approval, credential, and recovery explanations are not shortened for style.
- App-wide copy captures: `artifacts/screenshots/desktop/copy-deduplicated/` covers setup, browser and agent entry points, History, Downloads, every command-center destination, all Settings sections, and compact/reduced-motion states. Capture assertions reject a destination, active Settings label, or setup rail label that repeats as the adjacent visible heading.
