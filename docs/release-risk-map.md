...
Five parallel audits at baseline `59cf7830` identified **14 release blockers** across data integrity, secret storage, agent safety, browser automation, and distribution infrastructure. A hardening sprint opened **six PRs (#644–#649)** that address the highest-severity code gaps:
...
| **Browser** (audit `f5c504b4`) | Tab lifecycle races | #649 (pin guard); mutex deferred |
| **Security/IPC** (audit `cd7f30f9`) | RB-1, RB-2 | #646 |
| **Agent runtime** (audit `2314bc32`) | L1 | #647 |

**Code blockers with PRs in flight:** R1, R2, R3, R5, RB-1, RB-2, L1, P1-01, browser tab pin (partial).  
...
**Deferred P1 items:** R4 (legacy idempotency), security P1-1–P1-4 (CSP, step-up auth, DevTools, extension sideload), agent T2/N1 (SIGKILL escalation, provider connect timeout), browser operation mutex, lifecycle P1-03/P1-04.

**Release recommendation:** Do not ship public macOS builds until operator blockers RB-01–03 are proven, PRs #645–#649 are green and merged, and deferred P1 security items are either fixed or explicitly accepted in the threat model.
...
| **BR-1** | Browser | Agent browser tools vs tab discard/sleep/close races — WebContents destroyed mid-CDP | **fixed** (partial, PR open) | [#649](https://github.com/arnavsri993/kestrel-agent/pull/649) — ref-counted tab pins; mutex still pending |
| **BR-2** | Browser | No serialization between tab mutations (`closeTab`, organize, detach) and agent backend | **open** | Follow-up after #649 — per-service/per-tab operation mutex |
...
CI snapshot: 2026-08-29 ~16:12 UTC.
...
| [#649](https://github.com/arnavsri993/kestrel-agent/pull/649) | `codex/production-browser-tab-pin` | Agent browser tab pin guard | BR-1 (partial) | ⏳ Core **IN_PROGRESS** · ⏳ Desktop smoke **IN_PROGRESS** |

All six PRs: mergeable, review required.
...
1. **Browser operation mutex (BR-2)** — serialize `closeTab`, organize, detach vs `handleAgentRequest`; depends on #649 pin foundation.
2. **Legacy idempotency retirement (R4)** — audit `idempotent()` callers; migrate to claim-based API.
3. **Security P1 hardening** — CSP nonce/hash (P1-1), step-up auth (P1-2), block DevTools in prod (P1-3), restrict extension sideload (P1-4).
4. **Agent resilience (T2, N1)** — SIGKILL escalation for command-runner/subscription-cli; bounded connect timeout + offline fast-fail on `providerFetch`.
5. **Lifecycle P1-03/P1-04** — embed stable channel in prod plist; surface updater errors.
6. **Browser follow-ups (BR-3–BR-8)** — WebContents transfer on detach, CDP lifecycle hardening, render-process-gone while pinned.
...
- [ ] All PRs #644–#649 merged; full `pnpm verify` green on `main`.
...
- **#649 last** — browser service changes are self-contained; tab pin is prerequisite for follow-up mutex work.

After all six merge, run operator checklist for RB-01–03 before tagging a public release.
