...
**Baseline:** `origin/main` @ `8705c85b` (Codex and verified free-endpoint catalog merged in #669)
...
**Status legend:** `fixed` = merged on `main`; `in PR` = implemented and locally validated but not merged; `open` = code fix still needed; `operator` = requires operator/infrastructure action, not a code PR alone.

---

## Executive summary

Five parallel audits at baseline `59cf7830` identified **14 release blockers** across data integrity, secret storage, agent safety, browser automation, and distribution infrastructure. The first hardening sprint merged its focused fixes; follow-up [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) restores stable-build protected storage and closes additional release-critical browser, lifecycle, and unfinished-surface gaps. #672 is open and must not be counted as merged until GitHub says so.
...
| **Security/IPC** (audit `cd7f30f9`) | RB-1, RB-2 | #657 (supersedes closed #646); restored with a stable-build policy in open #672 |
| **Agent runtime** (audit `2314bc32`) | L1, T2, N1 | #647 (L1), #653 (T2), #656 (N1) |

**Code blockers merged on `main`:** R1, R2, R3, R5, L1, P1-01, browser tab pin (BR-1), browser mutation mutex (BR-2), SIGKILL escalation (T2), provider connect timeout (N1), renderer CSP (P1-1, #660), and packaged DevTools blocking (P1-3, #661).
**Implemented in open #672:** RB-1/RB-2 stable-build Keychain/safeStorage policy, P1-03 stable channel, P1-04 content-free updater diagnostics, P1-4 packaged extension restrictions, P2-5 CRX3 signature verification, and R7 corrupt browser-state preservation.
...
**Remaining code-risk decision:** security P1-2 (step-up authentication for privileged IPC) remains open; do not claim it complete without a tested packaged interaction and recovery flow. R4 and the browser follow-ups below remain non-blocking backlog items unless their threat/usage assumptions change.

**Release recommendation:** Do not ship public macOS builds until #672 is merged, operator blockers **RB-01–03** are proven, and P1-2 is either implemented with packaged evidence or explicitly accepted in the threat model. For Stanford vs public release decisions, use the [Engineering ship verdict](#engineering-ship-verdict-sprint-closeout).
...
| **Public macOS release** | **DO NOT SHIP** | Open #672 must merge, operator blockers **RB-01–03** remain unproven (production signing/notarization, live update feed, Widget App Group, tagged workflow and clean-machine Gatekeeper evidence), and security **P1-2** (step-up auth for privileged IPC) remains open. Run [public-release-operator-checklist.md](./public-release-operator-checklist.md) before any stable tag. |

**Engineering sprint status:** Final hardening PR #672 is open. No P0 code blockers remain for the offline demo path. Public release still requires #672, operator RB-01–03, a P1-2 decision, and presentation/clean-machine evidence.
...
| **RB-1** | Security | Database root key stored without OS protection (`PlaintextSecretProtection`) | **in PR** | [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) uses Electron `safeStorage` on the stable channel, migrates plaintext envelopes, and fails closed; explicit development/test modes remain isolated |
| **RB-2** | Security | Chromium cookie encryption disabled via `use-mock-keychain` in all builds | **in PR** | [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) uses the real macOS Keychain on the stable channel; explicit development/test modes retain mock Keychain |
...
| **P1-03** | Lifecycle | Production release channel not embedded in prod plist | **in PR** | [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) embeds `KESTREL_RELEASE_CHANNEL=stable` and verifies the installable plist |
| **P1-04** | Lifecycle | Updater failures silently ignored (`checkForUpdates().catch(() => undefined)`) | **in PR** | [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) records only content-free failure diagnostics while keeping startup non-blocking |
| **P1-1** | Security | Trusted renderer CSP allows `unsafe-inline` — XSS → full IPC bridge | **fixed** | [#660](https://github.com/arnavsri993/kestrel-agent/pull/660) uses a nonce for renderer scripts; style `unsafe-inline` remains for Vite/CSS injection |
...
| **P1-3** | Security | DevTools openable in packaged builds via IPC | **fixed** | [#661](https://github.com/arnavsri993/kestrel-agent/pull/661) rejects packaged DevTools requests and disables packaged DevTools entry points |
| **P1-4** | Security | Extension sideloading from renderer IPC | **in PR** | [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) removes packaged local-extension IPC/UI and accepts verified Chrome Web Store IDs only |
...
| **R7** | Persistence | Browser state load silently discards corrupt tabs | **in PR [#672](https://github.com/arnavsri993/kestrel-agent/pull/672)** — atomically preserves corrupt state as `.corrupt-*`; ordinary read errors remain errors |
...
| **P2-5** | Security | Browser extension CRX install lacks signature verification | **in PR [#672](https://github.com/arnavsri993/kestrel-agent/pull/672)** — verifies CRX3 signature and derived extension ID before bounded extraction |
...
## Hardening PRs (merged plus current follow-up)

The original hardening PRs merged 2026-08-29 with green required checks. Renderer CSP and packaged DevTools were also fixed on `main`. Follow-up #672 is open; its local validation is complete, but its GitHub checks and merge state must be read independently.
...
| [#660](https://github.com/arnavsri993/kestrel-agent/pull/660) | Nonce trusted-renderer scripts | P1-1 | ✅ 2026-08-29 |
| [#661](https://github.com/arnavsri993/kestrel-agent/pull/661) | Block DevTools in packaged builds | P1-3 | ✅ 2026-08-29 |
| [#672](https://github.com/arnavsri993/kestrel-agent/pull/672) | Release-critical desktop hardening | RB-1, RB-2, P1-03, P1-04, P1-4, P2-5, R7 | **open** — full local verify, package, packaged smoke, and functional Web Store extension proof passed |

**Note:** [#646](https://github.com/arnavsri993/kestrel-agent/pull/646) was closed without merge; #657 landed RB-1/RB-2 on `main`, then Keychain/safeStorage was reverted because real Keychain was unreliable under development/ad-hoc signatures. #672 narrows real protected storage to the stable channel and keeps explicit test/development isolation rather than silently weakening stable storage.

**Open PR:** [#672](https://github.com/arnavsri993/kestrel-agent/pull/672). Do not count it as shipped until required checks pass and it is merged through the protected PR path.
...
1. **Security P1-2 decision** — implement step-up authentication with a complete packaged interaction/recovery flow, or explicitly accept the residual risk in the threat model before public release.
2. **Legacy idempotency retirement (R4)** — audit `idempotent()` callers; migrate to claim-based API.
3. **Browser follow-ups (BR-3–BR-8)** — WebContents transfer on detach, CDP lifecycle hardening, render-process-gone while pinned.
...
- [x] Original hardening PRs #644–#651, #653, #656, #657 plus #660/#661 merged on `main`.
- [ ] #672 required GitHub checks green and PR merged through the protected path.
- [x] Full `/opt/homebrew/bin/pnpm verify` green on #672 (154 test files / 1,069 tests; 50/50 browser benchmark; 50/50 website E2E).
...
- [x] Packaged stable policy uses the real Keychain plus Electron `safeStorage`; explicit development/test modes use mock/plaintext isolation (#672 local package + packaged smoke).
- [x] Packaged Chrome Web Store install verifies CRX3 identity and functionally transforms a live JSON page (#672 local packaged proof).
...
✅ #660  nonce renderer scripts (P1-1)
✅ #661  block packaged DevTools (P1-3)
⏳ #672  stable protected storage, extension/install, persistence, lifecycle hardening (open)
```

Next step: get #672 green and merged through the protected PR path, then run the operator checklist for **RB-01–03** before tagging a public release.
