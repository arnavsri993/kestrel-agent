...
**Implemented in this increment:** Kestrel has a dual-primary Browser and Agent desktop workspace with a persistent user browser, an always-mounted agent conversation, a searchable durable-task workspace, independent conversations, History, Bookmarks, Downloads, a searchable capability launcher, browser settings, and command-routed access to existing specialist surfaces. The visible browser supports up to 32 tabs, horizontal or vertical tab layouts, HTTP(S) address/search navigation, twelve curated search engines, back/forward/reload/stop, tab restoration, local history retention, downloads, local browser-data export/import, explicit site permission decisions, unpacked browser extensions, development-only DevTools, ephemeral private tabs, an optional current-page context handoff to the conversation, and the Agent workspace remains a peer product surface.

**Not a claim of a general-purpose browser or unrestricted autonomy:** Kestrel provides a persistent Personal profile and isolated ephemeral Private tabs, plus a user-owned local transfer file; it does not claim named multi-profile management, cloud or cross-device sync, or a browser-specific per-action approval receipt. Site permissions default to blocked and are changed explicitly in Browser Settings. Existing Kestrel policy and approval gates remain the authority for consequential agent actions.
...
- Existing Kestrel consequential-action policy is still the approval authority; page content cannot grant permission. Kestrel does not expose a browser-specific per-action approval receipt, while per-site permission decisions remain explicit and local in Browser Settings.
...
| Settings, including Browser | Browser search/restore/history/bookmark controls, site permissions, extensions, local transfer, plus existing provider, privacy, permissions, skin, pet, connection, and reset paths remain reachable. |
