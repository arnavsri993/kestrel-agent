...
**Not a claim of a general-purpose browser or unrestricted autonomy:** Kestrel has a capability-assessed Chrome Web Store extension path, not Chrome-complete extension support. Each signed package is reviewed for declared access and known compatibility before installation, then receives runtime and restart evidence; unsupported or unknown API use is shown rather than hidden. See [browser extension compatibility](browser-extension-compatibility.md). Profile import/sync, private browsing, and a complete per-site agent-permission manager remain follow-up work. Site permission prompts now remember allow/deny per origin for this profile. Existing Kestrel policy and approval gates remain the authority for consequential agent actions, and mutating browser tools now contribute privacy-bounded receipts to the owning task outcome. Whole-desktop computer use is a separate opt-in capability, not an expansion of the browser boundary.
...
When a URL-reputation provider is configured, Kestrel checks top-level typed navigations, page-initiated navigations, redirects, and managed popups before the native view is allowed to load them. An explicit malicious verdict stops the request and replaces the native view with a Kestrel-owned interstitial; there is no proceed-through control. Provider outages and timeouts remain an `unknown` verdict and do not turn ordinary browsing into an outage.

Downloads are paused before Kestrel chooses a destination, opens a save panel, or resumes transfer. Kestrel checks the final download URL and any Electron-provided redirect chain; an explicit malicious verdict is cancelled and recorded as blocked. A safe or unknown verdict resumes the normal Electron download path. Kestrel does not implement an antivirus engine, sandbox, signature database, YARA rules, or automatic execution. It never removes or rewrites macOS quarantine metadata, bypasses Gatekeeper or notarization, or substitutes for XProtect. A reserved post-completion `SuspiciousDownloadAnalyzer` hook can later enrich an unknown download verdict without changing that policy or receiving an automatic execution capability.

`BrowserThreatProvider` is the provider boundary. The default adapter uses Google Safe Browsing v5 only when `KESTREL_SAFE_BROWSING_API_KEY` is supplied by the host; it is intentionally absent otherwise. Google's Safe Browsing API is documented for non-commercial use, so commercial distribution must supply a compatible commercial provider (such as a Web Risk or vendor adapter) rather than silently using the Google adapter. Provider credentials remain host configuration and are never written into browser state, logs, or renderer IPC.
