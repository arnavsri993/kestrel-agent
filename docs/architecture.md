...
Agent Core recovery deliberately separates durable state from uncertain execution. The main-process supervisor rejects every in-flight IPC request when the utility process exits and starts a replacement only from the last successful bootstrap configuration; it never replays the request. On the replacement core's first boot, `AgentLoop` scans persisted `running` runs. A run whose session claim has no live owner is atomically moved to `failed` with `recovery.reason = core_restarted` and `recovery.action = retry_last_turn`, while matching in-flight tool records become terminal with `outcomeUncertain = true`. Dead runtime-tool claims are closed into the idempotency journal, so a possibly completed side effect cannot be issued again under the same key. Persisted `waiting_approval` runs remain resumable and are not treated as interrupted work. The desktop reloads the active session, labels the task interrupted, shows uncertain action evidence, and waits for the user to choose **Retry last turn**, which creates a fresh run through the ordinary policy and approval boundaries. Model calls and tools are never resumed automatically because Kestrel has no durable proof that either dispatch was safe to repeat.
...
Every non-read-only runtime tool also crosses a universal action-receipt boundary. The utility process writes one encrypted bounded receipt before a mutation starts, then upserts the same receipt when the journal reaches a verified, blocked, failed, cancelled, waiting, or uncertain state. Receipt builders use descriptor metadata and narrow allowlists for destinations and result identifiers; they never copy arbitrary tool input, message bodies, page text, typed browser text, raw error bodies, credentials, URL fragments, or sensitive query parameters. The desktop requests receipts through the typed broker and shows only the latest run's consequential actions in a collapsed disclosure within the task outcome. The existing browser activity ledger remains the richer browser-specific observation record rather than a second receipt source. See [action receipts](action-receipts.md).
...
Browser automation is backend-neutral but requires each autonomous backend to create an isolated profile. Core adds per-agent ownership, explicit origin allowlists, bounded typed click/type/select/key/scroll inputs, untrusted accessibility snapshots, and approval-gated navigation/actions. Electron resolves accessibility refs into current backend DOM nodes, keeps hidden automation layout/input unthrottled while a session is active, settles layout before pointer input, and rejects stale, hidden, disabled, off-viewport, or obscured targets. Semantic native-select actions additionally require an existing option, then dispatch and verify the change without persisting the selected value in browser activity intent. Validated RGBA screenshots feed deterministic pixel comparisons whose hashes and results are encrypted. A versioned 50-workflow loopback benchmark exercises the real renderer/runtime/controller/IPC/Electron path in development and packaged builds and independently verifies server-side state; because it makes no model call or live-site request, model reasoning, tokens, cost, live drift, and real accounts remain outside that rate.
