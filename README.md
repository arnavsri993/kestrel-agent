...
Not yet complete for the stable production channel: public Google OAuth app verification and bundled client registration, additional mailbox/messaging connectors, Developer ID signing, notarization, and verified public website/update endpoints. The beta channel has a separate GitHub prerelease workflow and update feed; users can already connect their own Google Desktop OAuth client through PKCE for Gmail send, read-only code lookup, and Calendar event access.
