...
describe("remote backends and scoped supervision", () => {
	...
	it("approval-gates argv-only allowlisted remote execution", async () => {
		...
		expect(
			(
				await runtime.callTool(session.id, "remote.execute", input, {
		...
		expect(
			...
			status: "verified",
			output: { targetId: "build-host", attestedBy: "ssh-adapter" },
		});
		database.close();
	});
	...
	it("pairs scoped devices, returns redacted remote state, and supports revocation", async () => {
		...
		expect(() => remote.completePairing(pairing.pairingId, "wrong")).toThrow(
			"invalid",
		);
		...
		expect(remote.listSessions(device.token)[0]).not.toHaveProperty(
			"workspaceRoot",
		);
		const job = remote.submitJob(device.token, {
			title: "Remote task",
			sessionId: session.id,
			model: "fake",
			providerIds: ["fake"],
			prompt: "private remote prompt",
			schedule: { kind: "once", nextRunAt: "2026-07-23T00:00:00.000Z" },
		});
		expect(job).not.toHaveProperty("prompt");
		expect(remote.listJobs(device.token)).toMatchObject([
			{ id: job.id, title: "Remote task" },
		]);
		await expect(remote.resumeJob(device.token, job.id)).rejects.toThrow(
			"lacks",
		);
		...
		const ciphertext = database.db
			...
			.get("remote.devices") as { value_ciphertext: string };
	...
	it("recovers pairing and device authorization when persisted records are malformed", () => {
		...
		const storedPairing =
			database.getPrivateState<unknown[]>("remote.pairings")?.[0];
		database.setPrivateState("remote.pairings", [
			storedPairing,
			{ id: "malformed", codeHash: "short" },
			null,
		]);
		const device = remote.completePairing(pairing.pairingId, pairing.code);
		const storedDevice =
			database.getPrivateState<unknown[]>("remote.devices")?.[0];
		database.setPrivateState("remote.devices", [
			storedDevice,
			{ id: "malformed", tokenHash: "short" },
			null,
		]);
	...
	it("locks a pairing after five failed guesses", () => {
		...
		for (let attempt = 0; attempt < 5; attempt += 1)
			expect(() =>
				remote.completePairing(pairing.pairingId, `wrong-${attempt}`),
			).toThrow("invalid");
		expect(() =>
			remote.completePairing(pairing.pairingId, pairing.code),
	...
	it("bounds active pairings, removes expired records, and rejects oversized labels", () => {
		...
		expect(() => remote.beginPairing("x".repeat(101), ["read"])).toThrow(
			"valid label",
		);
		for (let index = 0; index < 200; index += 1)
			remote.beginPairing(`Pending ${index}`, ["read"], 30_000);
		expect(() => remote.beginPairing("Overflow", ["read"])).toThrow("limit");
		now += 31_000;
		remote.beginPairing("Replacement", ["read"]);
		expect(database.getPrivateState<unknown[]>("remote.pairings")).toHaveLength(
			1,
		);
		database.close();
	});
	...
	it("runs concrete Docker, SSH, and Kubernetes CLI adapters with argv containment", async () => {
		const root = mkdtempSync(join(tmpdir(), "kestrel-remote-cli-"));
		const bin = join(root, "bin");
		writeFileSync(join(root, "placeholder"), "workspace");
		await import("node:fs/promises").then(({ mkdir }) => mkdir(bin));
		for (const executable of ["docker", "ssh", "kubectl"]) {
		...
		try {
			const signal = new AbortController().signal;
			const docker = await new DockerCliRemoteBackend().execute({
				target: {
					id: "container",
					kind: "docker",
					backendId: "docker-cli",
					allowedCommands: ["node"],
					enabled: true,
					configuration: { image: "node:24", workspaceRoot: root },
				},
				command: "node",
				args: ["--version"],
				timeoutMs: 5_000,
			...
			expect(docker.stdout).toContain(
				"--network\nnone\nnode:24\nnode\n--version",
			);
			expect(docker.stdout).toContain(
				`type=bind,src=${realpathSync(root)},dst=/workspace`,
			);
			const ssh = await new SshCliRemoteBackend().execute({
				target: {
					id: "host",
					kind: "ssh",
					backendId: "ssh-cli",
				...
				command: "git",
				args: ["status; touch /tmp/not-run"],
				timeoutMs: 5_000,
				signal,
			});
			expect(ssh.stdout).toContain("builder@build.example.com");
			expect(ssh.stdout).toContain("exec 'git' 'status; touch /tmp/not-run'");
			const cluster = await new KubernetesCliRemoteBackend().execute({
				target: {
					id: "cluster",
					kind: "cluster",
					backendId: "kubernetes-cli",
					allowedCommands: ["pnpm"],
					enabled: true,
					configuration: { image: "node:24", namespace: "builds" },
				},
				command: "pnpm",
				args: ["test"],
				timeoutMs: 5_000,
			...
			expect(cluster.stdout).toContain(
				"--image=node:24\n--command\n--\npnpm\ntest",
			);
		} finally {
	...
	it("does not spawn a CLI when execution is already cancelled", async () => {
		const root = mkdtempSync(join(tmpdir(), "kestrel-remote-abort-"));
		const bin = join(root, "bin");
		await import("node:fs/promises").then(({ mkdir }) => mkdir(bin));
		...
		try {
			await expect(
				new DockerCliRemoteBackend().execute({
					target: {
						id: "container",
						kind: "docker",
						backendId: "docker-cli",
	...
	it("uses authenticated HTTPS serverless execution and stores only hash-verified artifacts", async () => {
		...
		const database = new KestrelDatabase(":memory:", createEncryptionKey());
		const bytes = Buffer.from("verified remote artifact");
		const sha256 = createHash("sha256").update(bytes).digest("hex");
		let authorization = "";
		let requestBody = "";
		const backend = new ServerlessHttpRemoteBackend(async (_input, init) => {
		...
		manager.setTargets([
			{
				id: "function",
				kind: "serverless",
				backendId: "serverless-http",
				allowedCommands: ["build"],
				enabled: true,
				configuration: {
					endpoint: "https://functions.example.test/run",
					bearerToken: "serverless-secret",
				},
			},
		]);
		...
		expect(authorization).toBe("Bearer serverless-secret");
		expect(requestBody).toContain('"command":"build"');
		expect(result).toMatchObject({
			exitCode: 0,
			remoteExecutionId: "lambda-1",
			artifacts: [{ filename: "report.txt", bytes: bytes.length, sha256 }],
		});
		...
		const ciphertext = database.db
			...
			.get("providers.remote-targets") as { value_ciphertext: string };
		expect(ciphertext.value_ciphertext).not.toContain("serverless-secret");
		database.close();
		rmSync(root, { recursive: true, force: true });
	});
		...
		await expect(
			backend.execute({
				target: {
					...
					allowedCommands: ["build"],
					enabled: true,
					configuration: { endpoint: "not a URL" },
				},
				command: "build",
				args: [],
				timeoutMs: 5_000,
		...
		await expect(
			backend.execute({
				target: {
					...
					allowedCommands: ["build"],
					enabled: true,
					configuration: { endpoint: "https://functions.example.test/run" },
				},
				command: "build",
				args: [],
				timeoutMs: 5_000,
	...
	it("constructs all production remote adapters from bounded environment configuration", () => {
		const config = environmentRemoteExecutionConfiguration({
			KESTREL_REMOTE_TARGETS: JSON.stringify([
				{
					id: "docker",
					kind: "docker",
					backendId: "docker-cli",
		...
		expect(config?.backends.map((backend) => backend.id)).toEqual([
			"docker-cli",
			"ssh-cli",
			"kubernetes-cli",
			"serverless-http",
		...
		expect(() =>
			environmentRemoteExecutionConfiguration({
				KESTREL_REMOTE_TARGETS: "not-json",
			}),
		).toThrow("valid JSON");
	});
});
