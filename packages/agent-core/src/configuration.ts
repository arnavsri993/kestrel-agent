...
const builtInSurfaces: AgentConfigurationSurface[] = [
	{
		...
		riskLevel: "low",
		liveEffect:
			"Applied to new model turns without changing protected prompts.",
		examples: [
			"Be more concise.",
	...
	},
	{
		...
		riskLevel: "sensitive",
		liveEffect:
			"Added to subsequent agent turns after the protected core prompt.",
	...
	},
	{
		...
		examples: [
			"Do not capture new explicit memories.",
			"Use a 30-day failure window.",
		],
	},
	{
		id: "integrations",
		...
		examples: [
			"Never use hosted fallbacks.",
			"Ask before any hosted fallback.",
		],
	},
	{
		id: "settings",
...
function validateBaseCandidate(
	...
	for (const toolName of PROTECTED_RECOVERY_TOOLS) {
		if (
			!parsed.tools.enabled.some((pattern) => patternMatches(pattern, toolName))
		)
			throw new Error(
		...
		if (
			parsed.tools.disabled.some((pattern) => patternMatches(pattern, toolName))
		)
			throw new Error(
	...
	return [
		...
		},
		{
			...
			status: "passed",
			detail:
				"No credential or private-key pattern was found in editable text.",
		},
		{
			id: "recovery-reachability",
			status: "passed",
			detail:
				"Configuration history, approval, rollback, and undo tools remain reachable.",
		},
		{
...
export class AgentConfigurationManager {
	...
	constructor(
		...
		for (const surface of builtInSurfaces) this.registerSurface({ surface });
		this.initialize();
	}

	registerSurface(registration: AgentConfigurationSurfaceRegistration): void {
		const surface = AgentConfigurationSurfaceSchema.parse(registration.surface);
		if (this.surfaceRegistrations.has(surface.id))
			throw new Error(
				`Configuration surface ${surface.id} is already registered.`,
			);
		if (surface.editablePaths.some((path) => !EDITABLE_PATH_SET.has(path)))
			throw new Error(
	...
	rollbackPreview(targetVersionId: string): string {
		const current = this.currentVersion();
		const target = this.database.getAgentConfigurationVersion(targetVersionId);
		if (!target) throw new Error("Rollback target was not found.");
		if (!target.knownGood)
			throw new Error(
				"Only a known-good configuration version can be restored.",
			);
		...
		if (patch.length === 0)
			throw new Error(
				"The rollback target has the same effective configuration.",
			);
		return [
	...
	rollback(input: {
		...
		const current = this.currentVersion();
		const target = this.database.getAgentConfigurationVersion(
	...
	rejectProposal(
		...
		const proposal = this.database.getAgentConfigurationProposal(proposalId);
		if (!proposal) throw new Error("Configuration plan was not found.");
	...
	scanImprovements(force = true): AgentImprovementProposal[] {
		...
		>();
		for (const execution of this.database.listAllToolExecutions(
	...
	runImprovementScanIfDue(at = this.now()): AgentImprovementProposal[] {
		return this.scanImprovements(this.improvementScanDue(at, this.current()));
	}
	...
	instructions(): string {
		...
		const responseStyle = {
			concise:
				"Use concise responses while preserving essential evidence, risk, and next actions.",
			balanced:
	...
	private improvementScanDue(
		...
		const lastTimestamp =
			typeof last === "string" ? Date.parse(last) : Number.NaN;
		return (
...
export function installAgentConfigurationTools(
		...
		({ session }, input) => ({
			proposal: manager.plan({
				...
				...(Array.isArray(input.patch)
					? {
							patch: AgentConfigurationPatchOperationSchema.array().parse(
								input.patch,
							),
						}
					: {}),
			...
			next: "Explain the diff and isolated checks. Apply only through agent.config.apply so the user receives a fresh one-time approval boundary.",
		}),
	...
	register(
		...
		(_context, input) => ({
			...
			liveConfigurationChanged: false,
			next: "Explain this restoration diff, then call agent.config.rollback with the exact preview so the user receives a fresh one-time approval.",
		}),
	);
	register(
		{
	...
	register(
		...
		(_context, _input, output) => {
			...
			if (manager.currentVersion().id !== version.id)
				throw new Error(
					"Configuration rollback read-back verification failed.",
				);
			return {
