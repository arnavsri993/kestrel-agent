...
function validateEndpoint(value: string): URL {
	...
	if (
		...
		throw new Error(
			"The OTLP endpoint must use HTTPS, except for a loopback collector.",
		);
	}
	if (
		endpoint.username ||
		...
		throw new Error(
			"The OTLP endpoint must not contain credentials, query parameters, or a fragment.",
		);
	}
	return endpoint;
}
...
function validateConfiguration(
	...
	if (
		...
		!configuration.prometheus.enabled
	) {
		throw new Error(
			"Enable OTLP or Prometheus before turning on observability.",
		);
	}
	if (configuration.otlp.enabled) {
		validateEndpoint(configuration.otlp.endpoint);
		if (!configuration.otlp.metrics && !configuration.otlp.traces)
			throw new Error("Enable OTLP metrics or traces.");
	}
...
function signalEndpoint(base: URL, signal: "metrics" | "traces"): string {
	...
	endpoint.pathname =
		`${endpoint.pathname.replace(/\/+$/, "")}/v1/${signal}`.replace(
			/^\/?/,
			"/",
		);
...
function labels(values: Record<string, string>): string {
	...
	return entries.length
		? `{${entries.map(([key, value]) => `${key}="${escapePrometheusLabel(value)}"`).join(",")}}`
		: "";
}

function aggregateModelCalls(
	stats: Array<{
...
export function renderPrometheusMetrics(database: KestrelDatabase): string {
	...
	const toolGroups = new Map<
		string,
		{ tool: string; outcome: string; count: number }
	>();
	...
	for (const group of toolGroups.values())
		push(
			`workstrand_tool_executions_total${labels({ outcome: group.outcome, tool: group.tool })} ${group.count}`,
		);
	output.push(
	...
	for (const group of modelGroups) {
		...
		push(
			`workstrand_model_tokens_total${labels({ ...base, token_type: "input" })} ${group.inputTokens}`,
		);
		push(
			`workstrand_model_tokens_total${labels({ ...base, token_type: "output" })} ${group.outputTokens}`,
		);
		push(
			`workstrand_model_cost_usd_total${labels(base)} ${group.costUsd.toFixed(8)}`,
		);
		let cumulative = 0;
		for (const bucket of HISTOGRAM_BUCKETS_SECONDS) {
			cumulative += group.durations.filter(
				(duration) =>
					duration >
						(HISTOGRAM_BUCKETS_SECONDS[
							HISTOGRAM_BUCKETS_SECONDS.indexOf(bucket) - 1
						] ?? -Infinity) && duration <= bucket,
			).length;
			push(
				`workstrand_model_call_duration_seconds_bucket${labels({ ...base, le: String(bucket) })} ${cumulative}`,
			);
		}
		push(
			`workstrand_model_call_duration_seconds_bucket${labels({ ...base, le: "+Inf" })} ${group.durations.length}`,
		);
		push(
			`workstrand_model_call_duration_seconds_sum${labels(base)} ${group.durations.reduce((sum, value) => sum + value, 0).toFixed(6)}`,
		);
		push(
			`workstrand_model_call_duration_seconds_count${labels(base)} ${group.durations.length}`,
		);
	}
...
export class ObservabilityManager {
	...
	constructor(
		...
		for (const call of database.listAllModelCallAudits())
	...
	status(): ObservabilityStatus {
		...
		const hasHeaderValue = Boolean(
			this.database.getPrivateState<string>(HEADER_VALUE_KEY),
		);
		return {
			running: Boolean(
				configuration.enabled &&
					((configuration.otlp.enabled &&
						(this.meterProvider || this.tracerProvider)) ||
						configuration.prometheus.enabled),
			),
			otlpConfigured: Boolean(
				configuration.otlp.enabled && configuration.otlp.endpoint,
			),
			prometheusAvailable: Boolean(
				configuration.enabled && configuration.prometheus.enabled,
			),
			hasHeaderValue,
			...
			...(exported
				? { lastExportAt: exported.at, lastExportState: exported.state }
				: {}),
		};
	}
	...
	async configure(
		...
		if (headerValue !== undefined) {
			...
			if (!clean || clean.length > 20_000 || /[\r\n\0]/.test(clean))
				throw new Error("The OTLP header value is invalid.");
	...
	async test(): Promise<void> {
		const configuration = this.configuration();
		if (!configuration.enabled || !configuration.otlp.enabled)
			throw new Error("Enable and save OTLP export before testing it.");
		try {
			this.scanModelCalls();
			await Promise.all([
			...
			this.database.setState(STATUS_KEY, {
				state: "success",
				at: this.now().toISOString(),
			} satisfies StoredExportStatus);
		} catch {
			this.database.setState(STATUS_KEY, {
				state: "error",
				at: this.now().toISOString(),
			...
			throw new Error(
				"The OTLP collector did not accept the content-free test export.",
			);
		}
	}
	...
	private start(): void {
		...
		const headers = headerValue
			? { [configuration.otlp.headerName]: headerValue }
			: {};
		const resource = resourceFromAttributes({
			[ATTR_SERVICE_NAME]: configuration.otlp.serviceName,
		});
		if (configuration.otlp.metrics) {
			...
			const reader = new PeriodicExportingMetricReader({
				exporter,
				exportIntervalMillis: configuration.otlp.exportIntervalMs,
			...
			this.tracerProvider = new BasicTracerProvider({
				...
				spanProcessors: [
					new BatchSpanProcessor(exporter, {
						...
						scheduledDelayMillis: Math.min(
							configuration.otlp.exportIntervalMs,
							5_000,
						),
						exportTimeoutMillis: 10_000,
					}),
		...
		this.modelScanTimer = setInterval(
			() => this.scanModelCalls(),
			Math.max(1_000, configuration.otlp.exportIntervalMs),
		);
	...
	private registerMetrics(provider: MeterProvider): void {
		...
		const aggregate = [
			[
				...
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.sessions,
			],
			[
				"workstrand.messages",
				"Durable message envelopes without message content.",
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.messages,
			],
			[
			...
			[
				"workstrand.tool.executions",
				"Tool executions.",
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.toolExecutions,
			],
			[
				"workstrand.model.calls",
				"Provider model requests.",
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.modelCalls,
			],
			[
				"workstrand.model.failures",
				"Failed provider model requests.",
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.failedModelCalls,
			],
			[
				"workstrand.model.input_tokens",
				"Provider-reported input tokens.",
				(value: ReturnType<KestrelDatabase["organizationAnalytics"]>) =>
					value.inputTokens,
			],
			[
				"workstrand.model.output_tokens",
		...
		for (const [name, description, select] of aggregate) {
			meter
				.createObservableCounter(name, { description })
				.addCallback((result) =>
					result.observe(select(this.database.organizationAnalytics())),
				);
		}
		meter
			.createObservableCounter("workstrand.model.cost_usd", {
				description: "Configured-rate estimated model spend.",
				unit: "USD",
			})
			.addCallback((result) =>
				result.observe(this.database.organizationAnalytics().estimatedCostUsd),
			);
		meter
			.createObservableGauge("workstrand.process.memory_bytes", {
				description: "Current exporter process memory.",
				unit: "By",
			})
			.addCallback((result) => {
				const memory = process.memoryUsage();
		...
		this.runtimeListener = (event) => {
			...
			const toolName =
				typeof event.payload.toolName === "string"
					? boundedLabel(event.payload.toolName)
					: "unknown";
			if (event.type === "tool.started" && event.executionId) {
				this.activeToolSpans.set(
					event.executionId,
					tracer.startSpan("workstrand.tool.execution", {
						attributes: {
							"gen_ai.tool.name": toolName,
							"workstrand.outcome": "started",
						},
			...
			if (event.type === "tool.completed" && event.executionId) {
				const span =
					this.activeToolSpans.get(event.executionId) ??
					tracer.startSpan("workstrand.tool.execution", {
				...
				const outcome =
					typeof event.payload.status === "string"
						? boundedLabel(event.payload.status)
						: "unknown";
