...
export interface ScheduledAgentJob {
  ...
  prompt: string;
  instructions?: string;
  schedule: { kind: "once"; nextRunAt: string } | { kind: "interval"; nextRunAt: string; intervalMs: number } | { kind: "cron"; nextRunAt: string; expression: string };
...
function cronField(value: string, minimum: number, maximum: number): (candidate: number) => boolean {
  ...
  if (step) { const amount = Number(step[1]); if (amount < 1 || amount > maximum - minimum + 1) throw new Error("Cron step is invalid."); return (candidate) => (candidate - minimum) % amount === 0; }
  const values = value.split(",").map(Number);
  if (values.some((item) => !Number.isInteger(item) || item < minimum || item > maximum)) throw new Error("Cron field is invalid.");
  return (candidate) => values.includes(candidate);
}

export function nextCronOccurrence(expression: string, after: Date): Date {
  const fields = expression.trim().split(/\s+/);
  if (fields.length !== 5) throw new Error("Cron expressions require five fields.");
  const [minute, hour, day, month, weekday] = fields as [string, string, string, string, string];
  const matches = [cronField(minute, 0, 59), cronField(hour, 0, 23), cronField(day, 1, 31), cronField(month, 1, 12), cronField(weekday, 0, 6)];
  const candidate = new Date(after.getTime());
  candidate.setUTCSeconds(0, 0); candidate.setUTCMinutes(candidate.getUTCMinutes() + 1);
  for (let count = 0; count < 527_040; count += 1, candidate.setUTCMinutes(candidate.getUTCMinutes() + 1)) {
    if (matches[0]!(candidate.getUTCMinutes()) && matches[1]!(candidate.getUTCHours()) && matches[2]!(candidate.getUTCDate()) && matches[3]!(candidate.getUTCMonth() + 1) && matches[4]!(candidate.getUTCDay())) return new Date(candidate);
  }
  throw new Error("Cron expression has no occurrence within one year.");
}

export function parseScheduleExpression(expression: string, now = new Date()): ScheduledAgentJob["schedule"] {
  const value = expression.trim().toLowerCase();
  const every = value.match(/^every\s+(\d+)\s+(second|seconds|minute|minutes|hour|hours|day|days)$/);
  if (every) {
  ...
  if (tomorrow) {
    let hour = Number(tomorrow[1]); const minute = Number(tomorrow[2] ?? 0);
    if (tomorrow[3]) { if (hour < 1 || hour > 12) throw new Error("Natural-language time is invalid."); hour %= 12; if (tomorrow[3] === "pm") hour += 12; }
    if (hour > 23 || minute > 59) throw new Error("Natural-language time is invalid.");
    const next = new Date(now); next.setUTCDate(next.getUTCDate() + 1); next.setUTCHours(hour, minute, 0, 0);
    return { kind: "once", nextRunAt: next.toISOString() };
  }
  if (value.split(/\s+/).length === 5) return { kind: "cron", expression: value, nextRunAt: nextCronOccurrence(value, now).toISOString() };
  const instant = new Date(expression);
  if (!Number.isFinite(instant.getTime()) || instant <= now) throw new Error("Schedule expression is not a future time, interval, or five-field cron.");
...
function resolveInput(value: unknown, results: Record<string, RuntimeToolExecution>): unknown {
...
function substituteItem(value: unknown, item: unknown): unknown {
  ...
  if (Array.isArray(value)) return value.map((entry) => substituteItem(entry, item));
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, substituteItem(entry, item)]));
  return value;
}
...
function workflowConditionMatches(condition: WorkflowStep["when"], results: Record<string, RuntimeToolExecution>): boolean {
  ...
  return !("equals" in condition) || JSON.stringify(value) === JSON.stringify(condition.equals);
}
...
export class TaskOrchestrator {
  ...
  async delegate(input: DelegatedTaskInput): Promise<DelegatedTaskResult> {
    ...
    if (input.isolateWorktree) {
      if (!parent.workspaceRoot) throw new Error("Worktree isolation requires a workspace-backed parent session.");
      const branch = `kestrel/${taskId.slice(-12)}`;
      const worktree = await this.runtime.callTool(parent.id, "git.worktree-create", { branch, startPoint: "HEAD", createBranch: true }, { approvalStatus: "approved", idempotencyKey: `delegate:${taskId}:worktree` });
    ...
    try {
      ...
      const result = await this.loop.run({
        ...
        ...(input.providerModels || selected ? { providerModels: { ...selected?.execution.providerModels, ...input.providerModels } } : {}),
        ...(selected?.route.reasoningEffort || input.reasoningEffort ? { reasoningEffort: selected?.route.reasoningEffort ?? input.reasoningEffort } : {}),
        ...(selected ? {
      ...
      this.database.setPrivateState(`orchestrator.task.${taskId}`, {
        taskId, sessionId: session.id, parentSessionId: parent.id, title: input.title, status: terminalStatus, error: "Delegated task failed.", updatedAt: this.now().toISOString()
      });
      throw error;
    }
  }
  ...
  private async selectWorker(input: DelegatedTaskInput, taskId: string): Promise<{
    ...
    if (!this.providers || !this.modelRouter || !this.modelRegistry)
      throw new Error("Automatic delegation is unavailable because the adaptive model registry is not attached.");
    ...
    const parentRun = input.role === "reviewer"
    ...
    const reviewedModelIds = parentRun
      ? this.modelRegistry.list()
          .filter((profile) =>
            ...
            && parentRun.providerIds.some((providerId) =>
              providerId === profile.provider || providerId === profile.endpointId))
    ...
    for (let attempt = 0; attempt <= policy.maximumRetries; attempt += 1) {
      ...
      const checked = verification.find((item) => item.providerId === decision.endpointId);
      if (checked?.ok) {
        ...
        return {
          route: {
            ...
            ...(decision.estimatedCost === undefined ? {} : { estimatedCost: decision.estimatedCost }),
  ...
  async runTeam(inputs: DelegatedTaskInput[], maximumConcurrency = this.maximumWorkers): Promise<Array<DelegatedTaskResult | Error>> {
    ...
    const worker = async () => {
      while (cursor < inputs.length) {
        ...
        if (!input) continue;
        try { results[index] = await this.delegate(input); }
        catch (error) { results[index] = error instanceof Error ? error : new Error("Delegated task failed."); }
      }
    };
    await Promise.all(Array.from({ length: Math.min(maximumConcurrency, inputs.length) }, () => worker()));
    return results;
  }
  ...
  teamUsage(teamId: string): { runs: number; inputTokens: number; outputTokens: number } {
    ...
    const runs = team.memberSessionIds.flatMap((sessionId) => this.database.listAgentRuns(sessionId));
    const calls = runs.flatMap((run) => this.database.listModelCallAudits(run.id));
    return { runs: runs.length, inputTokens: calls.reduce((sum, call) => sum + call.inputTokens, 0), outputTokens: calls.reduce((sum, call) => sum + call.outputTokens, 0) };
  }

  updateTeam(teamId: string, input: { memberSessionIds?: string[]; sharedPlan?: string[] }): TeamRecord {
  ...
  sendPeerMessage(teamId: string, fromSessionId: string, toSessionId: string, text: string): TeamMessage {
    const teams = this.listTeams();
    const index = teams.findIndex((team) => team.id === teamId);
  ...
  cancelJob(id: string): ScheduledAgentJob {
    ...
    const updated = { ...job, status: "cancelled" as const, updatedAt: this.now().toISOString() };
    jobs[index] = updated;
    this.saveJobs(jobs);
    return updated;
  }
  ...
  startWorkflow(sessionId: string, title: string, steps: WorkflowStep[]): Promise<WorkflowRecord> {
    ...
    const expanded = steps.flatMap((step) => {
      ...
      if (step.forEach.length === 0 || step.forEach.length > 20) throw new Error("Workflow loops require from 1 through 20 items.");
      const { forEach: _items, ...template } = step;
      return step.forEach.map((item, index) => ({ ...template, id: `${step.id}[${index}]`, input: substituteItem(step.input, item) as Record<string, unknown> }));
    });
    if (expanded.length > 200) throw new Error("Expanded workflows cannot exceed 200 steps.");
    const timestamp = this.now().toISOString();
    const workflow: WorkflowRecord = {
      id: `workflow-${randomUUID()}`, sessionId, title, steps: expanded, nextStep: 0, status: "running", results: {}, createdAt: timestamp, updatedAt: timestamp
    };
    this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
  ...
  private async continueWorkflow(initial: WorkflowRecord, approved = new Set<string>()): Promise<WorkflowRecord> {
    let workflow = initial;
    for (let index = workflow.nextStep; index < workflow.steps.length; index += 1) {
      const step = workflow.steps[index]!;
      if (!workflowConditionMatches(step.when, workflow.results)) {
        workflow = { ...workflow, nextStep: index + 1, updatedAt: this.now().toISOString() };
        this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
        continue;
      }
      const input = resolveInput(step.input, workflow.results) as Record<string, unknown>;
      const execution = await this.runtime.callTool(workflow.sessionId, step.toolName, input, {
        approvalStatus: approved.has(step.id) ? "approved" : step.approvalStatus ?? "pending",
        idempotencyKey: `${workflow.id}:${step.id}`
      });
      if (execution.status === "blocked") {
        workflow = { ...workflow, status: execution.output?.approvalRequired === true ? "waiting_approval" : "failed", nextStep: index, results: { ...workflow.results, [step.id]: execution }, updatedAt: this.now().toISOString() };
        this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
        return workflow;
      }
      if (execution.status !== "verified") {
        workflow = { ...workflow, status: "failed", nextStep: index, results: { ...workflow.results, [step.id]: execution }, updatedAt: this.now().toISOString() };
        this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
        return workflow;
      }
      workflow = { ...workflow, nextStep: index + 1, results: { ...workflow.results, [step.id]: execution }, updatedAt: this.now().toISOString() };
      this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
    }
    workflow = { ...workflow, status: "completed", updatedAt: this.now().toISOString() };
    this.database.setPrivateState(`orchestrator.workflow.${workflow.id}`, workflow);
    return workflow;
  }
  ...
  private finishJob(job: ScheduledAgentJob, result: AgentLoopResult, at: Date): ScheduledAgentJob {
    if (result.run.status === "waiting_approval") {
      return { ...job, status: "waiting_approval", lastRunId: result.run.id, error: undefined, updatedAt: this.now().toISOString() };
    }
    if (result.run.status !== "completed") {
      return { ...job, status: "failed", lastRunId: result.run.id, error: `Agent run ended with status ${result.run.status}.`, updatedAt: this.now().toISOString() };
    }
...
export function installOrchestrationTools(runtime: AgentRuntime, orchestrator: TaskOrchestrator, sessionId: string): void {
  ...
  add("orchestration.delegate", "Delegate one focused child-agent task. Automatic routing selects a verified model by required capability, quality, reliability, latency, cost, locality, and policy; use a reviewer role for independent validation.", false, { type: "object", properties: { parentSessionId: { type: "string" }, title: { type: "string" }, prompt: { type: "string" }, model: { type: "string", default: "auto" }, providerIds: { type: "array", items: { type: "string" }, minItems: 1, default: ["auto"] }, role: { enum: ["orchestrator", "worker", "reviewer", "fallback"], default: "worker" }, requiredCapabilities: { type: "object", additionalProperties: { type: "number", minimum: 0, maximum: 1 } }, allowedTools: { type: "array", items: { type: "string" } }, isolateWorktree: { type: "boolean" } }, required: ["parentSessionId", "title", "prompt"], additionalProperties: false }, async (context, input) => ({ delegated: await orchestrator.delegate({ parentSessionId: String(input.parentSessionId), title: String(input.title), prompt: String(input.prompt), model: input.model ? String(input.model) : "auto", providerIds: Array.isArray(input.providerIds) ? input.providerIds.map(String) : ["auto"], ...(input.role ? { role: input.role as NonNullable<DelegatedTaskInput["role"]> } : {}), ...(input.requiredCapabilities && typeof input.requiredCapabilities === "object" ? { requiredCapabilities: input.requiredCapabilities as Partial<Record<ModelCapability, number>> } : {}), ...(Array.isArray(input.allowedTools) ? { allowedTools: input.allowedTools.map(String) } : {}), isolateWorktree: Boolean(input.isolateWorktree), signal: context.signal }) }));
  ...
  add("team.update", "Update team membership or shared plan", false, { type: "object", properties: { teamId: { type: "string" }, memberSessionIds: { type: "array", items: { type: "string" }, minItems: 1 }, sharedPlan: { type: "array", items: { type: "string" } } }, required: ["teamId"], additionalProperties: false }, async (_context, input) => ({ team: orchestrator.updateTeam(String(input.teamId), { ...(Array.isArray(input.memberSessionIds) ? { memberSessionIds: input.memberSessionIds.map(String) } : {}), ...(Array.isArray(input.sharedPlan) ? { sharedPlan: input.sharedPlan.map(String) } : {}) }) }));
  add("team.message", "Send a provenance-backed peer message", false, { type: "object", properties: { teamId: { type: "string" }, fromSessionId: { type: "string" }, toSessionId: { type: "string" }, text: { type: "string" } }, required: ["teamId", "fromSessionId", "toSessionId", "text"], additionalProperties: false }, async (_context, input) => ({ message: orchestrator.sendPeerMessage(String(input.teamId), String(input.fromSessionId), String(input.toSessionId), String(input.text)) }));
}

export class AutomationDaemon {
  constructor(private readonly orchestrator: TaskOrchestrator, private readonly pollMs = 5_000, private readonly now: () => Date = () => new Date()) {
    if (!Number.isInteger(pollMs) || pollMs < 250 || pollMs > 300_000) throw new Error("Automation poll interval must be between 250 ms and 5 minutes.");
  }
