...
export interface RuntimeHook {
	...
	): RuntimeHookResult | Promise<RuntimeHookResult>;
}
...
export interface DeclarativeRuntimeHook {
	...
	toolGlob?: string;
	conditions?: Array<{
		field: "tool.name" | "session.id" | "execution.status" | `input.${string}`;
		equals?: unknown;
		matches?: string;
	}>;
	action:
...
function globPattern(value: string): RegExp {
	if (!value || value.length > 200)
		throw new Error("Hook tool glob is invalid.");
	return new RegExp(
...
function hookField(
	context: RuntimeHookContext,
	field: DeclarativeRuntimeHook["conditions"] extends Array<infer T> | undefined
		? T extends { field: infer F }
			? F
			: never
		: never,
): unknown {
...
interface RuntimeToolDefinition {
	...
	verify?(
		...
		| { method: string; evidence: unknown }
		| Promise<{ method: string; evidence: unknown }>;
}
...
const binaryReadInputSchema = z.object({
	path: z.string().min(1),
	maxBytes: z.number().int().positive().max(1_000_000).default(256_000),
});
...
const gitStageInputSchema = z.object({
	pathspec: z.array(z.string().min(1).max(1_000)).min(1).max(100),
});
...
const githubPrInputSchema = z.object({
	...
	head: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._/-]{0,199}$/),
	base: z
		.string()
		.regex(/^[A-Za-z0-9][A-Za-z0-9._/-]{0,199}$/)
		.default("main"),
...
const githubRefInputSchema = z.object({
	remote: z
		.string()
		.regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/)
		.default("origin"),
...
const githubReviewInputSchema = z.object({
	remote: z
		.string()
		.regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/)
		.default("origin"),
...
function resolveExistingPath(root: string, requestedPath: string): string {
	...
	if (!isWithin(root, candidate))
		throw new Error("Workspace path escapes the granted root.");
	return candidate;
}
...
function resolveWritablePath(root: string, requestedPath: string): string {
	...
	if (!isWithin(root, parent))
		throw new Error("Workspace parent path escapes the granted root.");
	if (existsSync(candidate)) {
		if (lstatSync(candidate).isSymbolicLink())
			throw new Error("Workspace mutations do not follow symbolic links.");
...
function parsePersistedCheckpointState(
	value: unknown,
): PersistedCheckpointState | undefined {
...
export class AgentRuntime extends EventEmitter {
	...
	private readonly deferredTools = new Map<
		string,
		{ catalogId: string; descriptor: RuntimeToolDescriptor }
	>();
	...
	private readonly backgroundProcesses = new Map<
		string,
		{
			sessionId: string;
	...
	constructor(
		...
		if (previousProcesses.some((process) => process.status === "running"))
				...
				previousProcesses.map((process) =>
					process.status === "running"
						? {
								...process,
								status: "interrupted",
								error: "The host restarted while this process was running.",
								updatedAt: this.now(),
							}
						: process,
				),
	...
	ensureMainSession(): RuntimeSession {
		...
		const storedId =
			typeof storedValue === "string" && storedValue ? storedValue : undefined;
		const existing = storedId
			? this.database.getRuntimeSession(storedId)
			: undefined;
	...
	createSession(input: {
		...
		this.emitRuntimeEvent("session.created", session.id, {
			title: session.title,
		});
		return session;
	}
	...
	workspaceInstructions(
		sessionId: string,
		targetPath?: string,
	): Array<{ path: string; content: string; precedence: number }> {
	...
	checkpoint(sessionId: string, summary: string): RuntimeSession {
		...
		this.database.setPrivateState(`session.checkpoint.${checkpoint.id}`, {
			sessionId,
			messageCount: this.listMessages(sessionId).length,
			mutationIds: this.database.listWorkspaceMutationIds(sessionId),
		});
	...
	restoreCheckpoint(sessionId: string, checkpointId: string): RuntimeSession {
		...
		if (!state || state.sessionId !== sessionId)
			throw new Error("Checkpoint restoration state is unavailable.");
		...
		for (const mutation of this.database
			...
			.filter((item) => !baseline.has(item.id) && !item.undoneAt))
			this.undoWorkspaceMutation(sessionId, mutation.id);
	...
	rewindLastTurn(sessionId: string): { message: string } {
		...
		const matchingRun = [...this.database.listAgentRuns(sessionId)]
			.reverse()
			.find((run) => {
				const baseline = this.database.getPrivateState<{
					userMessageId?: string;
				}>(`agent-run-baseline.${run.id}`);
				return baseline?.userMessageId === user.id;
			});
		const baseline = matchingRun
			? this.database.getPrivateState<{
					sessionId: string;
					userMessageId: string;
					messageCount: number;
					mutationIds: string[];
		...
		if (baseline?.sessionId === sessionId) {
			...
			for (const mutation of this.database
				.listWorkspaceMutations(sessionId)
				.filter((item) => !item.undoneAt && !mutationIds.has(item.id)))
				this.undoWorkspaceMutation(sessionId, mutation.id);
		} else {
			// Legacy turns created before exact run baselines use the safest available fallback.
			for (const mutation of this.database
				.listWorkspaceMutations(sessionId)
				.filter((item) => !item.undoneAt && item.createdAt >= user.createdAt))
				this.undoWorkspaceMutation(sessionId, mutation.id);
		}
		const prunedCheckpointIds = session.checkpoints
			.filter((checkpoint) => {
	...
	forkSession(sessionId: string, title?: string): RuntimeSession {
		...
		for (const message of this.listMessages(parent.id)) {
			const cloned = this.appendMessage({
				...
				...(message.parentMessageId && messageIds.has(message.parentMessageId)
					? { parentMessageId: messageIds.get(message.parentMessageId)! }
					: {}),
				...(message.modelToolCalls
					? { modelToolCalls: message.modelToolCalls }
					: {}),
	...
	resumeSession(sessionId: string): RuntimeSession {
		...
		if (session.status === "cancelled")
			throw new Error("A cancelled session cannot be resumed.");
		const updated = this.saveSession({
			...session,
			status: "active",
			updatedAt: this.now(),
	...
	searchMessages(query: string, limit = 20): RuntimeMessage[] {
		...
		const ranked = this.database
			...
			.map((message) => {
				...
				const lexical = queryTerms.reduce(
					(total, term) =>
						total +
						(body.includes(term)
							? 4
				...
				const semantic = semanticSimilarity(
					queryEmbedding,
					localSemanticEmbedding(message.content),
				);
				return { message, score: lexical + semantic * 3, semantic };
			})
			.filter(({ score, semantic }) => score > 0.55 || semantic >= 0.18)
			.sort(
				(left, right) =>
	...
	listApprovalRules(): ApprovalRule[] {
		return (
			this.database.getPrivateState<ApprovalRule[]>(this.approvalRulesKey) ?? []
		);
	}

	setApprovalRule(
		input: Pick<ApprovalRule, "toolName" | "decision" | "scope"> & {
			sessionId?: string;
		},
	): ApprovalRule {
		...
		if (input.scope === "session") {
			if (!input.sessionId)
				throw new Error("Session-scoped approval rules require a session.");
			this.requireSession(input.sessionId);
		}
		const timestamp = this.now();
		...
		if (!existing && records.length >= AgentRuntime.MAX_APPROVAL_RULES)
			throw new Error("Approval rule limit reached.");
		const rule = ApprovalRuleSchema.parse({
			id: existing?.id ?? `approval-rule-${randomUUID()}`,
			...input,
			createdAt: existing?.createdAt ?? timestamp,
			updatedAt: timestamp,
		});
		this.database.setPrivateState(this.approvalRulesKey, [
			...records.filter((candidate) => candidate.id !== rule.id),
			rule,
		]);
		return rule;
	}
	...
	undoWorkspaceMutation(
		sessionId: string,
		mutationId?: string,
	): { mutationId: string; path: string; restored: boolean } {
		const session = this.requireSession(sessionId);
		if (!session.workspaceRoot)
			throw new Error("Workspace root is unavailable.");
		const mutation = mutationId
			? this.database.getWorkspaceMutation(mutationId)
			: this.database
					.listWorkspaceMutations(session.id)
					.find((item) => !item.undoneAt);
		if (!mutation || mutation.sessionId !== session.id)
			throw new Error("Undoable workspace mutation not found in this session.");
		if (mutation.undoneAt)
			throw new Error("Workspace mutation has already been undone.");
		...
		if (mutation.operation === "create") {
			if (existsSync(path)) {
				if (mutation.entryKind === "directory") {
					if (!statSync(path).isDirectory() || readdirSync(path).length > 0)
						throw new Error(
							"Cannot undo because the created directory is no longer empty.",
						);
					rmdirSync(path);
				} else {
					if (
						mutation.afterContent !== undefined &&
						this.readMutationText(path) !== mutation.afterContent
					)
						throw new Error(
							"Cannot undo because the created file changed afterward.",
						);
					unlinkSync(path);
				}
			}
			...
			mutation.entryKind === "directory"
		) {
			if (existsSync(path))
				throw new Error(
					"Cannot undo delete because the directory path is occupied.",
				);
			...
			if (mutation.beforeContent === undefined)
				throw new Error("Mutation is missing its encrypted recovery content.");
			if (
				mutation.operation === "update" &&
				(!existsSync(path) ||
					(mutation.afterContent !== undefined &&
				...
				throw new Error(
					"Cannot undo because the updated file changed afterward.",
				);
			if (mutation.operation === "delete" && existsSync(path))
				throw new Error("Cannot undo delete because the path is occupied.");
			this.writeTextAtomically(path, mutation.beforeContent);
		} else if (mutation.operation === "move") {
			if (!mutation.destinationPath)
				throw new Error("Move mutation is missing its destination path.");
			const destination = resolveExistingPath(
				session.workspaceRoot,
				mutation.destinationPath,
			);
			if (existsSync(path))
				throw new Error(
					"Cannot undo move because the original path is occupied.",
				);
	...
	registerHook(hook: RuntimeHook): void {
		if (this.hooks.some((candidate) => candidate.id === hook.id))
			throw new Error(`Hook ${hook.id} is already registered.`);
	...
	registerDeclarativeHook(config: DeclarativeRuntimeHook): void {
		if (!/^[a-z0-9][a-z0-9_.-]{0,127}$/.test(config.id))
			throw new Error("Declarative hook ID is invalid.");
		if (config.conditions && config.conditions.length > 20)
			throw new Error("Declarative hooks support at most 20 conditions.");
		if (config.action.kind === "block" && config.event !== "pre_tool")
			throw new Error("Only pre-tool declarative hooks can block execution.");
		const toolPattern = config.toolGlob
			? globPattern(config.toolGlob)
			: undefined;
		const conditions = (config.conditions ?? []).map((condition) => ({
			...condition,
			...(condition.matches ? { matcher: globPattern(condition.matches) } : {}),
		}));
		this.registerHook({
			id: config.id,
			event: config.event,
			...
			run: (context) => {
				const matched = conditions.every((condition) => {
					...
					return (
						!condition.matcher ||
						(typeof value === "string" && condition.matcher.test(value))
					);
				});
				if (!matched) return {};
				if (config.action.kind === "block")
					return {
						blocked: true,
						reason: config.action.reason.slice(0, 2_000),
					};
				this.emitRuntimeEvent(
					"tool.progress",
					context.session.id,
					{ hookId: config.id, message: config.action.message.slice(0, 2_000) },
	...
	registerDeferredCatalog(catalog: DeferredToolCatalog): void {
		if (!/^[a-z][a-z0-9_.-]+$/.test(catalog.id))
			throw new Error("Deferred catalog ID is invalid.");
	...
	refreshDeferredCatalog(catalogId: string): RuntimeToolDescriptor[] {
		...
		if (!catalog)
			throw new Error(`Deferred catalog ${catalogId} is not registered.`);
		for (const [name, entry] of this.deferredTools)
			if (entry.catalogId === catalogId) this.deferredTools.delete(name);
		for (const raw of catalog.list()) {
			...
			if (existing && existing.catalogId !== catalogId)
				throw new Error(
					`Deferred tool ${descriptor.name} is already registered by ${existing.catalogId}.`,
				);
		...
		return [...this.deferredTools.values()]
			...
			.filter(
				(tool) =>
					terms.length === 0 ||
					terms.every((term) =>
						`${tool.name} ${tool.title} ${tool.description} ${tool.tags.join(" ")}`
							.toLowerCase()
							.includes(term),
					),
			)
	...
	async activateDeferredTool(
		...
		if (!catalog)
			throw new Error(`Deferred catalog ${entry.catalogId} is unavailable.`);
		const tool = await catalog.activate(name);
		const descriptor = RuntimeToolDescriptorSchema.parse(tool.descriptor);
		if (JSON.stringify(descriptor) !== JSON.stringify(entry.descriptor))
			throw new Error(
				"Activated tool metadata does not match its deferred catalog descriptor.",
			);
	...
	unregisterExternalTool(toolName: string): void {
		...
		if (definition.descriptor.source === "builtin")
			throw new Error("Built-in runtime tools cannot be unregistered.");
		this.tools.delete(toolName);
		for (const session of this.listSessions()) {
			if (!session.allowedTools.includes(toolName)) continue;
			this.saveSession({
				...session,
				allowedTools: session.allowedTools.filter((name) => name !== toolName),
				updatedAt: this.now(),
			});
		}
	}
	...
	discoverTools(sessionId: string, query?: string): RuntimeToolDescriptor[] {
		...
		const hasActiveWorkspace = Boolean(
			this.resolveActiveWorkspaceRoot(session),
		);
		const terms = query?.toLowerCase().split(/\s+/).filter(Boolean) ?? [];
	...
	private registerDiscoveryTools(): void {
		this.registerTool({
			descriptor: {
				...
				description:
					"Search active and deferred tools by capability before loading them.",
				category: "extension",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: false,
				source: "builtin",
				tags: ["tools", "discovery", "deferred", "catalog"],
			},
		...
		this.registerTool({
			descriptor: {
				name: "tools.activate",
				title: "Activate deferred tool",
				description:
					"Load one discovered deferred tool after explicit approval.",
				category: "extension",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: false,
				source: "builtin",
				tags: ["tools", "discovery", "deferred", "activate"],
			},
			inputSchema: z.object({ name: z.string().min(1) }),
			outputSchema: z.record(z.string(), z.unknown()),
			execute: async ({ session }, input) => ({
				descriptor: await this.activateDeferredTool(
					session.id,
					String(input.name),
				),
			}),
	...
	async callTool(
		...
		if (definition.descriptor.requiresWorkspace && !workspaceRoot)
			throw new Error("This tool requires a user-granted workspace root.");
	...
	private async executeToolCall(
		...
		const approvalRule = this.listApprovalRules()
			.filter(
				(rule) =>
					...
					(rule.scope === "global" || rule.sessionId === session.id),
			)
			.sort((left, right) =>
				left.scope === "session" ? -1 : right.scope === "session" ? 1 : 0,
			)[0];
		...
		const policy = options.executionBlock
			...
			: approvalRule?.decision === "deny"
				? {
						allowed: false,
						approvalRequired: false,
						reason: `A persistent ${approvalRule.scope} rule denies ${toolName}.`,
					}
				: mayExecute({
						risk: effectiveRisk,
						...(alwaysRequireApproval
		...
		let execution = RuntimeToolExecutionSchema.parse({
			...
			startedAt,
			...(!policy.allowed
				? { error: policy.reason, completedAt: startedAt }
				: {}),
		});
		if (!policy.allowed) {
			...
			this.emitRuntimeEvent(
				"tool.started",
				session.id,
				{ toolName, status: execution.status },
			...
			if (controller && !controller.signal.aborted) {
				controller.abort(
					options.signal
						...
						: new Error("Execution cancelled by the user."),
				);
			}
		};
		try {
			...
			const context: RuntimeToolContext = {
				...
				progress: (payload) =>
					this.emitRuntimeEvent("tool.progress", session.id, payload, {
						executionId: activeExecutionId,
					}),
				...(workspaceRoot ? { workspaceRoot } : {}),
			};
			...
			const cancelled =
				controller?.signal.aborted === true || options.signal?.aborted === true;
	...
	private async acquireIdempotentExecution(
		...
		while (true) {
			...
			if (!processIsAlive(activeClaim.ownerPid)) {
				return this.recoverAbandonedIdempotentClaim(
					idempotencyKey,
					activeClaim,
				);
			}
			if (Date.now() - waitStartedAt >= 30_000) {
				throw new Error(
					"A matching tool execution is still running in another Kestrel process. Retry after it finishes or cancel this wait.",
				);
			}
			...
			const observed =
				this.database.getIdempotentClaim<RuntimeToolExecution>(idempotencyKey);
			if (observed) {
			...
			if (retry.state === "completed")
				return RuntimeToolExecutionSchema.parse(retry.result);
	...
	private recoverAbandonedIdempotentClaim(
		...
		const terminal =
			recorded && recorded.status !== "running"
				? recorded
	...
	private reconcileIdempotencyClaims(): void {
		for (const claim of this.database.listIdempotentClaims<RuntimeToolExecution>(
			"runtime-tool:",
		)) {
	...
	private registerWorkspaceTools(): void {
		this.registerTool({
			descriptor: {
				...
				description:
					"List files under a user-granted workspace without following paths outside it.",
				category: "workspace",
				riskLevel: "read_only",
				readOnly: true,
			...
			execute: ({ workspaceRoot }, input) => {
				...
				if (!statSync(start).isDirectory())
					throw new Error("workspace.list requires a directory path.");
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Read a bounded UTF-8 text file inside a user-granted workspace.",
				category: "workspace",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
			...
			execute: ({ workspaceRoot }, input) => {
				...
				if (!text)
					throw new Error("Binary files are not exposed as text context.");
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Search bounded text files inside a user-granted workspace and return matching lines.",
				category: "workspace",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
			...
			execute: ({ workspaceRoot }, input) => {
				...
				const files = statSync(start).isFile()
					? [start]
					: walkFiles(workspaceRoot, start, 2_000);
				...
				return {
					query: parsed.query,
					matches: matches.slice(0, parsed.maxMatches),
					truncated: matches.length > parsed.maxMatches,
				};
			},
		});
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Read a bounded binary file chunk inside a user-granted workspace with size, media type, and digest metadata.",
				category: "workspace",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
			...
			execute: ({ workspaceRoot }, input) => {
				...
				if (!metadata.isFile())
					throw new Error("workspace.read-binary requires a file path.");
				const chunk = Buffer.alloc(Math.min(metadata.size, parsed.maxBytes));
				const descriptor = openSync(path, "r");
				try {
					readSync(descriptor, chunk, 0, chunk.byteLength, 0);
				} finally {
				...
				return {
					...
					truncated: metadata.size > chunk.byteLength,
				};
			},
		});
		...
		this.registerTool({
			...
			execute: ({ workspaceRoot }, input) => {
				...
				return {
					instructions: this.loadInstructions(workspaceRoot, parsed.targetPath),
				};
			},
		});
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Create or replace a bounded UTF-8 file atomically and record an encrypted undo mutation.",
				category: "workspace",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				const before = existsSync(path)
					? this.readMutationText(path)
					: undefined;
				if (
					parsed.expectedContent !== undefined &&
					before !== parsed.expectedContent
				)
					throw new Error(
						"Workspace file changed since it was read; refusing to overwrite it.",
					);
				const operation =
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Delete one bounded UTF-8 file after explicit approval and retain an encrypted undo record.",
				category: "workspace",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				const path = resolveExistingPath(workspaceRoot, parsed.path);
				if (!statSync(path).isFile())
					throw new Error("workspace.delete only deletes files.");
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Create one directory inside the granted workspace and record an undo mutation.",
				category: "workspace",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
				source: "builtin",
				tags: ["directory", "create", "write", "undo"],
			},
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				const path = resolveWritablePath(workspaceRoot, parsed.path);
				if (existsSync(path))
					throw new Error("Workspace directory path already exists.");
				mkdirSync(path, { mode: 0o700 });
				const mutation = WorkspaceMutationSchema.parse({
					id: `mutation-${randomUUID()}`,
					sessionId: session.id,
		...
		this.registerTool({
			descriptor: {
				name: "workspace.rmdir",
				title: "Delete empty workspace directory",
				description:
					"Delete one empty directory after approval and retain an undo mutation.",
				category: "workspace",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
				source: "builtin",
				tags: ["directory", "delete", "undo", "sensitive"],
			},
			inputSchema: directoryInputSchema,
			outputSchema: z.object({
				path: z.string(),
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				if (!statSync(path).isDirectory())
					throw new Error("workspace.rmdir requires a directory path.");
				if (readdirSync(path).length > 0)
					throw new Error("workspace.rmdir only deletes empty directories.");
				rmdirSync(path);
				const mutation = WorkspaceMutationSchema.parse({
					id: `mutation-${randomUUID()}`,
					sessionId: session.id,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Apply exact checked text replacements atomically and retain an encrypted undo record.",
				category: "workspace",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				const path = resolveExistingPath(workspaceRoot, parsed.path);
				if (!statSync(path).isFile())
					throw new Error("workspace.patch requires a file path.");
				const before = this.readMutationText(path);
				const beforeHash = createHash("sha256").update(before).digest("hex");
				if (parsed.expectedSha256 && parsed.expectedSha256 !== beforeHash)
					throw new Error(
						"Workspace file hash changed since it was read; refusing to patch it.",
					);
				let after = before;
				let replacements = 0;
				for (const edit of parsed.edits) {
					...
					if (first < 0)
						throw new Error("Patch context was not found in the current file.");
					if (
						!edit.replaceAll &&
						after.indexOf(edit.oldText, first + edit.oldText.length) >= 0
					)
						throw new Error(
							"Patch context is ambiguous; set replaceAll or provide a more specific oldText value.",
						);
					if (edit.replaceAll) {
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Rename one file within the granted workspace and retain an undo record.",
				category: "workspace",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session, executionId, workspaceRoot }, input) => {
				...
				const entryKind = statSync(from).isDirectory()
					? ("directory" as const)
					: statSync(from).isFile()
						? ("file" as const)
						: undefined;
				if (!entryKind)
					throw new Error("workspace.move only moves files or directories.");
				if (existsSync(to))
					throw new Error(
						"workspace.move will not overwrite an existing destination.",
					);
				renameSync(from, to);
				const mutation = WorkspaceMutationSchema.parse({
					id: `mutation-${randomUUID()}`,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Restore a recorded create, update, or delete mutation exactly once.",
				category: "workspace",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
	...
	private registerExecutionTools(): void {
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Run one allowlisted executable without a shell in a macOS sandbox that denies network and all file writes.",
				category: "execution",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				...
				return {
					...(await this.commandRunner.run(
						{ ...parsed, cwd, workspaceRoot, mode: "read_only" },
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Start an approved allowlisted process under the workspace Seatbelt profile and supervise it by process ID.",
				category: "execution",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session, workspaceRoot, progress }, input) => {
				...
				const handle = this.commandRunner.start(
					{ ...parsed, cwd, workspaceRoot, mode: "workspace_write" },
					{
				...
				void handle.completion
					.then((result) => {
						...
						this.saveProcessJournal({
							...
							createdAt:
								this.processJournal(processId)?.createdAt ?? this.now(),
							updatedAt: this.now(),
						});
						progress({
							processId,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Read bounded output and lifecycle state for a session-owned background process.",
				category: "execution",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Write a bounded input chunk to an approved interactive background process.",
				category: "execution",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: ({ session }, input) => {
				...
				const process = this.requireBackgroundProcess(
					session.id,
					parsed.processId,
				);
				...
				return {
					...
					bytes: Buffer.byteLength(parsed.data),
				};
			},
		});
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Request termination of a session-owned background process.",
				category: "execution",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Run one allowlisted executable without a shell in a macOS sandbox that denies network and writes outside the granted workspace.",
				category: "execution",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Read a bounded staged or unstaged Git diff for optional pathspecs.",
				category: "execution",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				...
				const args = [
					...
					"--no-ext-diff",
					"--no-color",
					...(parsed.staged ? ["--cached"] : []),
					...(parsed.pathspec.length ? ["--", ...parsed.pathspec] : []),
				];
				const result = await this.commandRunner.run(
					{
						command: "git",
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Create an approved branch worktree under the granted repository's .kestrel/worktrees directory.",
				category: "execution",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				...
				if (existsSync(absolutePath))
					throw new Error(
						"The requested Kestrel worktree path already exists.",
					);
				const args = [
					"worktree",
					"add",
					...(parsed.createBranch ? ["-b", parsed.branch] : []),
					relativePath,
					parsed.startPoint,
				];
				const result = await this.commandRunner.run(
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Create an approved local branch from an explicit start point without switching worktrees.",
				category: "execution",
				riskLevel: "low",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				...
				const result = await this.commandRunner.run(
					{
						command: "git",
						args: ["branch", parsed.branch, parsed.startPoint],
						cwd: workspaceRoot,
						workspaceRoot,
						mode: "workspace_write",
						timeoutMs: 30_000,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Create an explicitly approved commit from the current index and return the verified commit ID.",
				category: "execution",
				riskLevel: "sensitive",
				readOnly: false,
				requiresWorkspace: true,
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				...
				const verify = await this.commandRunner.run(
					{
						command: "git",
						args: ["rev-parse", "HEAD"],
						cwd: workspaceRoot,
				...
				if (verify.exitCode !== 0)
					throw new Error(verify.stderr || "git commit verification failed.");
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Push an explicit branch to an explicit remote through the protected GitHub credential without placing the token in argv, files, output, or audit records.",
				category: "execution",
				riskLevel: "external",
				readOnly: false,
				requiresWorkspace: true,
				source: "builtin",
				tags: ["git", "push", "remote", "github", "publish"],
			},
			inputSchema: gitPushInputSchema,
			outputSchema: z.object({
				remote: z.string(),
				branch: z.string(),
				remoteSha: z.string().regex(/^[a-f0-9]{40,64}$/),
				stdout: z.string(),
			}),
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				if (!workspaceRoot || !this.githubToken)
					throw new Error(
						"GitHub publishing requires a protected GitHub token.",
					);
				const parsed = gitPushInputSchema.parse(input);
				await this.githubRepository(
					workspaceRoot,
					parsed.remote,
					signal,
					progress,
				);
				const helperDirectory = resolve(workspaceRoot, ".kestrel", "runtime");
				...
				const environment = {
					...
					KESTREL_GIT_TOKEN: this.githubToken,
				};
				const pushed = await this.commandRunner.run(
					{
						command: "git",
						args: [
							"push",
							...(parsed.setUpstream ? ["--set-upstream"] : []),
							parsed.remote,
							parsed.branch,
						],
						cwd: workspaceRoot,
						workspaceRoot,
						mode: "network_workspace_write",
						timeoutMs: 300_000,
						environment,
					},
					{ signal, onProgress: progress },
				...
				if (pushed.exitCode !== 0)
					throw new Error(pushed.stderr || "git push failed.");
				const verify = await this.commandRunner.run(
					{
						command: "git",
						args: [
				...
				if (verify.exitCode !== 0 || !/^[a-f0-9]{40,64}\s/.test(verify.stdout))
					throw new Error(
						verify.stderr || "Git push remote verification failed.",
					);
				return {
					remote: parsed.remote,
					branch: parsed.branch,
					remoteSha: verify.stdout.trim().split(/\s+/)[0]!,
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Open a GitHub pull request for an already-pushed branch after explicit external-action approval.",
				category: "connector",
				riskLevel: "external",
				readOnly: false,
				requiresWorkspace: true,
				source: "connector",
				tags: ["github", "pull-request", "pr", "publish"],
			},
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				if (!workspaceRoot) throw new Error("Workspace root is unavailable.");
				const parsed = githubPrInputSchema.parse(input);
				const repository = await this.githubRepository(
					workspaceRoot,
					parsed.remote,
					signal,
					progress,
				);
				const result = await this.githubApi<{
					number: number;
					html_url: string;
					title: string;
					state: string;
		...
		this.registerTool({
			descriptor: {
				...
				title: "Inspect GitHub CI checks",
				description:
					"Read bounded GitHub check-run conclusions and diagnostic summaries for an explicit ref.",
				category: "connector",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
				source: "connector",
				tags: ["github", "ci", "checks", "review", "diagnose"],
			},
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				if (!workspaceRoot) throw new Error("Workspace root is unavailable.");
				const parsed = githubRefInputSchema.parse(input);
				const repository = await this.githubRepository(
					workspaceRoot,
					parsed.remote,
					signal,
				...
				const response = await this.githubApi<{
					...
					`/repos/${repository.owner}/${repository.repo}/commits/${encodeURIComponent(parsed.ref)}/check-runs`,
					{},
					signal,
				);
				return {
					...
					ref: parsed.ref,
					checks: (response.check_runs ?? []).slice(0, 100).map((check) => ({
						name: String(check.name ?? "check"),
						status: String(check.status ?? "unknown"),
						conclusion:
							check.conclusion == null ? null : String(check.conclusion),
						url: String(check.html_url ?? "https://github.com"),
						title: String(check.output?.title ?? "").slice(0, 2_000),
						summary: String(check.output?.summary ?? "").slice(0, 20_000),
					})),
				};
			},
		});
		...
		this.registerTool({
			descriptor: {
				name: "engineering.review-prepare",
				title: "Prepare structured code review",
				description:
					"Build a bounded review packet from changed files, line counts, whitespace errors, staged or unstaged diff, and nearby tests for model review and CI repair.",
				category: "execution",
				riskLevel: "read_only",
				readOnly: true,
				requiresWorkspace: true,
				source: "builtin",
				tags: ["review", "diff", "ci", "tests", "repository"],
			},
			inputSchema: gitDiffInputSchema,
			outputSchema: z.object({
				staged: z.boolean(),
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				if (!workspaceRoot) throw new Error("Workspace root is unavailable.");
				const parsed = gitDiffInputSchema.parse(input);
				const prefix = parsed.staged ? ["--cached"] : [];
				const suffix = parsed.pathspec.length ? ["--", ...parsed.pathspec] : [];
				...
				const [names, stats, check, diff] = await Promise.all([
					run(["diff", ...prefix, "--name-only", ...suffix]),
					run(["diff", ...prefix, "--numstat", ...suffix]),
					run(["diff", ...prefix, "--check", ...suffix]),
					run(["diff", ...prefix, "--no-ext-diff", "--no-color", ...suffix]),
				]);
				if (names.exitCode !== 0 || stats.exitCode !== 0 || diff.exitCode !== 0)
					throw new Error(
						names.stderr ||
							stats.stderr ||
							diff.stderr ||
							"Git review preparation failed.",
					);
				...
				return {
					...
					files,
					testFiles,
					numstat: stats.stdout,
					whitespaceErrors: check.stdout || check.stderr,
					diff: diff.stdout.slice(0, 900_000),
					truncated: diff.stdout.length > 900_000,
				};
			},
		});
		...
		this.registerTool({
			descriptor: {
				...
				description:
					"Submit an explicit comment, approval, or request-changes review to GitHub.",
				category: "connector",
				riskLevel: "external",
				readOnly: false,
				requiresWorkspace: true,
				source: "connector",
				tags: ["github", "review", "pull-request", "external"],
			},
			...
			execute: async ({ workspaceRoot, signal, progress }, input) => {
				if (!workspaceRoot) throw new Error("Workspace root is unavailable.");
				const parsed = githubReviewInputSchema.parse(input);
				const repository = await this.githubRepository(
					workspaceRoot,
					parsed.remote,
					signal,
				...
				const result = await this.githubApi<{
					...
					`/repos/${repository.owner}/${repository.repo}/pulls/${parsed.pullNumber}/reviews`,
					{
						method: "POST",
						body: JSON.stringify({ body: parsed.body, event: parsed.event }),
					},
					signal,
				);
	...
	private async githubRepository(
		...
		if (result.exitCode !== 0)
			throw new Error(result.stderr || `Git remote ${remote} is unavailable.`);
		...
		if (!match)
			throw new Error(
				"GitHub workflows require an explicit github.com HTTPS or SSH remote.",
			);
	...
	private async githubApi<T>(
		...
		if (!this.githubToken)
			throw new Error("GitHub workflows require a protected GitHub token.");
		...
		if (!response.ok) {
			...
			try {
				message = String(
					(JSON.parse(text) as { message?: unknown }).message ?? message,
				);
			} catch {}
			throw new Error(
				`GitHub request failed (${response.status}: ${message.slice(0, 2_000)}).`,
			);
		}
		try {
	...
	private async verifyMutation(
		...
		if (root && tool === "workspace.write") {
			...
			if (content !== String(input.content ?? ""))
				throw new Error("Workspace write read-back verification failed.");
			return {
		...
		if (root && (tool === "workspace.delete" || tool === "workspace.rmdir")) {
			...
			if (!isWithin(root, path) || existsSync(path))
				throw new Error("Workspace deletion read-back verification failed.");
			return {
				method: "filesystem-absence-readback",
		...
		if (root && tool === "workspace.mkdir") {
			const path = resolveExistingPath(root, String(output.path));
			if (!statSync(path).isDirectory())
				throw new Error("Workspace directory read-back verification failed.");
			return {
				method: "filesystem-directory-readback",
				evidence: { path: output.path },
			};
		}
		if (root && tool === "workspace.patch") {
			...
			const sha256 = createHash("sha256")
				.update(this.readMutationText(path))
				.digest("hex");
			if (sha256 !== output.sha256)
				throw new Error("Workspace patch read-back verification failed.");
			return {
				method: "filesystem-digest-readback",
				evidence: { path: output.path, sha256 },
			};
		}
		if (root && tool === "workspace.move") {
			...
			if (
				...
				throw new Error("Workspace move read-back verification failed.");
			return {
				method: "filesystem-move-readback",
				evidence: { from: output.from, to: output.to },
			};
		}
		if (tool === "workspace.undo") {
			const mutation = this.database.getWorkspaceMutation(
				String(output.mutationId),
			);
			if (!mutation?.undoneAt || output.restored !== true)
				throw new Error("Workspace undo read-back verification failed.");
			return {
				method: "mutation-journal-readback",
				evidence: { mutationId: mutation.id, undoneAt: mutation.undoneAt },
			};
		}
		if (tool === "tools.activate") {
			const descriptor = output.descriptor as
				| Record<string, unknown>
				| undefined;
			const name = String(descriptor?.name ?? "");
			if (!name || !this.tools.has(name))
				throw new Error("Deferred tool activation verification failed.");
		...
		if (tool === "execution.start-background") {
			...
			if (!process || process.sessionId !== context.session.id)
				throw new Error("Background process read-back verification failed.");
			return {
				method: "process-supervisor-readback",
				evidence: { processId: output.processId, pid: output.pid },
			};
		}
		if (
			tool === "execution.process-write" ||
			tool === "execution.process-stop"
		) {
			const process = this.requireBackgroundProcess(
				context.session.id,
				String(output.processId),
			);
			return {
				method: "process-supervisor-readback",
				evidence: {
		...
		if (root && tool === "git.worktree-create") {
			...
			if (
				...
				throw new Error("Git worktree read-back verification failed.");
			return {
				method: "git-worktree-readback",
				evidence: { branch: output.branch, path: output.path },
			};
		}
		if (root && tool === "git.stage") {
			const parsed = gitStageInputSchema.parse(input);
			const snapshot = await this.commandRunner.run(
				{
					command: "git",
					args: ["diff", "--cached", "--name-only", "--", ...parsed.pathspec],
					cwd: root,
					workspaceRoot: root,
					mode: "read_only",
					timeoutMs: 30_000,
			...
			if (snapshot.exitCode !== 0)
				throw new Error(
					snapshot.stderr || "Git index read-back verification failed.",
				);
			return {
				method: "git-index-readback",
				evidence: {
					pathspec: parsed.pathspec,
					stagedPaths: snapshot.stdout.trim().split("\n").filter(Boolean),
				},
			};
		}
		if (root && tool === "git.branch-create") {
			...
			const snapshot = await this.commandRunner.run(
				{
					command: "git",
					args: ["rev-parse", `refs/heads/${branch}`],
					cwd: root,
					workspaceRoot: root,
					mode: "read_only",
					timeoutMs: 30_000,
			...
			if (snapshot.exitCode !== 0 || !snapshot.stdout.trim())
				throw new Error(
					snapshot.stderr || "Git branch read-back verification failed.",
				);
			return {
				method: "git-reference-readback",
				evidence: { branch, commitId: snapshot.stdout.trim() },
			};
		}
		if (root && tool === "git.commit") {
			...
			if (snapshot.exitCode !== 0 || snapshot.stdout.trim() !== output.commitId)
				throw new Error(
					snapshot.stderr || "Git commit read-back verification failed.",
				);
			return {
				method: "git-commit-readback",
				evidence: { commitId: output.commitId },
			};
		}
		return {
			method: `${definition.descriptor.source}-typed-receipt`,
			evidence: output,
		};
	}
	...
	private loadInstructions(
		...
		const resolvedTarget = targetPath
			? resolveExistingPath(workspaceRoot, targetPath)
			: workspaceRoot;
		const targetDirectory = statSync(resolvedTarget).isDirectory()
			? resolvedTarget
		...
		for (const directory of directories) {
			for (const name of names) {
				...
				if (!text) continue;
				instructions.push({
					path: relative(workspaceRoot, canonical),
					content: text.content,
					precedence: instructions.length,
				});
			}
		}
		return instructions;
	...
	private readMutationText(path: string): string {
		if (statSync(path).size > 1_000_000)
			throw new Error("Workspace mutation files are limited to 1 MB.");
		const buffer = readFileSync(path);
		if (buffer.includes(0))
			throw new Error("Binary workspace mutations are not supported.");
	...
	private approvalPreview(
		...
		if (!session.workspaceRoot || !toolName.startsWith("workspace."))
			return JSON.stringify(input, null, 2).slice(0, 20_000);
		try {
			if (toolName === "workspace.move")
				return `Move ${String(input.from)} → ${String(input.to)}`;
			if (toolName === "workspace.rmdir")
				return `Delete empty directory ${String(input.path)}`;
			...
			if (toolName === "workspace.write") after = String(input.content ?? "");
				...
				for (const edit of input.edits as Array<Record<string, unknown>>) {
					...
					after = edit.replaceAll
						? after.split(oldText).join(newText)
						: after.replace(oldText, newText);
				}
	...
	private requireBackgroundProcess(sessionId: string, processId: string) {
		...
		if (!process || process.sessionId !== sessionId)
			throw new Error("Background process not found in this session.");
		return process;
	}
	...
	private ensureBackgroundProcessCapacity(): void {
		...
		throw new Error(
			`At most ${MAX_BACKGROUND_PROCESSES} background processes can run at once.`,
		);
	}
	...
	private backgroundProcessSnapshot(sessionId: string, processId: string) {
		...
		if (!process) {
			...
			if (!journal || journal.sessionId !== sessionId)
				throw new Error("Background process not found in this session.");
			return {
				processId,
				pid: Number(journal.pid ?? 0),
				status: String(journal.status),
				...(journal.exitCode !== undefined
					? { exitCode: Number(journal.exitCode) }
					: {}),
				...(journal.signal !== undefined
					? { signal: journal.signal as string | null }
					: {}),
				stdout: String(journal.stdout ?? ""),
				stderr: String(journal.stderr ?? ""),
				durationMs: Number(journal.durationMs ?? 0),
				...(journal.error ? { error: String(journal.error) } : {}),
			};
		}
	...
	private processJournal(
		...
		return this.processJournalRecords().find(
	...
	private processJournalRecords(): Array<Record<string, unknown>> {
		...
		return Array.isArray(stored) ? stored.filter(isRecord).slice(-200) : [];
	}
	...
	private saveProcessJournal(record: Record<string, unknown>): void {
		...
		this.database.setPrivateState(
			this.processJournalKey,
			[
				...records.filter((process) => process.processId !== record.processId),
				record,
			].slice(-200),
		);
	}
	...
	private resolveGrantedRoot(requestedRoot: string): string {
		...
		if (!granted)
			throw new Error("Workspace access has not been granted for this root.");
		return candidate;
	}

	private resolveActiveWorkspaceRoot(
		session: RuntimeSession,
	): string | undefined {
	...
	private reconcilePersistedWorkspaceRoots(): void {
		for (const session of this.database.listRuntimeSessions()) {
			...
			if (
				this.configuredWorkspaceRoots.some((root) => isWithin(root, candidate))
			)
				continue;
	...
	private async runHooks(
		...
		for (const hook of this.hooks) {
			if (
				hook.event !== event ||
				(hook.toolPattern && !hook.toolPattern.test(tool.name))
			)
				continue;
