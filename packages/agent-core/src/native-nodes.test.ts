...
describe("native node manager", () => {
  it("keeps presence privacy-preserving and delivers bounded commands", () => {
    ...
    const exposed = manager.result(location.id)!;
    exposed.error!.message = "tampered";
    expect(manager.result(location.id)?.error?.message).toBe("Location permission is off.");
  ...
  it("normalizes non-finite location bounds", () => {
    const manager = new NativeNodeManager(() => new Date("2026-07-23T12:00:00.000Z"));
