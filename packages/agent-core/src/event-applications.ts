...
export interface EventEligibilityItem {
	id: string;
	label: string;
	met: boolean | null;
...
function validHttps(value: string): string {
	...
	try {
		...
		throw new Error("Event URL must be credential-free HTTPS.");
	}
...
function parseStored(value: unknown): EventApplication | undefined {
	...
	try {
		...
		if (parsed.data.deadline !== undefined)
			application.deadline = parsed.data.deadline;
		if (parsed.data.approvedAt !== undefined)
			application.approvedAt = parsed.data.approvedAt;
		if (parsed.data.submittedAt !== undefined)
			application.submittedAt = parsed.data.submittedAt;
		if (parsed.data.receipt !== undefined)
			application.receipt = parsed.data.receipt;
		return application;
	} catch {
		return undefined;
	}
...
export class EventApplicationManager {
	...
	create(input: {
		...
		if (deadline && !Number.isFinite(new Date(deadline).getTime()))
			throw new Error("Event deadline is invalid.");
		const application: EventApplication = {
	...
	update(
		...
		if (
			patch.answers &&
			(patch.answers.length > 100 ||
				patch.answers.some(
					(answer) =>
						!answer.id ||
						answer.label.length > 300 ||
						answer.value.length > 20_000 ||
						!["public", "personal", "sensitive"].includes(answer.sensitivity),
				))
		)
			throw new Error("Event application answers are invalid.");
		if (
			patch.eligibility &&
			(patch.eligibility.length > 100 ||
				patch.eligibility.some(
					(item) =>
						!item.id ||
						item.label.length > 500 ||
						(item.evidence?.length ?? 0) > 2_000,
				))
		)
			throw new Error("Event eligibility is invalid.");
		...
		if (patch.status === "approved") {
			const unresolved = (patch.eligibility ?? current.eligibility).some(
				(item) => item.met !== true,
			);
			const unreviewed = (patch.answers ?? current.answers).some(
				(answer) =>
					answer.required && (!answer.value.trim() || !answer.reviewed),
			);
			if (unresolved || unreviewed)
				throw new Error(
					"Review required answers and eligibility before approval.",
				);
	...
	markSubmitted(id: string, receipt: string): EventApplication {
		...
		if (!current || current.status !== "approved")
			throw new Error(
				"Only an approved event application can be marked submitted.",
			);
		const timestamp = this.now().toISOString();
		return this.finishSubmitted(
			id,
			clean(receipt, 4_000, "Submission receipt"),
			timestamp,
		);
	}
	private finishSubmitted(
		id: string,
		receipt: string,
		timestamp: string,
	): EventApplication {
	...
	remove(id: string): void {
		...
		if (next.length === this.list().length)
			throw new Error("Event application was not found.");
		this.save(next);
	}
	private save(value: EventApplication[]): void {
		if (
			value.length > 200 ||
			Buffer.byteLength(JSON.stringify(value)) > 5_000_000
		)
			throw new Error("Event application storage exceeds limits.");
		this.database.setPrivateState(KEY, value);
	}
}

export const EVENT_APPLICATION_TOOL_NAMES = [
	"events.list",
	"events.prepare",
	"events.mark_submitted",
] as const;
export function installEventApplicationTools(
	runtime: AgentRuntime,
	manager: EventApplicationManager,
	sessionId: string,
): void {
	runtime.registerExternalTool({
		descriptor: {
			name: "events.list",
			title: "List event applications",
			description:
				"Read the user's local event and hackathon application workspace.",
			category: "automation",
			riskLevel: "sensitive",
			readOnly: true,
			requiresWorkspace: false,
			source: "builtin",
			tags: ["events", "applications", "private"],
		},
		inputSchema: {
			type: "object",
			properties: {},
	...
	runtime.registerExternalTool({
		descriptor: {
			name: "events.mark_submitted",
			title: "Record a verified event submission",
			description:
				"Mark an explicitly approved application submitted only after the external site returns a visible confirmation or receipt.",
			category: "automation",
		...
		inputSchema: {
			...
			properties: {
				id: { type: "string" },
				receipt: { type: "string", minLength: 1, maxLength: 4_000 },
			},
			required: ["id", "receipt"],
			additionalProperties: false,
		},
		execute: async (_context, input) => ({
			application: manager.markSubmitted(
				String(input.id),
	...
	runtime.registerExternalTool({
		descriptor: {
			name: "events.prepare",
			title: "Prepare an event application for review",
			description:
				"Write eligibility checks and draft answers into a local application. This never submits an external form.",
			category: "automation",
			riskLevel: "sensitive",
			readOnly: false,
			requiresWorkspace: false,
			source: "builtin",
			tags: ["events", "applications", "review-required"],
		},
		inputSchema: {
			type: "object",
			properties: {
	...
	for (const name of EVENT_APPLICATION_TOOL_NAMES)
