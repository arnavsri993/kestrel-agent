...
export class PreResponseContextResolver {
	...
	categoriesFor(message: string): ContextCategory[] {
		...
		if (
			/connect|controller|device|phone|drone|dji|error|not working/.test(lower)
		) {
			return [
				"devices",
				"hardware",
		...
		if (/when|schedule|date|calendar|meeting|test/.test(lower))
			return ["schedule", "deadlines", "preferences"];
	...
	resolve(request: ContextResolutionRequest): ResolvedContext {
		...
		const matches = this.memoryProvider()
			...
			.sort(
				(a, b) =>
					(b.userConfirmed ? 1 : 0) - (a.userConfirmed ? 1 : 0) ||
