...
function checkedBaseUrl(value: string): string {
	...
	try {
		...
		throw new Error("Honcho base URL must be a valid URL.");
	}
...
export class HonchoMemoryProvider {
	...
	configure(input: HonchoMemoryConfiguration): HonchoMemoryStatus {
	...
	async contextFor(input: {
		...
		const configuration = this.configuration();
		if (!configuration.enabled || configuration.recallMode === "tools")
			return "";
		const states =
		...
		try {
			...
			if (next.turn === 1 || next.turn % configuration.contextCadence === 0) {
			...
			if (next.turn === 1 || next.turn % configuration.dialecticCadence === 0) {
				...
				for (let pass = 0; pass < configuration.dialecticDepth; pass += 1) {
					...
					if (result && result.length >= configuration.dialecticMaxChars) break;
				}
	...
	captureMessage(message: RuntimeMessage, workspaceRoot?: string): void {
		const configuration = this.configuration();
		if (
			!configuration.enabled ||
		...
		this.syncQueue = this.syncQueue
			.then(async () => {
				...
				const chunks = message.content.match(/[\s\S]{1,25000}/g) ?? [];
	...
	async profile(card?: string[]): Promise<string[] | null> {
		...
		if (card) return remote.agent.setCard(card.slice(0, 100), remote.user);
	...
	async fullContext(sessionId?: string) {
		const remote = await this.remoteSession(sessionId ?? "workstrand-tools");
		const context = await remote.session.context({
			summary: true,
	...
	async reason(
		...
		return remote.agent.chat(query.slice(0, 10_000), {
			target: remote.user,
			session: remote.session,
			reasoningLevel: level ?? this.configuration().dialecticReasoningLevel,
		});
	}
	...
	return [
		{
			...
			execute: async (_context: unknown, input: Record<string, unknown>) => ({
		...
		},
		{
			...
			inputSchema: {
				...
				oneOf: [{ required: ["conclusion"] }, { required: ["deleteId"] }],
				additionalProperties: false,
			},
			execute: async (_context: unknown, input: Record<string, unknown>) => ({
				...(await provider.conclude({
					...(input.conclusion ? { conclusion: String(input.conclusion) } : {}),
...
export function installHonchoMemoryTools(
	...
	for (const definition of definitions) {
		...
		for (const sessionId of sessionIds)
			runtime.allowTool(sessionId, definition.name);
	}
}
