...
export interface TenantFleetRunner {
	run(
		executable: string,
		args: string[],
...
export class TenantFleet {
	...
	constructor(
		stateRoot: string,
		private readonly runner: TenantFleetRunner = new ProcessFleetRunner(),
		...
		mkdirSync(join(this.root, "auth-profile-secrets"), {
			recursive: true,
			mode: 0o700,
		});
	}
	...
	async create(input: {
		...
		image?: string;
		blockEgress?: boolean;
	}): Promise<{ cell: TenantCell; gatewayToken: string }> {
		const tenant = input.tenant.trim().toLowerCase();
		if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(tenant))
			throw new Error("Tenant cell name is invalid.");
		if (
			!Number.isInteger(input.port) ||
			input.port < 1024 ||
			input.port > 65_535
		)
			throw new Error("Tenant cell port is invalid.");
		const runtime = input.runtime ?? "docker";
		if (runtime === "docker" && input.blockEgress)
			throw new Error(
				"Docker internal networking breaks the published gateway port; use host firewall policy.",
			);
		const image = input.image ?? "ghcr.io/kestrel-ai/kestrel:latest";
		if (!/^[A-Za-z0-9][A-Za-z0-9._/:@-]{0,500}$/.test(image))
			throw new Error("Tenant cell image is invalid.");
		return this.mutate(async () => {
			const cells = await this.list();
			if (
				cells.some((cell) => cell.tenant === tenant || cell.port === input.port)
			)
				throw new Error("Tenant cell or loopback port already exists.");
			...
			const networkResult = await this.runner.run(runtime, [
				"network",
				"create",
				...(runtime === "podman" && input.blockEgress ? ["--internal"] : []),
				network,
			]);
			if (networkResult.exitCode !== 0)
				throw new Error(
					`Could not create tenant network: ${networkResult.stderr.slice(0, 500)}`,
				);
			const args = [
				"run",
				"-d",
				...
				"-v",
				`${state}:/home/node/.kestrel`,
				"-v",
				`${auth}:/home/node/.config/kestrel`,
				"-e",
				`KESTREL_GATEWAY_TOKEN=${token}`,
				image,
			];
			...
			if (result.exitCode !== 0) {
				...
				throw new Error(
					`Could not start tenant cell: ${result.stderr.slice(0, 500)}`,
				);
			}
			const cell: TenantCell = {
				tenant,
				container,
				port: input.port,
				image,
	...
	async status(
		tenant: string,
	): Promise<{ cell: TenantCell; running: boolean }> {
		const cell = (await this.list()).find(
			(candidate) => candidate.tenant === tenant,
		);
		if (!cell) throw new Error("Tenant cell was not found.");
		const result = await this.runner.run(cell.runtime, [
			"inspect",
			"-f",
			"{{.State.Running}}",
			cell.container,
		...
		return {
			cell,
			running: result.exitCode === 0 && result.stdout.trim() === "true",
		};
	}
	...
	async remove(tenant: string): Promise<TenantCell> {
		return this.mutate(async () => {
			...
			if (result.exitCode !== 0)
				throw new Error(
					`Could not remove tenant cell: ${result.stderr.slice(0, 500)}`,
				);
			await this.runner.run(cell.runtime, [
				"network",
				"rm",
				`kestrel-cell-${cell.tenant}`,
			]);
	...
	private async mutate<T>(operation: () => Promise<T>): Promise<T> {
		const previous =
			fleetMutationQueues.get(this.registryPath) ?? Promise.resolve();
		let release!: () => void;
		...
		try {
			...
			release();
			if (fleetMutationQueues.get(this.registryPath) === queued)
				fleetMutationQueues.delete(this.registryPath);
		}
	}
