...
function sourceDays(memory: MemoryRecord): string[] {
	const found = memory.sourceIds.flatMap(
		(sourceId) => sourceId.match(/\d{4}-\d{2}-\d{2}/g) ?? [],
	);
	return [
		...new Set([
			...found,
...
export class DreamingManager {
	...
	configuration(): DreamingConfiguration {
		return DreamingConfigurationSchema.parse(
			this.database.getPrivateState(CONFIGURATION_KEY) ??
				DEFAULT_DREAMING_CONFIGURATION,
		);
	}
	...
	status(): DreamingStatus {
		...
		return DreamingStatusSchema.parse({
			...
			...(configuration.enabled
				? { nextRunAt: this.nextRunAt(state.lastRunAt).toISOString() }
				: {}),
			detail: state.detail,
		});
	}
	...
	run(preview = false): DreamingStatus {
		const configuration = this.configuration();
		if (!preview && !configuration.enabled)
			throw new Error("Enable memory consolidation before running it.");
		...
		const current = this.storedState();
		const reviewedByMemory = new Map(
			current.candidates
				.filter((candidate) => candidate.status !== "review")
				.map((candidate) => [candidate.memoryId, candidate]),
		);
		const candidates = evaluation.candidates.map(
			(candidate) => reviewedByMemory.get(candidate.memoryId) ?? candidate,
		);
		this.saveState({
			phase: "idle",
	...
	runIfDue(at = this.now()): DreamingStatus | undefined {
		...
		const state = this.storedState();
		if (at.getTime() < this.nextRunAt(state.lastRunAt, at).getTime())
			return undefined;
	...
	review(id: string, decision: "promote" | "reject"): DreamingStatus {
		...
		if (candidate.status !== "review")
			throw new Error("Dreaming candidate was already reviewed.");
		const timestamp = this.now().toISOString();
		if (decision === "promote") {
			const memory = this.database
				.listMemories()
				.find(
					(item) => item.id === candidate.memoryId && item.status === "active",
				);
		...
		this.saveState({
			...
			detail:
				decision === "promote"
					? "Candidate promoted after explicit review."
					: "Candidate rejected; durable memory was not changed.",
		});
		return this.status();
	}
	...
	private evaluate(
		...
		const termSets = new Map(
			active.map((memory) => [
				memory.id,
				new Set(normalizedTerms(memory.content)),
			]),
		);
		const scored = active.map((memory) => {
			...
			const sourceIds = [
				...new Set(
					[memory.sourceIds, ...related.map((item) => item.sourceIds)].flat(),
				),
			];
			const days = [
				...new Set([sourceDays(memory), ...related.map(sourceDays)].flat()),
			];
			const kinds = new Set(sourceIds.map(sourceKind));
			const ageDays = Math.max(
				0,
				(startedAt.getTime() - new Date(memory.updatedAt).getTime()) /
					86_400_000,
			);
		...
		const candidates = scored
			.filter(
				({ sourceIds, days, score }) =>
					sourceIds.length >= configuration.minimumRecallCount &&
					days.length >= configuration.minimumUniqueDays &&
					score >= configuration.minimumScore,
			)
			.sort((left, right) => right.score - left.score)
			.slice(0, 500)
			.map<DreamingCandidate>(
				({ memory, sourceIds, days, score, related }) => ({
					id: `dream-${randomUUID()}`,
					memoryId: memory.id,
					...
					reasons: [
						...
						related > 0
							? `Reinforced by ${related} related active ${related === 1 ? "memory" : "memories"}.`
							: "Scored from direct provenance, confidence, importance, and recency.",
	...
	private nextRunAt(lastRunAt?: string, reference = this.now()): Date {
		...
		if (lastRunAt) {
			...
			while (afterLast.getTime() <= last.getTime())
				afterLast.setDate(afterLast.getDate() + 1);
			return afterLast;
		}
	...
	private storedState(): StoredDreamingState {
		...
		if (stored && typeof stored === "object" && !Array.isArray(stored)) {
			...
			if (
				...
				candidates.every(
					(candidate) => DreamingCandidateSchema.safeParse(candidate).success,
				) &&
				Array.isArray(diary) &&
				diary.length <= 100 &&
				diary.every(
					(entry) => DreamDiaryEntrySchema.safeParse(entry).success,
				) &&
