...
describe("parity capability catalog", () => {
	it("uses stable unique IDs and evidence for every non-planned claim", () => {
		...
		expect(CAPABILITY_CATALOG).toHaveLength(59);
	...
	it("keeps all four official source families and renders every catalog ID in the parity document", () => {
		expect(Object.keys(PARITY_SOURCE_SNAPSHOT).sort()).toEqual([
			"checkedAt",
			"claude-code",
			"codex",
			"hermes",
		...
		expect(PARITY_SOURCE_SNAPSHOT.pageAudit).toBe(
			"docs/reference-page-audit.json",
		);
		const document = readFileSync(
			resolve(process.cwd(), "docs/parity-matrix.md"),
			"utf8",
		);
		for (const item of CAPABILITY_CATALOG)
			expect(document, item.id).toContain(`\`${item.id}\``);
	});
	...
	it("keeps every evidence path real and the documented summary synchronized", () => {
		...
		const document = readFileSync(
			resolve(root, "docs/parity-matrix.md"),
			"utf8",
		);
		expect(document).toContain(
			`${CAPABILITY_CATALOG.length} capability families — ${summary.implemented} implemented, ${summary.partial} partial, ${summary.planned} planned`,
		);
	});

	it("reports an honest status summary", () => {
		const summary = capabilitySummary();
		expect(summary.implemented + summary.partial + summary.planned).toBe(
			CAPABILITY_CATALOG.length,
		);
		expect(summary).toEqual({ implemented: 59, partial: 0, planned: 0 });
		for (const item of CAPABILITY_CATALOG) {
