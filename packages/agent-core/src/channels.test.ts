...
describe("authenticated channel gateway", () => {
	it("recovers to default interaction settings when persisted state is malformed", () => {
		...
		database.setPrivateState("channels.interaction", {
	...
	it("verifies and deduplicates untrusted inbound messages and approval-gates outbound sends", async () => {
			...
			[
				{
					...
					send: async ({ text }) => {
						...
						return {
							externalId: "out-1",
							deliveredAt: "2026-07-22T23:30:00.000Z",
						};
					},
				},
			],
		...
		const envelope: ChannelEnvelope = {
			...
			receivedAt: "2026-07-22T23:29:00.000Z",
		};
		expect(() => gateway.receive(session.id, envelope, "00")).toThrow(
			"Invalid channel signature",
		);
		const signature = signChannelEnvelope(envelope, secret);
		expect(gateway.receive(session.id, envelope, signature)).toEqual({
			accepted: true,
			duplicate: false,
		});
		expect(gateway.receive(session.id, envelope, signature)).toEqual({
		...
		expect(
			(
				await runtime.callTool(session.id, "channel.send", input, {
	...
	it("delivers through a bounded HTTPS webhook adapter and rejects private DNS targets", async () => {
		...
		expect(
			() => new WebhookChannelAdapter({ id: "invalid-url", url: "not a URL" }),
		).toThrow("HTTPS");
		...
		const unsafe = new WebhookChannelAdapter({
			id: "unsafe",
	...
	it("rejects an already-aborted webhook before DNS or network access", async () => {
		...
		).rejects.toThrow("Webhook cancelled before start.");
	...
	it("normalizes a non-finite webhook timeout", () => {
		...
		expect((adapter as unknown as { timeoutMs: number }).timeoutMs).toBe(
			15_000,
		);
	});
	...
	it("loads channel credentials only from an owner-only bounded configuration file", () => {
		...
		try {
			...
			expect(
				environmentChannelConfiguration({ KESTREL_CHANNEL_CONFIG: path }),
			).toMatchObject({
				sessionRoutes: { chat: "session-1" },
				adapters: [{ id: "chat" }],
			});
	...
	it("delivers through native Slack, Discord, Teams, and Gmail provider APIs", async () => {
		...
		const fetcher: typeof fetch = async (input, init) => {
			...
			const payload = url.includes("slack.com")
				? { ok: true, ts: "slack-1" }
				: {
						id: `${url.includes("discord") ? "discord" : url.includes("googleapis") ? "gmail" : "teams"}-1`,
					};
		...
		for (const kind of ["slack", "discord", "teams", "gmail"] as const) {
			...
			expect(
				await adapter.send({
					conversationId: kind === "gmail" ? "person@example.test" : "room",
					text: "Hello",
					idempotencyKey: `${kind}-send`,
					signal: new AbortController().signal,
				}),
			).toMatchObject({
				externalId: expect.stringContaining(kind),
				deliveredAt: "2026-07-23T02:00:00.000Z",
			});
		}
		const discord = new NativeChannelAdapter({
			id: "discord-edit",
			kind: "discord",
			token: "discord-token",
			fetcher,
		...
		expect(requests.map((request) => request.url)).toEqual(
			expect.arrayContaining([
				...
				expect.stringContaining("discord.com/api/v10/channels"),
				expect.stringContaining("graph.microsoft.com/v1.0/chats"),
				expect.stringContaining("gmail.googleapis.com/gmail/v1"),
			]),
		);
		expect(requests).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
					url: expect.stringContaining("/messages/discord-1"),
					method: "PATCH",
				}),
				expect.objectContaining({
					url: expect.stringContaining("/typing"),
					method: "POST",
				}),
			]),
		);
		expect(
			requests.every((request) => request.authorization.includes("token")),
		).toBe(true);
	});
	...
	it("rejects cancelled native actions before loading credentials or making requests", async () => {
		...
		await expect(
			adapter.send({
				conversationId: "room",
				text: "Hello",
				idempotencyKey: "cancelled-send",
				signal: controller.signal,
			}),
		).rejects.toThrow("native action cancelled before start");
		await expect(
			adapter.edit!({
				conversationId: "room",
				externalId: "message",
		...
		await expect(
			adapter.typing!({ conversationId: "room", signal: controller.signal }),
		).rejects.toThrow("native action cancelled before start");
		await expect(
			adapter.react!({
				conversationId: "room",
	...
	it("bounds every native provider JSON path before parsing or taking follow-up actions", async () => {
		...
		const deliveryAdapter = new NativeChannelAdapter({
			id: "bounded-delivery",
			kind: "slack",
			token: "slack-token",
			fetcher: async () => delivery.response,
		});
		await expect(
		...
		const reactionAdapter = new NativeChannelAdapter({
			id: "bounded-reaction",
			kind: "slack",
			token: "slack-token",
			fetcher: async () => reaction.response,
		});
		await expect(
			reactionAdapter.react({
				conversationId: "room",
				externalId: "message",
				emoji: "thumbsup",
				remove: false,
	...
	it("adds and removes reactions through Slack, Discord, and Teams provider APIs", async () => {
		...
		const gmail = new NativeChannelAdapter({
			id: "gmail",
			kind: "gmail",
			token: "gmail-token",
			fetcher,
		...
		await expect(
			gmail.react({
				conversationId: "person@example.test",
				externalId: "message-1",
				emoji: "👍",
				remove: false,
				signal: new AbortController().signal,
	...
	it("approval-gates reactions, applies the configured level, and tracks clear safely", async () => {
			...
			[
				{
					...
					react: async ({ emoji, remove }) => {
						calls.push({ emoji, remove });
					},
				},
			],
			{},
		);
		gateway.configureInteraction({
			progressMode: "progress",
			typingMode: "thinking",
			typingIntervalSeconds: 6,
			reactionLevel: "ack",
		...
		expect(
			(
				await runtime.callTool(session.id, "channel.react", reaction, {
		...
		).toMatchObject({ removed: true, trackedReactionCount: 0 });
	...
	it("recovers malformed persisted reaction tracking before applying a reaction", async () => {
		...
		).toMatchObject({ trackedReactionCount: 1 });
	...
	it("uses one editable content-free progress draft and supported typing signals", async () => {
			...
			[
				{
					...
					send: async ({ text }) => {
						...
						return {
							externalId: "draft-1",
							deliveredAt: "2026-07-23T10:00:00.000Z",
						};
					},
					edit: async ({ externalId, text }) => {
						expect(externalId).toBe("draft-1");
						edited.push(text);
						return { externalId, deliveredAt: "2026-07-23T10:00:01.000Z" };
					},
					typing: async () => {
						typing += 1;
					},
		...
		expect(gateway.list()).toMatchObject([
			{ editableProgress: true, typingSignals: true },
		]);
		const progress = await gateway.beginProgress(
			"editable",
			"room-1",
			"run-1",
			new AbortController().signal,
		...
		expect(edited).toEqual([
			"Verifying the result · 3/3…",
			"Final user-visible answer.",
		]);
		expect(JSON.stringify({ sent, edited })).not.toMatch(
			/argument|output|credential|path/i,
		);
		database.close();
	});
});
