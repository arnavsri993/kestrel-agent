...
function positiveSafeInteger(
	...
	if (!Number.isSafeInteger(resolved) || resolved <= 0)
		throw new Error("Plugin bundle limits must be positive safe integers.");
	return resolved;
}
...
function normalizedRelativePath(root: string, path: string): string {
	...
	if (!value || value === ".." || value.startsWith("../"))
		throw new Error("Plugin bundle path escapes its root.");
...
export class PluginInstaller {
	...
	constructor(options: PluginInstallerOptions) {
		...
		this.maximumFileBytes = positiveSafeInteger(
			options.maximumFileBytes,
			20 * 1024 * 1024,
		);
		for (const trustKey of options.trustKeys) {
			if (
				!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(trustKey.keyId) ||
				this.trustKeys.has(trustKey.keyId)
			)
				throw new Error("Plugin trust key ID is invalid or duplicated.");
			const publicKey =
				trustKey.publicKey instanceof KeyObject
					? trustKey.publicKey
					: createPublicKey(trustKey.publicKey);
			if (publicKey.asymmetricKeyType !== "ed25519")
				throw new Error(`Plugin trust key ${trustKey.keyId} is not Ed25519.`);
		...
		while (pending.length) {
			...
			for (const entry of readdirSync(directory, { withFileTypes: true })) {
				...
				if (metadata.isSymbolicLink())
					throw new Error(
						`Plugin bundles cannot contain symbolic links: ${normalizedRelativePath(sourceRoot, path)}`,
					);
				...
				if (!metadata.isFile())
					throw new Error(
						`Plugin bundles cannot contain special files: ${normalizedRelativePath(sourceRoot, path)}`,
					);
				if (metadata.size > this.maximumFileBytes)
					throw new Error(
						`Plugin file exceeds the per-file size limit: ${normalizedRelativePath(sourceRoot, path)}`,
					);
				totalBytes += metadata.size;
				if (totalBytes > this.maximumTotalBytes)
					throw new Error("Plugin bundle exceeds the total size limit.");
				const relativePath = normalizedRelativePath(sourceRoot, path);
				if (normalizedPaths.has(relativePath))
					throw new Error(
						`Plugin bundle contains colliding normalized paths: ${relativePath}`,
					);
				...
				if (files.length > this.maximumFiles)
					throw new Error("Plugin bundle exceeds the file count limit.");
			}
		}
	...
	digestForSigning(source: string): PluginBundleDigest {
		...
		if (!files.some((file) => file.relativePath === manifestRelativePath))
			throw new Error("Plugin bundle is missing .codex-plugin/plugin.json.");
		return {
	...
	inspect(source: string): VerifiedPluginBundle {
		...
		const signatureFile = files.find(
			(file) => file.relativePath === signatureRelativePath,
		);
		if (!manifestFile)
			throw new Error("Plugin bundle is missing .codex-plugin/plugin.json.");
		if (!signatureFile || signatureFile.bytes > 64_000)
			throw new Error("Plugin bundle is missing a bounded signature file.");
		const manifest = JSON.parse(
			readFileSync(manifestFile.absolutePath, "utf8"),
		...
		if (!description || description.length > 2_000)
			throw new Error("Plugin description is invalid.");
		const signature = JSON.parse(
			readFileSync(signatureFile.absolutePath, "utf8"),
		) as Record<string, unknown>;
		if (
			...
			throw new Error("Plugin signature metadata is invalid.");
		const publicKey = this.trustKeys.get(signature.keyId);
		if (!publicKey)
			throw new Error(
				`Plugin signature key ${signature.keyId} is not trusted.`,
			);
		const digest = this.digest(files);
		if (!/^[a-f0-9]{64}$/.test(signature.digest) || signature.digest !== digest)
			throw new Error(
				"Plugin bundle digest does not match its signature metadata.",
			);
		const signatureBytes = Buffer.from(signature.signature, "base64");
		if (
			signatureBytes.byteLength !== 64 ||
			!verify(null, Buffer.from(digest, "hex"), publicKey, signatureBytes)
		)
			throw new Error("Plugin bundle signature verification failed.");
		const discovered = new PluginRegistry([sourceRoot]).discover();
		if (
			discovered.length !== 1 ||
			discovered[0]?.root !== sourceRoot ||
			discovered[0].name !== name ||
			discovered[0].version !== version
		)
			throw new Error(
				"Plugin bundle does not resolve to exactly one valid root manifest.",
			);
		if (discovered[0].skillsRoot)
			new SkillRegistry([discovered[0].skillsRoot]).discover();
		return {
			name,
			version,
			description,
	...
	private copyVerified(sourceRoot: string, stagingRoot: string): void {
		...
		for (const file of files) {
			...
			if (!within(stagingRoot, destination))
				throw new Error("Plugin staging path escaped its root.");
	...
	private stage(bundle: VerifiedPluginBundle): string {
		...
		const stagingRoot = join(
			this.managedRoot,
			`.staging-${bundle.name}-${randomUUID()}`,
		);
		try {
			this.copyVerified(bundle.sourceRoot, stagingRoot);
			const staged = this.inspect(stagingRoot);
			if (
				staged.digest !== bundle.digest ||
				staged.name !== bundle.name ||
				staged.version !== bundle.version
			)
				throw new Error("Staged plugin verification changed unexpectedly.");
			return stagingRoot;
		} catch (error) {
	...
	install(source: string): InstalledPlugin {
		...
		if (existsSync(destination))
			throw new Error(
				`Plugin ${bundle.name} is already installed; use update instead.`,
			);
	...
	update(source: string): InstalledPlugin {
		...
		const backup = join(
			trashRoot,
			`${bundle.name}-${current.version}-${randomUUID()}`,
		);
	...
	remove(name: string): RemovedPlugin {
		...
		const recoveryPath = join(
			trashRoot,
			`${current.name}-${current.version}-${randomUUID()}`,
		);
	...
	restore(recoveryPath: string): InstalledPlugin {
		...
		if (
			!within(canonicalTrashRoot, source) ||
			dirname(source) !== canonicalTrashRoot
		)
			throw new Error(
				"Plugin recovery path is outside the managed trash directory.",
			);
		const bundle = this.inspect(source);
		const destination = this.destination(bundle.name);
		if (existsSync(destination))
			throw new Error(`Plugin ${bundle.name} is already installed.`);
