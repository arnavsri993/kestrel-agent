...
const secretPattern =
	/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\b(?:sk|xox[baprs]|gh[pousr])[-_][A-Za-z0-9_-]{16,}\b/i;
...
export class SkillLearningManager {
	...
	constructor(
		...
		if (!metadata.isDirectory() || metadata.isSymbolicLink())
			throw new Error("Learned skill root must be a regular directory.");
	...
	list(): SkillLearningProposal[] {
		return (
			this.database.getPrivateState<SkillLearningProposal[]>(
				this.proposalKey,
			) ?? []
		);
	}
	listFeedback(skillName?: string): SkillLearningFeedback[] {
		const records =
			this.database.getPrivateState<SkillLearningFeedback[]>(
				this.feedbackKey,
			) ?? [];
		return skillName
			? records.filter((record) => record.skillName === skillName)
			: records;
	}
	...
	propose(
		...
		if (
			!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(input.name) ||
			input.name.length > 64
		)
			throw new Error("Learned skill name is invalid.");
		if (
			...
			throw new Error(
				"Learned skill description must be a single line of 1-1024 characters.",
			);
		if (!input.instructions.trim() || input.instructions.length > 200_000)
			throw new Error(
				"Learned skill instructions must be 1-200000 characters.",
			);
		if (input.sourceMessageIds.length === 0)
			throw new Error("Learned skill proposals require message provenance.");
		if (!this.database.getRuntimeSession(input.sourceSessionId))
			throw new Error("Learned skill source session was not found.");
		const sourceMessages = new Set(
			this.database
				.listRuntimeMessages(input.sourceSessionId)
				.map((message) => message.id),
		);
		if (input.sourceMessageIds.some((id) => !sourceMessages.has(id)))
			throw new Error(
				"Learned skill message provenance does not belong to its source session.",
			);
		if (secretPattern.test(`${input.description}\n${input.instructions}`))
			throw new Error(
				"Learned skill proposal appears to contain a credential or private key.",
			);
		const checks = this.validatePackage(
			input.name,
			input.description.trim(),
		...
		const proposal: SkillLearningProposal = {
			...
			evaluation: {
				...
				checks: [
					...checks,
					`${this.listFeedback(input.name).length} prior feedback records available`,
				],
			},
			createdAt: timestamp,
			updatedAt: timestamp,
	...
	suggestForSession(sourceSessionId: string): SkillLearningProposal {
		if (!this.database.getRuntimeSession(sourceSessionId))
			throw new Error("Learned skill source session was not found.");
		const runs = this.database.listAgentRuns(sourceSessionId);
		const latestRun = runs[runs.length - 1];
		if (!latestRun || latestRun.status !== "completed")
			throw new Error(
				"Only a completed workflow can be saved as a learned skill.",
			);
		...
		if (!userMessage)
			throw new Error(
				"The completed workflow has no user request to learn from.",
			);

		const turnMessages = messages
			.slice(userIndex)
			.filter((message) => message.role !== "system");
		const sourceMessageIds = turnMessages
			.slice(0, 40)
			.map((message) => message.id);
		const prompt = singleLine(userMessage.content, 6_000);
		if (!prompt)
			throw new Error(
				"The completed workflow has no usable user request to learn from.",
			);
		const toolNames = [
			...new Set(
		...
		return this.propose({
			...
			description: `Repeat the successful workflow for: ${prompt}`.slice(
				0,
				1_024,
			),
			instructions,
	...
	review(id: string, decision: "install" | "reject"): SkillLearningProposal {
		...
		if (current.status !== "proposed")
			throw new Error("Only proposed learned skills can be reviewed.");
		if (decision === "reject") {
			const rejected = {
				...current,
	...
	feedback(
		input: Omit<SkillLearningFeedback, "id" | "createdAt">,
	): SkillLearningFeedback {
		if (!input.feedback.trim() || input.sourceIds.length === 0)
			throw new Error("Skill feedback and provenance are required.");
		const record: SkillLearningFeedback = {
			...input,
			feedback: input.feedback.trim(),
			id: `skill-feedback-${randomUUID()}`,
			createdAt: this.now().toISOString(),
		};
		this.database.setPrivateState(
			this.feedbackKey,
			[...this.listFeedback(), record].slice(-MAX_FEEDBACK_RECORDS),
		);
		return record;
	}
	...
	private validatePackage(
		...
		if (!within(this.root, container))
			throw new Error("Learned skill validation path escaped its root.");
		mkdirSync(skillRoot, { recursive: true, mode: 0o700 });
		try {
			writeFileSync(
				resolve(skillRoot, "SKILL.md"),
				this.skillDocument(name, description, instructions),
				{ mode: 0o600, flag: "wx" },
			);
			...
			return [
				...
				"isolated Agent Skills parse passed",
			];
		} finally {
	...
	private install(proposal: SkillLearningProposal): void {
		...
		const backup = resolve(
			this.root,
			`.backup-${proposal.name}-${randomUUID()}`,
		);
		if (
			!within(this.root, target) ||
			!within(this.root, staged) ||
			!within(this.root, backup)
		)
			throw new Error("Learned skill installation path escaped its root.");
		mkdirSync(staged, { recursive: true, mode: 0o700 });
		writeFileSync(
			resolve(staged, "SKILL.md"),
			this.skillDocument(
				proposal.name,
				proposal.description,
				proposal.instructions,
		...
		try {
			if (existsSync(target)) {
				...
				if (!metadata.isDirectory() || metadata.isSymbolicLink())
					throw new Error("Existing learned skill target is unsafe.");
	...
	private saveProposals(proposals: SkillLearningProposal[]): void {
		this.database.setPrivateState(this.proposalKey, proposals);
	}
	...
	private nextName(seed: string): string {
		...
		for (let suffix = 2; suffix < 100; suffix += 1) {
			const ending = `-${suffix}`;
			const candidate = `${seed.slice(0, 64 - ending.length)}${ending}`.replace(
				/-+$/g,
				"",
			);
...
export function installSkillLearningTools(
	...
	register(
		...
		async (_context, input) => ({
			proposal: manager.propose({
				...
				sourceMessageIds: (input.sourceMessageIds as unknown[]).map(String),
			}),
		}),
	);
	register(
		...
		false,
		{
			...
			required: ["skillName", "succeeded", "feedback", "sourceIds"],
			additionalProperties: false,
		},
		async (_context, input) => ({
