...
describe("MCP extensions", () => {
  ...
  it("cancels oversized Streamable HTTP MCP responses before buffering them", async () => {
    ...
    const transport = new StreamableHttpMcpTransport("https://mcp.example.test/rpc", { fetcher: async () => response });
...
describe("Codex-compatible plugin manifests", () => {
  ...
  it("rejects an oversized plugin manifest before reading it", () => {
    ...
    mkdirSync(join(pluginRoot, ".codex-plugin"), { recursive: true });
    writeFileSync(join(pluginRoot, ".codex-plugin", "plugin.json"), Buffer.alloc(256_001));
