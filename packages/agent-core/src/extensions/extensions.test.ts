...
import { installSkillTools, SkillRegistry } from "./skills";
...
describe("MCP extensions", () => {
	it.each([
		...
		0,
		1.5,
	])("rejects invalid client timeout %s before subscribing", (timeoutMs) => {
		expect(() => new McpClient(new ControlledTransport(), timeoutMs)).toThrow(
			"finite positive integer",
		);
	});

	it("uses Streamable HTTP sessions and exposes resources and prompts", async () => {
		expect(() => new StreamableHttpMcpTransport("not a URL")).toThrow(
			"MCP HTTP endpoints",
		);
		let sessionHeader = "";
		let deleted = false;
		const fetcher: typeof fetch = async (_input, init) => {
			...
			sessionHeader =
				new Headers(init?.headers).get("mcp-session-id") ?? sessionHeader;
			if (!request.id)
				...
				request.method === "initialize"
					...
					: request.method === "resources/list"
						...
						: request.method === "resources/read"
							? { contents: [{ uri: "file:///guide", text: "safe" }] }
		...
		const transport = new StreamableHttpMcpTransport(
			...
			{ fetcher, authorization: "Bearer protected" },
		);
		...
		expect(await client.readResource("file:///guide")).toMatchObject({
			contents: [{ text: "safe" }],
		});
		expect(await client.listPrompts()).toMatchObject([{ name: "review" }]);
		expect(await client.getPrompt("review")).toMatchObject({
			description: "Review",
		});
	...
	it("cancels oversized Streamable HTTP MCP responses before buffering them", async () => {
		let pulls = 0;
		let cancellations = 0;
		...
		).rejects.toThrow("MCP HTTP response exceeded 1 MB");
	...
	it("negotiates the current lifecycle, lists tools, and calls a runtime tool", async () => {
		...
		const client = new McpClient(
			new LoopbackTransport(
				new McpRuntimeServer(fixture.runtime, fixture.session.id),
			),
		);
		...
		const result = await client.callTool("workspace.read", {
			path: "README.md",
		});
		expect(result).toMatchObject({
			isError: false,
			structuredContent: { content: expect.stringContaining("mcp-server") },
		});
		await client.close();
	...
	it("does not send a tool request for an already-aborted signal", async () => {
		...
		await expect(
			client.callTool("workspace.read", {}, controller.signal),
	...
	it("stops MCP listings that never finish pagination", async () => {
		...
		expect(
			transport.sent.filter(
				(message) => "method" in message && message.method === "resources/list",
			),
		).toHaveLength(100);
		await client.close();
	});
	...
	it("filters and rejects tool calls when a runtime server has read-only access", async () => {
		...
		expect(called).toMatchObject({
			error: { message: expect.stringContaining("task authorization") },
		});
	...
	it("contains a rejected timeout-cancellation send", async () => {
		...
		const transport: McpTransport = {
			send: () => {
				...
				return sends === 1
					? undefined
					: Promise.reject(new Error("cancellation wire down"));
			},
			onMessage: () => () => undefined,
			onError: () => () => undefined,
			close: () => undefined,
	...
	it.each([
		...
		[
			"oversized stderr record",
			"process.stderr.write('x'.repeat(1000001));setTimeout(()=>{},10000)",
			"stderr record exceeded 1 MB",
		],
		[
			"abusive stderr burst",
			"process.stderr.write(('x'.repeat(999) + '\\n').repeat(2100));setTimeout(()=>{},10000)",
			"stderr burst exceeded 1 MB",
		],
	])(
		"turns %s from a stdio server into a bounded client error",
	...
	it("bridges remote MCP tools into a session as sensitive untrusted tools", async () => {
		...
		const client = new McpClient(
			new LoopbackTransport(
				new McpRuntimeServer(source.runtime, source.session.id),
			),
		);
		const names = await bridgeMcpTools(
			client,
			target.runtime,
			target.session.id,
			"source",
		...
		expect(execution).toMatchObject({
			status: "verified",
			output: {
				structuredContent: { content: expect.stringContaining("source") },
			},
		});
		await client.close();
...
describe("Agent Skills extensions", () => {
	it("uses metadata-first discovery, full activation, and contained resource reads", async () => {
		...
		writeFileSync(
			...
			"Check correctness, safety, and tests.\n",
		);
		...
		expect(discovered).toMatchObject([
			{
				name: "code-review",
				description: expect.stringContaining("Review code"),
				metadata: { author: "kestrel-test" },
			},
		]);
		expect(discovered[0]).not.toHaveProperty("instructions");
		expect(registry.activate("code-review").instructions).toContain(
			"Inspect the diff",
		);
		expect(
			registry.readResource("code-review", "references/CHECKLIST.md").content,
		).toContain("correctness");
		expect(() => registry.readResource("code-review", "../secret.txt")).toThrow(
			"escapes its skill root",
		);

		const fixture = runtimeFixture("skills");
		installSkillTools(fixture.runtime, registry, fixture.session.id);
		const listed = await fixture.runtime.callTool(
			fixture.session.id,
			"skills.list",
			{},
		);
		expect(listed).toMatchObject({
			status: "verified",
			output: { skills: [{ name: "code-review" }] },
		});
		expect(
			await fixture.runtime.callTool(fixture.session.id, "skills.list", {
				query: "correctness review",
			}),
		).toMatchObject({
			output: {
				skills: [{ name: "code-review", relevance: expect.any(Number) }],
			},
		});
		expect(
			await fixture.runtime.callTool(fixture.session.id, "skills.list", {
				query: "calendar travel",
			}),
		).toMatchObject({ output: { skills: [] } });
		const activated = await fixture.runtime.callTool(
			fixture.session.id,
			"skills.activate",
			{ name: "code-review" },
		);
	...
	it("rejects oversized skill instructions before reading them", () => {
		...
		expect(() => new SkillRegistry([container]).discover()).toThrow(
	...
	it("turns provenance-backed experience into reviewable, validated, feedback-informed learned skills", () => {
		...
		const firstSource = runtime.appendMessage({
			sessionId: source.id,
			role: "user",
			content: "Remember the release verification steps.",
		});
		const secondSource = runtime.appendMessage({
			sessionId: source.id,
			role: "assistant",
			content: "The packaged executable also needs verification.",
		});
		...
		const proposal = manager.propose({
			...
			instructions: "Run the build and verify its checksums before publishing.",
		...
		expect(proposal).toMatchObject({
			...
			evaluation: {
				valid: true,
				checks: expect.arrayContaining(["isolated Agent Skills parse passed"]),
			},
		});
		expect(registry.list()).toEqual([]);
		expect(manager.review(proposal.id, "install").status).toBe("installed");
		expect(registry.activate("release-check").instructions).toContain(
			"verify its checksums",
		);
		const feedback = manager.feedback({
			skillName: "release-check",
			succeeded: false,
			feedback: "Also verify the packaged executable.",
			sourceIds: ["run-1"],
		});
		expect(feedback.succeeded).toBe(false);
		const revision = manager.propose({
			name: "release-check",
			description: "Verify a release before publishing.",
			instructions:
				"Run the build, packaged executable, and checksum verification before publishing.",
			sourceSessionId: source.id,
			sourceMessageIds: [secondSource.id],
		});
		expect(revision.evaluation.checks).toContain(
			"1 prior feedback records available",
		);
		...
		const encrypted = database.db
			...
			.get("skills.learning.proposals") as { value_ciphertext: string };
		expect(encrypted.value_ciphertext).not.toContain("packaged executable");
		expect(() =>
			manager.propose({
				name: "unsafe",
				description: "Unsafe",
				instructions: "Use sk-example0123456789012345",
				sourceSessionId: source.id,
				sourceMessageIds: [firstSource.id],
			}),
		).toThrow("credential");
		database.close();
	});
	...
	it("turns a completed session into a reviewable workflow proposal without copying tool output", () => {
		const learnedRoot = mkdtempSync(
			join(tmpdir(), "kestrel-learned-skill-suggestion-"),
		);
		directories.push(learnedRoot);
		const database = new KestrelDatabase(":memory:", createEncryptionKey());
		const runtime = new AgentRuntime(
		...
		const user = runtime.appendMessage({
			sessionId: source.id,
			role: "user",
			content: "Prepare the weekly release notes.",
		});
		runtime.appendMessage({
			sessionId: source.id,
			role: "tool",
			toolName: "workspace.read",
			content: "PRIVATE_TOOL_OUTPUT_SHOULD_NOT_BE_COPIED",
		});
		const assistant = runtime.appendMessage({
			sessionId: source.id,
			role: "assistant",
		...
		database.saveAgentRun({
			...
			status: "completed",
			turn: 1,
			createdAt: "2026-07-22T22:59:00.000Z",
			updatedAt: "2026-07-22T23:00:00.000Z",
		});
		const manager = new SkillLearningManager(
			database,
			learnedRoot,
			new SkillRegistry([learnedRoot]),
			() => new Date("2026-07-22T23:00:00.000Z"),
		);
		const proposal = manager.suggestForSession(source.id);
		expect(proposal).toMatchObject({
			name: "prepare-the-weekly-release-notes",
			status: "proposed",
			sourceSessionId: source.id,
			sourceMessageIds: [user.id, expect.any(String), assistant.id],
		});
		expect(proposal.instructions).toContain("workspace.read");
		expect(proposal.instructions).not.toContain(
			"PRIVATE_TOOL_OUTPUT_SHOULD_NOT_BE_COPIED",
		);
		database.close();
	});
	...
	it("bounds learned skill feedback history", () => {
		const learnedRoot = mkdtempSync(
			join(tmpdir(), "kestrel-learned-skill-feedback-"),
		);
		directories.push(learnedRoot);
		const database = new KestrelDatabase(":memory:", createEncryptionKey());
		...
		const latest = manager.feedback({
			skillName: "release-check",
			succeeded: false,
			feedback: "Review the packaged executable too.",
			sourceIds: ["run-latest"],
		});

		const records = manager.listFeedback();
		expect(records).toHaveLength(500);
		expect(records[0]?.id).toBe("skill-feedback-1");
		expect(records.some((record) => record.id === "skill-feedback-0")).toBe(
			false,
		);
...
describe("Codex-compatible plugin manifests", () => {
	it("discovers Camarade-shaped manifests, persists enablement, and exposes enabled skill roots", () => {
		...
		mkdirSync(join(pluginRoot, "skills", "improve-coding-prompt"), {
			recursive: true,
		});
		writeFileSync(
			join(pluginRoot, "skills", "improve-coding-prompt", "SKILL.md"),
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				interface: {
					...
					capabilities: ["Read"],
					defaultPrompt: ["Improve this request."],
				},
			}),
		);
		const database = new KestrelDatabase(":memory:", createEncryptionKey());
		...
		expect(registry.discover()).toMatchObject([
			{
				name: "camarade",
				version: "0.1.0-test",
				enabled: false,
				interface: { displayName: "Camarade" },
			},
		]);
		expect(registry.summary()).toMatchObject([
			{ name: "camarade", hasSkills: true, hasMcpServers: true },
		]);
		expect(registry.skillRoots()).toEqual([]);
		registry.setEnabled("camarade", true);
		expect(registry.skillRoots()).toEqual([
			realpathSync(join(pluginRoot, "skills")),
		]);
		expect(
			new PluginRegistry([container], database).discover()[0]?.enabled,
		).toBe(true);
		database.close();
	});

	it("rejects an oversized plugin manifest before reading it", () => {
	...
	it("enforces declared plugin dependency versions and enable order", () => {
		...
		const registry = new PluginRegistry([container], database);
		registry.discover();
		expect(() => registry.setEnabled("dependent", true)).toThrow(
			"Enable dependency base",
		);
		registry.setEnabled("base", true);
		expect(registry.setEnabled("dependent", true).enabled).toBe(true);
		expect(() => registry.setEnabled("base", false)).toThrow(
			"Disable dependent plugin dependent",
		);
	...
	it("connects explicitly approved plugin MCP servers and removes their tools on disconnect", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				name: "example",
				version: "1.0.0",
				description: "Example MCP plugin.",
				mcpServers: "./.mcp.json",
			}),
		);
		writeFileSync(
			...
			JSON.stringify({
				mcpServers: {
					example: { command: "node", args: ["./mcp/server.mjs"], cwd: "." },
				},
			}),
		);
		writeFileSync(
		...
		expect(await manager.connect("example", session.id)).toMatchObject([
			{
				pluginName: "example",
				serverName: "example",
				toolNames: ["mcp.example.example.echo"],
			},
		]);
		const blocked = await runtime.callTool(
			session.id,
			"mcp.example.example.echo",
			{ text: "hello" },
			{ idempotencyKey: "echo" },
		);
		...
		const called = await runtime.callTool(
			...
			"mcp.example.example.echo",
			{ text: "hello" },
			{ approvalStatus: "approved", idempotencyKey: "echo" },
		);
		expect(called).toMatchObject({
			status: "verified",
			output: { structuredContent: { text: "hello" } },
		});
		const secondSession = runtime.createSession({
			title: "Second plugin session",
		});
		manager.attachSession(secondSession.id);
		expect(
			runtime
				.discoverTools(secondSession.id)
				.some((tool) => tool.name === "mcp.example.example.echo"),
		).toBe(true);
		expect(await manager.connect("example", secondSession.id)).toMatchObject([
			{ toolNames: ["mcp.example.example.echo"] },
		]);
	...
	it("rejects plugin-controlled process loader environment variables", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				name: "unsafe",
				version: "1.0.0",
				description: "Unsafe MCP environment fixture.",
				mcpServers: "./.mcp.json",
			}),
		);
		writeFileSync(
			...
			JSON.stringify({
				mcpServers: {
					unsafe: {
						command: "node",
						args: ["./mcp/server.mjs"],
						env: { NODE_OPTIONS: "--import ./mcp/server.mjs" },
					},
				},
			}),
		);
		...
		const manager = new PluginMcpManager(registry, runtime);
		await expect(manager.connect("unsafe", session.id)).rejects.toThrow(
	...
	it("enables a discovered Camarade bundle through the core and refreshes skills without restart", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				description: "Repository-aware prompt improvement.",
				skills: "./skills/",
				interface: {
					displayName: "Camarade",
					capabilities: ["Read"],
		...
		expect(await core.handle({ type: "plugin-list" })).toMatchObject({
			ok: true,
			plugins: [
				{ name: "camarade", enabled: false, version: "0.1.0+codex.test" },
			],
		});
		...
		expect(
			...
			ok: true,
			plugins: [{ name: "camarade", enabled: true }],
		});
		expect(core.skillRegistry?.list()).toMatchObject([
			{ name: "improve-coding-prompt" },
		]);
		const main = core.runtime.ensureMainSession();
		expect(
			await core.runtime.callTool(main.id, "skills.list", {}),
		).toMatchObject({
			status: "verified",
			output: { skills: [{ name: "improve-coding-prompt" }] },
		});
		core.close();
	});
});
