...
export class StreamableHttpMcpTransport implements McpTransport {
	...
	constructor(
		...
		try {
			...
			throw new Error(
				"MCP HTTP endpoints require credential-free HTTPS or loopback HTTP.",
			);
		}
		...
		if (
			(this.url.protocol !== "https:" &&
				!(this.url.protocol === "http:" && loopback)) ||
			this.url.username ||
			this.url.password ||
			this.url.hash
		)
	...
	async send(message: JsonRpcMessage): Promise<void> {
		...
		const bytes = await readBoundedResponseBytes(
			response,
			1_000_000,
			"MCP HTTP response exceeded 1 MB.",
		);
		...
		for (const payload of payloads)
			this.events.emit("message", JSON.parse(payload) as JsonRpcMessage);
	}
	...
	close(): Promise<void> {
		...
		this.closePromise = (async () => {
			try {
				if (this.sessionId)
					await (this.options.fetcher ?? fetch)(this.url, {
						method: "DELETE",
						headers: {
...
export class StdioMcpTransport implements McpTransport {
	...
	constructor(config: {
		...
		this.child.stdout.on("data", (chunk: Buffer) => {
			...
			if (
				Buffer.byteLength(this.buffer) > StdioMcpTransport.MAX_MESSAGE_BYTES
			) {
			...
			while ((newlineIndex = this.buffer.indexOf("\n")) !== -1) {
				...
				try {
					...
					this.child.kill("SIGTERM");
					this.reportError(
						new Error("MCP server wrote invalid JSON to stdout."),
					);
					return;
				}
			}
		...
		this.child.stderr.on("data", (chunk: Buffer) => {
			...
			for (let offset = 0; offset < chunk.byteLength; offset += 1) {
				...
				if (
					this.stderrRecordBytes > StdioMcpTransport.MAX_STDERR_RECORD_BYTES
				) {
					this.child.kill("SIGTERM");
					this.reportError(
			...
			if (this.stderrBurstBytes > StdioMcpTransport.MAX_STDERR_BURST_BYTES) {
				this.child.kill("SIGTERM");
				this.reportError(
		...
		this.child.once("error", (error) =>
			this.reportError(new Error(`MCP stdio server failed: ${error.message}`)),
		);
		this.child.once("exit", (code, signal) => {
			if (!this.closing && !this.failure)
				this.reportError(
					new Error(
						`MCP stdio server exited unexpectedly (${signal ?? code ?? "unknown"}).`,
					),
				);
		});
		this.child.stdin.on("error", (error) => {
			if (!this.closing)
				this.reportError(new Error(`MCP stdio input failed: ${error.message}`));
		});
	}
	...
	async send(message: JsonRpcMessage): Promise<void> {
		...
		if (
			Buffer.byteLength(encoded, "utf8") > StdioMcpTransport.MAX_MESSAGE_BYTES
		)
			throw new Error("MCP stdio message exceeds 1 MB.");
		if (encoded.includes("\n"))
			throw new Error(
				"MCP stdio messages must be newline-delimited single-line JSON.",
			);
...
export class McpClient {
	...
	async initialize(): Promise<Record<string, unknown>> {
		...
		const negotiated = String(
			(result as Record<string, unknown>).protocolVersion ?? "",
		);
		if (negotiated !== MCP_PROTOCOL_VERSION)
			throw new Error(`Unsupported MCP protocol version ${negotiated}.`);
	...
	async listTools(): Promise<McpTool[]> {
		...
		do {
			pages += 1;
			if (pages > McpClient.MAX_PAGINATION_PAGES)
				throw new Error("MCP tools/list pagination exceeded 100 pages.");
			const result = (await this.request(
				"tools/list",
				cursor ? { cursor } : {},
			)) as Record<string, unknown>;
			if (!Array.isArray(result.tools))
				throw new Error("MCP tools/list returned an invalid tool array.");
	...
	async readResource(uri: string): Promise<Record<string, unknown>> {
		this.requireInitialized();
		return (await this.request("resources/read", { uri })) as Record<
			string,
			unknown
		>;
	}
	...
	private async paginatedList(
		...
		do {
			pages += 1;
			if (pages > McpClient.MAX_PAGINATION_PAGES)
				throw new Error(`MCP ${method} pagination exceeded 100 pages.`);
			const result = (await this.request(
				method,
				cursor ? { cursor } : {},
			)) as Record<string, unknown>;
			...
			if (
				!Array.isArray(page) ||
				page.some((item) => !item || typeof item !== "object")
			)
				throw new Error(`MCP ${method} returned an invalid ${field} array.`);
	...
	private async send(message: JsonRpcMessage): Promise<void> {
		...
		try {
			...
			const failure =
				error instanceof Error ? error : new Error("MCP transport failed.");
			this.fail(failure);
			throw failure;
		}
	}
...
export class McpRuntimeServer {
	...
	async handle(
		...
		access: McpRuntimeAccess = { allowMutatingTools: true },
...
function safeToolName(serverId: string, toolName: string): string {
	const suffix = `${serverId}.${toolName}`
		.toLowerCase()
		.replace(/[^a-z0-9_.-]+/g, "-")
		.replace(/^[^a-z]+/, "");
	...
	for (const tool of await client.listTools()) {
		...
		runtime.registerExternalTool({
			...
			execute: async ({ signal }, input) => {
				...
				if (result.isError)
					throw new Error(JSON.stringify(result.content).slice(0, 2_000));
				return {
