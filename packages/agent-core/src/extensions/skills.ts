...
function parseSkill(path: string): ActivatedSkill {
	if (statSync(path).size > 512_000)
		throw new Error("SKILL.md exceeds the 512 KB safety limit.");
	const content = readFileSync(path, "utf8");
	if (Buffer.byteLength(content) > 512_000)
		throw new Error("SKILL.md exceeds the 512 KB safety limit.");
	...
	if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name) || name.length > 64)
		throw new Error(
			"Skill name does not satisfy the Agent Skills naming rules.",
		);
	...
	if (values.compatibility && values.compatibility.length > 500)
		throw new Error("Skill compatibility exceeds 500 characters.");
	return {
		name,
		description,
		root,
		...
		...(values["allowed-tools"]
			? { allowedTools: values["allowed-tools"].split(/\s+/).filter(Boolean) }
			: {}),
		metadata,
		instructions,
	};
...
export class SkillRegistry {
	...
	discover(): SkillDescriptor[] {
		...
		for (const configuredRoot of this.roots) {
			...
			for (const candidate of candidates) {
				...
				const { instructions: _instructions, ...descriptor } =
					parseSkill(skillFile);
				if (this.skills.has(descriptor.name))
					throw new Error(`Duplicate skill ${descriptor.name}.`);
	...
	rank(query: string): Array<SkillDescriptor & { relevance: number }> {
		...
		return this.list()
			.map((skill) => {
				...
				const haystack =
					`${name} ${skill.description} ${Object.entries(skill.metadata).flat().join(" ")}`.toLowerCase();
			...
			.filter((skill) => terms.length === 0 || skill.relevance > 0)
			.sort(
				(left, right) =>
	...
	readResource(
		...
		if (!within(descriptor.root, candidate))
			throw new Error("Skill resource escapes its skill root.");
		if (!statSync(candidate).isFile())
			throw new Error("Skill resource must be a file.");
		const buffer = readFileSync(candidate);
		if (buffer.byteLength > 1_000_000)
			throw new Error("Skill resources are limited to 1 MB.");
		if (buffer.includes(0))
			throw new Error(
				"Binary skill resources are not exposed as model context.",
			);
		return { path: relativePath, content: buffer.toString("utf8") };
	}
}

export function installSkillTools(
	...
	runtime.registerExternalTool({
		descriptor: {
			...
			description:
				"List installed Agent Skills metadata without loading their full instructions.",
			category: "extension",
			riskLevel: "read_only",
			readOnly: true,
			requiresWorkspace: false,
			source: "skill",
			tags: ["skills", "discovery", "progressive-disclosure"],
		},
		inputSchema: {
			type: "object",
		...
		execute: (_context, input) => ({
			skills: registry
				.rank(String(input.query ?? ""))
				.map(({ root: _root, ...skill }) => skill),
		}),
	});
	runtime.registerExternalTool({
		descriptor: {
			...
			description:
				"Load one discovered SKILL.md fully after its metadata matches the task.",
			category: "extension",
			riskLevel: "read_only",
			readOnly: true,
			requiresWorkspace: false,
			source: "skill",
			tags: ["skills", "instructions", "activate"],
		},
		inputSchema: {
			type: "object",
	...
	runtime.registerExternalTool({
		descriptor: {
			name: "skills.read-resource",
			title: "Read Agent Skill resource",
			description:
				"Read a bounded text resource inside an activated skill root.",
			category: "extension",
			riskLevel: "read_only",
			readOnly: true,
			requiresWorkspace: false,
			source: "skill",
			tags: ["skills", "references", "assets", "scripts"],
		},
		inputSchema: {
			type: "object",
		...
		execute: (_context, input) =>
			registry.readResource(String(input.name ?? ""), String(input.path ?? "")),
	});
	for (const name of ["skills.list", "skills.activate", "skills.read-resource"])
