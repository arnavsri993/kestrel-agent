...
function resolvePluginPath(root: string, value: unknown): string | undefined {
	...
	if (!within(root, candidate))
		throw new Error("Plugin manifest path escapes the plugin root.");
	return candidate;
}
...
export class PluginRegistry {
	...
	discover(): PluginDescriptor[] {
		...
		for (const configuredRoot of this.roots) {
			...
			for (const manifestPath of findManifests(root)) {
				if (statSync(manifestPath).size > 256_000)
					throw new Error("Plugin manifest exceeds 256 KB.");
				...
				const dependencies = rawDependencies
					? Object.fromEntries(
							Object.entries(rawDependencies).map(([dependency, version]) => {
								if (
									...
									throw new Error("Plugin dependency declaration is invalid.");
								return [dependency, version];
							}),
						)
					: undefined;
				const skillsRoot = resolvePluginPath(pluginRoot, manifest.skills);
				const mcpServersPath = resolvePluginPath(
					pluginRoot,
					manifest.mcpServers,
				);
				...
				const descriptor: PluginDescriptor = {
					...
					...(ui
						? {
								interface: {
									...(typeof ui.displayName === "string"
										? { displayName: ui.displayName }
										: {}),
									...(typeof ui.shortDescription === "string"
										? { shortDescription: ui.shortDescription }
										: {}),
									...(typeof ui.category === "string"
										? { category: ui.category }
										: {}),
									capabilities: Array.isArray(ui.capabilities)
										? ui.capabilities.filter(
												(value): value is string => typeof value === "string",
											)
										: [],
									defaultPrompt: Array.isArray(ui.defaultPrompt)
										? ui.defaultPrompt.filter(
												(value): value is string => typeof value === "string",
											)
										: [],
								},
						...
						: {}),
					enabled:
						this.database?.getState<unknown>(`plugin.enabled.${name}`) === true,
					managed: this.managedRoots.some((managedRoot) =>
						within(managedRoot, pluginRoot),
					),
				};
	...
	setEnabled(name: string, enabled: boolean): PluginDescriptor {
		...
		if (enabled) {
			for (const [dependencyName, requiredVersion] of Object.entries(
				...
				if (!dependency)
					throw new Error(
						`Plugin ${name} requires missing dependency ${dependencyName}.`,
					);
				if (requiredVersion !== "*" && dependency.version !== requiredVersion)
					throw new Error(
						`Plugin ${name} requires ${dependencyName} ${requiredVersion}, but ${dependency.version} is installed.`,
					);
				if (!dependency.enabled)
					throw new Error(
						`Enable dependency ${dependencyName} before ${name}.`,
					);
			}
		} else {
			const dependent = [...this.plugins.values()].find(
				(candidate) =>
					candidate.enabled &&
					candidate.name !== name &&
					candidate.dependencies?.[name],
			);
			if (dependent)
				throw new Error(
					`Disable dependent plugin ${dependent.name} before ${name}.`,
				);
		}
	...
	skillRoots(): string[] {
		return this.list()
			.filter((plugin) => plugin.enabled && plugin.skillsRoot)
			.map((plugin) => plugin.skillsRoot!);
	}
	...
	summary(): Array<
		Omit<
			...
			| "mcpServersPath"
			| "dashboardPath"
		> & { hasSkills: boolean; hasMcpServers: boolean; hasDashboard: boolean }
	> {
