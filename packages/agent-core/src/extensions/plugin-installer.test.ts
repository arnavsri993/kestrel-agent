...
function fixture() {
  ...
  const installer = new PluginInstaller({ managedRoot, trustKeys: [{ keyId: "publisher.test", publicKey }] });
  const createBundle = (version: string, content: string) => {
    ...
    writeFileSync(join(bundle, ".codex-plugin", "plugin.json"), JSON.stringify({ name: "example", version, description: "Signed example plugin.", skills: "./skills" }));
    writeFileSync(join(bundle, "skills", "example", "SKILL.md"), `---\nname: example\ndescription: Example skill.\n---\n\n${content}\n`);
    const digest = installer.digestForSigning(bundle).digest;
    writeFileSync(join(bundle, ".codex-plugin", "signature.json"), JSON.stringify({ algorithm: "ed25519", keyId: "publisher.test", digest, signature: sign(null, Buffer.from(digest, "hex"), privateKey).toString("base64") }));
    return bundle;
  };
...
describe("signed plugin installer", () => {
  it("installs, atomically updates, removes recoverably, and restores a trusted bundle", () => {
    ...
    expect(first).toMatchObject({ name: "example", version: "1.0.0", keyId: "publisher.test" });
    expect(new PluginRegistry([managedRoot], undefined, [managedRoot]).discover()).toMatchObject([{ name: "example", version: "1.0.0", enabled: false, managed: true }]);

    const updated = installer.update(createBundle("2.0.0", "Second version."));
    expect(updated).toMatchObject({ name: "example", version: "2.0.0", replacedVersion: "1.0.0" });
    expect(readFileSync(join(managedRoot, "example", "skills", "example", "SKILL.md"), "utf8")).toContain("Second version.");
    expect(new PluginRegistry([managedRoot]).discover()).toMatchObject([{ name: "example", version: "2.0.0" }]);
    ...
    expect(restored).toMatchObject({ name: "example", version: "2.0.0", installedRoot: join(managedRoot, "example") });
  ...
  it("rejects tampering, untrusted publishers, and symbolic links", () => {
    ...
    writeFileSync(join(tampered, "skills", "example", "SKILL.md"), "tampered\n");
    expect(() => installer.inspect(tampered)).toThrow("digest does not match");

    const trusted = createBundle("1.0.1", "Trusted content.");
    const untrusted = new PluginInstaller({ managedRoot: join(root, "other-managed"), trustKeys: [] });
