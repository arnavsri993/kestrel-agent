...
export class NativeChannelAdapter implements ChannelAdapter {
  ...
  private async jsonBody(url: string, body: BodyInit, signal: AbortSignal, headers: Record<string, string>): Promise<Record<string, unknown>> { const response = await this.fetcher(url, { method: "POST", signal, headers: { authorization: `Bearer ${await this.accessToken()}`, ...headers }, body }); if (!response.ok) throw new Error(`Channel provider failed with status ${response.status}.`); const bytes = await readBoundedResponseBytes(response, 1_000_000, "Channel provider response exceeds 1 MB."); let parsed: unknown; try { parsed = JSON.parse(new TextDecoder().decode(bytes)) as unknown; } catch { throw new Error("Channel provider returned malformed JSON."); } if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Channel provider returned an invalid JSON response."); return parsed as Record<string, unknown>; }
