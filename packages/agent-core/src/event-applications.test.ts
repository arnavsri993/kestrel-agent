...
describe("event application manager", () => {
	it("persists a review and explicit approval boundary before submission", () => {
		...
		const manager = new EventApplicationManager(
			database,
			() => new Date("2026-07-23T14:00:00.000Z"),
		);
		const draft = manager.create({
			title: "Builder Weekend",
			...
			eligibility: [
				{
					...
					label: "18 or older",
					met: true,
					evidence: "Confirmed by applicant",
				},
			],
			answers: [
				{
		...
		expect(() => manager.markSubmitted(draft.id, "receipt")).toThrow(
			"approved",
		);
		expect(
			manager.update(draft.id, { status: "approved" }).approvedAt,
		).toBeTruthy();
		expect(manager.markSubmitted(draft.id, "Confirmation #123")).toMatchObject({
			status: "submitted",
			receipt: "Confirmation #123",
		});
	...
	it("rejects unsafe URLs and incomplete review", () => {
		...
		expect(() =>
			manager.create({ title: "Event", organizer: "Host", url: "not a URL" }),
		).toThrow("HTTPS");
		expect(() =>
			manager.create({
				title: "Event",
				organizer: "Host",
				url: "http://events.example.test",
			}),
		).toThrow("HTTPS");
		const draft = manager.create({
			title: "Event",
		...
		expect(() => manager.update(draft.id, { status: "approved" })).toThrow(
			"Review",
		);
		database.close();
	});
