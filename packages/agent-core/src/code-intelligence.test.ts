...
class LoopbackLanguageServer implements LanguageServerTransport {
	...
	async send(message: LspMessage): Promise<void> {
				...
				this.emit({
					...
					result:
						message.method === "textDocument/definition"
							? [{ uri: "file:///project/a.ts", range: {} }]
							: message.method === "textDocument/references"
								? [{ uri: "file:///project/b.ts", range: {} }]
								: {},
				}),
			);
		}
		if (
			...
			const uri = (message.params as { textDocument: { uri: string } })
				.textDocument.uri;
			queueMicrotask(() =>
				this.emit({
					jsonrpc: "2.0",
					...
					params: {
						uri,
						diagnostics: [{ severity: 1, message: "Example error" }],
					},
				}),
			);
		}
	...
	private emit(message: LspMessage): void {
...
describe("language server code intelligence", () => {
	it.each([
		...
		expect(() => new LanguageServerClient(transport, timeoutMs)).toThrow(
	...
	it("negotiates LSP and serves approval-gated diagnostics, definitions, and references", async () => {
		...
		expect(
			(
				await runtime.callTool(session.id, "code.diagnostics", {
		...
		const diagnostics = await runtime.callTool(
			session.id,
			"code.diagnostics",
			{ uri: "file:///project/a.ts", languageId: "typescript", text: "bad" },
			{ approvalStatus: "approved" },
		);
		expect(diagnostics).toMatchObject({
			status: "verified",
			output: {
				diagnostics: [{ message: "Example error" }],
		...
		const malformedVersionChange = [...transport.sent]
			.reverse()
			.find((message) => message.method === "textDocument/didChange");
		expect(
			(malformedVersionChange?.params as { textDocument: { version: number } })
		...
		const definition = await runtime.callTool(
			session.id,
			"code.definition",
			{ uri: "file:///project/a.ts", line: 1, character: 2 },
			{ approvalStatus: "approved" },
		);
		expect(definition.output?.locations).toEqual([
			{ uri: "file:///project/a.ts", range: {} },
		]);
		const references = await runtime.callTool(
			session.id,
			"code.references",
			{ uri: "file:///project/a.ts", line: 1, character: 2 },
			{ approvalStatus: "approved" },
		);
		expect(references.output?.locations).toEqual([
			{ uri: "file:///project/b.ts", range: {} },
		]);
		await client.close();
		database.close();
	...
	it("initializes and shuts down a configured owner-only stdio language server", async () => {
		...
		try {
			const configured = await environmentLanguageServerClient({
				KESTREL_LSP_CONFIG: configPath,
			});
			expect(
				await configured?.client.definition(
					pathToFileURL(join(root, "a.ts")).toString(),
					{ line: 0, character: 0 },
				),
			).toEqual([]);
	...
	it("normalizes malformed language server configuration JSON", async () => {
		...
		try {
			await expect(
				environmentLanguageServerClient({ KESTREL_LSP_CONFIG: configPath }),
			).rejects.toThrow("Language server configuration is invalid.");
		} finally {
			rmSync(root, { recursive: true, force: true });
		}
	});
