...
const remoteToolStatuses = new Set<RuntimeToolExecution["status"]>([
	"running",
	"verified",
	"blocked",
	"failed",
...
const remoteProgressStates = new Set<
	NonNullable<RemoteRuntimeEvent["payload"]["state"]>
...
function bearer(request: IncomingMessage): string | undefined {
	...
	if (!authorization.startsWith("Bearer ") || authorization.length > 600)
		throw new Error("Remote bearer token is invalid.");
...
function credentialKey(credential: RemoteCredential): string {
	return typeof credential === "string"
		? credential
		: `proxy:${credential.identity}:${credential.scopes.slice().sort().join(",")}`;
}
...
function webAsset(
	response: ServerResponse,
	asset: NonNullable<ReturnType<typeof remoteWebAsset>>,
): void {
	...
	response.writeHead(200, {
		...
		"content-security-policy":
			"default-src 'self'; connect-src 'self'; img-src 'self'; style-src 'self'; script-src 'self'; manifest-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'",
		"permissions-policy":
...
function serializeRemoteRuntimeEvent(rawEvent: unknown): string | undefined {
	...
	if (event.type === "tool.started" || event.type === "tool.completed") {
		...
		if (typeof toolName === "string" && remoteToolNamePattern.test(toolName))
			payload.toolName = toolName;
		if (
			typeof status === "string" &&
			remoteToolStatuses.has(status as RuntimeToolExecution["status"])
		)
		...
		if (
			typeof state === "string" &&
			remoteProgressStates.has(
				state as NonNullable<RemoteRuntimeEvent["payload"]["state"]>,
			)
		)
			payload.state = state as NonNullable<
				RemoteRuntimeEvent["payload"]["state"]
			>;
	}
...
function jobInput(
	body: Record<string, unknown>,
): Omit<ScheduledAgentJob, "id" | "status" | "createdAt" | "updatedAt"> {
	const rawProviders = body.providerIds;
	if (
		!Array.isArray(rawProviders) ||
		rawProviders.length < 1 ||
		rawProviders.length > 8 ||
		rawProviders.some(
			(value) => typeof value !== "string" || !value || value.length > 100,
		)
	)
		throw new Error("Field providerIds is invalid.");
	...
	if (
		...
		throw new Error("Field schedule is invalid.");
	const scheduleRecord = rawSchedule as Record<string, unknown>;
	const nextRunAt = stringField(scheduleRecord, "nextRunAt", 100);
	if (!Number.isFinite(new Date(nextRunAt).getTime()))
		throw new Error("Schedule nextRunAt is invalid.");
	const schedule =
		scheduleRecord.kind === "once"
			? { kind: "once" as const, nextRunAt }
			: scheduleRecord.kind === "interval" &&
					Number.isInteger(scheduleRecord.intervalMs) &&
					Number(scheduleRecord.intervalMs) >= 60_000 &&
					Number(scheduleRecord.intervalMs) <= 31_536_000_000
				? {
						kind: "interval" as const,
						nextRunAt,
						intervalMs: Number(scheduleRecord.intervalMs),
					}
				: scheduleRecord.kind === "cron" &&
						typeof scheduleRecord.expression === "string" &&
						scheduleRecord.expression.length > 0 &&
						scheduleRecord.expression.length <= 200
					? {
							kind: "cron" as const,
							nextRunAt,
							expression: scheduleRecord.expression,
						}
					: undefined;
	if (!schedule) throw new Error("Schedule kind or interval is invalid.");
	const instructions =
		typeof body.instructions === "string" && body.instructions.length <= 100_000
			? body.instructions
			: undefined;
	return {
...
export class RemoteHttpServer {
	...
	constructor(private readonly options: RemoteHttpServerOptions) {
		for (const [name, value] of [
			["maximumRequestsPerMinute", options.maximumRequestsPerMinute],
			["maximumSseClients", options.maximumSseClients],
		] as const) {
			if (
				value !== undefined &&
	...
	async start(): Promise<{ origin: string }> {
		...
		if (
			!isLoopback(host) &&
			!this.options.tls &&
			!(this.options.trustedProxy && this.options.allowProxyTerminatedTls)
		)
			throw new Error(
				"Non-loopback remote transport requires TLS or explicit trusted-proxy TLS termination.",
			);
		if (this.options.allowProxyTerminatedTls && !this.options.trustedProxy)
			throw new Error(
				"Proxy-terminated TLS requires trusted-proxy authentication.",
			);
		...
		return {
			origin: `${this.options.tls ? "https" : "http"}://${displayHost}:${address.port}`,
		};
	}
	...
	private checkRate(request: IncomingMessage): void {
		...
		record.count += 1;
		if (record.count > (this.options.maximumRequestsPerMinute ?? 120))
			throw new Error("Remote request rate limit exceeded.");
	}
	...
	private async handle(
		...
		try {
			...
			if (request.method === "OPTIONS") {
				response.writeHead(204, {
					...
					"access-control-allow-headers":
						"authorization,content-type,x-kestrel-session-id",
			...
			const asset =
				request.method === "GET" ? remoteWebAsset(url.pathname) : undefined;
			if (asset) {
				webAsset(response, asset);
				return;
			}
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/pairings/complete"
			) {
				const body = await readJson(request);
				json(
					response,
					200,
					this.options.remote.completePairing(
						stringField(body, "pairingId", 200),
						stringField(body, "code", 200),
					),
				);
				return;
			}
			if (
				request.method === "POST" &&
				url.pathname === "/v1/channels/inbound"
			) {
				if (!this.options.channelGateway || !this.options.resolveChannelSession)
					throw new Error("Channel ingress is not configured.");
				const signature = request.headers["x-kestrel-signature"];
				if (typeof signature !== "string" || signature.length > 256)
					throw new Error("Channel signature is required.");
				const envelope = channelEnvelope(await readJson(request));
				const sessionId = this.options.resolveChannelSession(envelope);
				json(
					response,
					202,
					this.options.channelGateway.receive(sessionId, envelope, signature),
				);
				return;
			}
			const token =
				bearer(request) ??
				this.options.trustedProxy?.authorize({
			...
			if (!token)
				throw new Error(
					"Remote bearer token or trusted proxy identity is required.",
				);
			if (
				request.method === "GET" &&
				url.pathname === "/v1/diagnostics/prometheus"
			) {
				if (!this.options.prometheusMetrics) {
					json(response, 404, {
						error: "Prometheus diagnostics are disabled.",
					});
					return;
				}
				this.options.remote.assertAuthorized(
					token,
					"read" satisfies RemoteScope,
				);
			...
			if (request.method === "GET" && url.pathname === "/v1/presence") {
				if (!this.options.presence) {
					json(response, 404, { error: "Presence is not configured." });
					return;
				}
				this.options.remote.assertAuthorized(
					token,
			...
			if (request.method === "POST" && url.pathname === "/v1/presence") {
				...
				if (!["ui", "webchat", "node", "test"].includes(mode))
			...
			if (url.pathname === "/v1/nodes" && request.method === "GET") {
				if (!this.options.nativeNodes) {
					json(response, 404, { error: "Native nodes are not configured." });
					return;
				}
			...
			if (url.pathname === "/v1/nodes/beacon" && request.method === "POST") {
				...
				const body = await readJson(request);
				json(response, 200, {
					node: this.options.nativeNodes.beacon({
						nodeId: stringField(body, "nodeId", 128),
						label: stringField(body, "label", 100),
						platform: stringField(body, "platform", 20) as
							| "ios"
							| "android"
							| "macos",
						capabilities: Array.isArray(body.capabilities)
							? (body.capabilities.map(String) as never)
							: [],
						...(typeof body.version === "string"
							? { version: body.version }
							: {}),
						...(Number.isInteger(body.idleSeconds)
			...
			const nodePoll = url.pathname.match(
				/^\/v1\/nodes\/([A-Za-z0-9._-]{1,128})\/poll$/,
			);
			...
			const nodeResult = url.pathname.match(
				/^\/v1\/nodes\/([A-Za-z0-9._-]{1,128})\/results$/,
			);
			if (request.method === "POST" && nodeResult) {
				...
				this.options.nativeNodes.complete(nodeResult[1]!, {
					...
					...(rawError
						? {
								error: {
									code: stringField(rawError, "code", 100),
									message: stringField(rawError, "message", 1_000),
								},
							}
						: {}),
				});
			...
			const nodeTalk = url.pathname.match(
				/^\/v1\/nodes\/([A-Za-z0-9._-]{1,128})\/talk$/,
			);
			if (request.method === "POST" && nodeTalk) {
				if (!this.options.nativeNodes || !this.options.onNodeTalk) {
					json(response, 404, { error: "Native Talk is not configured." });
					return;
				}
				this.options.remote.assertAuthorized(token, "tasks");
				const body = await readJson(request);
				...
				void this.options
					.onNodeTalk({ nodeId, text })
					.then((result) =>
						this.options.nativeNodes?.enqueueTalk(
							nodeId,
							result.text,
							result.sessionId,
						),
					...
					.catch((error) =>
						this.options.nativeNodes?.enqueueTalk(
							nodeId,
							`Talk failed: ${error instanceof Error ? error.message.slice(0, 500) : "The agent could not respond."}`,
						),
					);
				json(response, 202, { accepted: true });
				return;
			...
			if (url.pathname === "/v1/nodes/voice-wake" && request.method === "GET") {
				...
				this.options.remote.assertAuthorized(token, "read");
				json(response, 200, {
					triggers: this.options.nativeNodes.getVoiceWake(),
				});
				return;
			}
			if (
				url.pathname === "/v1/nodes/voice-wake" &&
				request.method === "POST"
			) {
				if (!this.options.nativeNodes) {
					json(response, 404, { error: "Native nodes are not configured." });
				...
				const body = await readJson(request);
				json(response, 200, {
					triggers: this.options.nativeNodes.setVoiceWake(body.triggers),
				});
				return;
			}
			if (request.method === "POST" && url.pathname === "/v1/mcp") {
				...
				const allowMutatingTools = this.options.remote.hasAuthorizedScope(
					token,
					"tasks",
				);
				const sessionHeader = request.headers["x-kestrel-session-id"];
				if (typeof sessionHeader !== "string" || sessionHeader.length > 200)
					throw new Error("MCP session header is required.");
				...
				if (body.method === "tools/call" && !allowMutatingTools) {
					const name = String(
						(body.params as Record<string, unknown> | undefined)?.name ?? "",
					);
					const tool = this.options.runtime
						.modelTools(sessionHeader)
				...
				const server =
					this.mcpSessions.get(key) ??
					new McpRuntimeServer(this.options.runtime, sessionHeader);
			...
			if (request.method === "GET" && url.pathname === "/v1/sessions") {
				json(response, 200, {
					sessions: this.options.remote.listSessions(token),
				});
				return;
			}
			if (request.method === "GET" && url.pathname === "/v1/jobs") {
				json(response, 200, { jobs: this.options.remote.listJobs(token) });
				return;
			}
			if (request.method === "POST" && url.pathname === "/v1/jobs") {
				json(response, 202, {
					job: this.options.remote.submitJob(
						token,
						jobInput(await readJson(request)),
					),
				});
				return;
			}
			const resume = url.pathname.match(
				/^\/v1\/jobs\/([A-Za-z0-9._-]{1,200})\/resume$/,
			);
			if (request.method === "POST" && resume) {
				json(response, 200, {
					job: await this.options.remote.resumeJob(token, resume[1]!),
				});
				return;
			}
			if (request.method === "GET" && url.pathname === "/v1/events") {
				this.openEvents(token, request, response);
				return;
			}
			...
			const status =
				/token|bearer|scope|trusted proxy|identity|untrusted source|not allowed/i.test(
					message,
				)
					? 401
					: /rate limit/i.test(message)
						? 429
						: /invalid|requires|exceeds|must|field|schedule/i.test(message)
							? 400
							: 500;
			json(response, status, {
				error: status === 500 ? "Remote request failed." : message,
			});
		}
	}
	...
	private openEvents(
		...
		if (this.sse.size >= (this.options.maximumSseClients ?? 8))
			throw new Error("Remote event client limit exceeded.");
		response.writeHead(200, {
