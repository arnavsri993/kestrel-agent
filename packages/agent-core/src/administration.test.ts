...
describe("managed organization policy", () => {
  it("enforces retention, exposes content-free analytics, and verifies provisioned SSO identities", () => {
    ...
    expect(() => store.verifyIdentityToken("bm90LWpzb24.eyJub3QiOiJqc29uIn0.signature")).toThrow("SSO identity token is malformed.");
  ...
  it("recovers when persisted organization members are not an array", () => {
    ...
    expect(store.provisionMember({ externalId: "user-1", email: "user@example.test", displayName: "User", role: "member" })).toMatchObject({ externalId: "user-1" });
