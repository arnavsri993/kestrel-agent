...
function validateTimeout(timeoutMs: number): number {
	if (
		...
		throw new Error(
			"Language server timeout must be a finite positive integer.",
		);
	}
	return timeoutMs;
}
...
export class StdioLanguageServerTransport implements LanguageServerTransport {
	...
	constructor(options: StdioLanguageServerOptions) {
		...
		if (
			...
			throw new Error(
				"Language server command must be an executable regular file.",
			);
		if (
			(options.args ?? []).length > 100 ||
			(options.args ?? []).some(
				(arg) => arg.length > 10_000 || /[\0\r\n]/.test(arg),
			)
		)
			throw new Error("Language server arguments are invalid.");
		if (
			...
			throw new Error("Language server environment is invalid.");
		this.maximumMessageBytes = Math.max(
			1_024,
			Math.min(10_000_000, options.maximumMessageBytes ?? 2_000_000),
		);
		...
		this.child.stderr.on("data", (chunk: Buffer) => {
			...
			if (stderrBytes > 1_000_000) this.child.kill("SIGKILL");
		});
	}

	send(message: LspMessage): void {
		if (this.closed || !this.child.stdin.writable)
			throw new Error("Language server transport is closed.");
		const body = Buffer.from(JSON.stringify(message));
		if (body.byteLength > this.maximumMessageBytes)
			throw new Error("Language server message exceeds the configured limit.");
	...
	private receive(chunk: Buffer): void {
		...
		if (this.buffer.byteLength > this.maximumMessageBytes + 8_192) {
			this.child.kill("SIGKILL");
			return;
		}
		while (true) {
			...
			if (
				...
				length < 2 ||
				length > this.maximumMessageBytes
			) {
				this.child.kill("SIGKILL");
				return;
			}
			...
			try {
				message = JSON.parse(body.toString("utf8")) as LspMessage;
			} catch {
				this.child.kill("SIGKILL");
				return;
			}
...
export async function environmentLanguageServerClient(
	environment: NodeJS.ProcessEnv = process.env,
): Promise<{ client: LanguageServerClient; rootUri: string } | undefined> {
	...
	if (
		...
		throw new Error(
			"KESTREL_LSP_CONFIG must be an owner-only regular file no larger than 1 MB.",
		);
	let parsed: Record<string, unknown>;
	try {
		parsed = JSON.parse(readFileSync(realpathSync(path), "utf8")) as Record<
			string,
			unknown
		>;
	} catch {
	...
	if (
		...
		typeof parsed.rootUri !== "string" ||
		!Array.isArray(parsed.args) ||
		parsed.args.some((arg) => typeof arg !== "string")
	)
		throw new Error("Language server configuration is invalid.");
	const client = new LanguageServerClient(
		new StdioLanguageServerTransport({
...
export class LanguageServerClient {
	...
	async initialize(rootUri: string): Promise<unknown> {
		const result = await this.request("initialize", {
			...
			clientInfo: { name: "Kestrel", version: "0.1.0" },
		});
	...
	async diagnostics(input: {
		...
		const result = new Promise<unknown[]>((resolve, reject) => {
			const timer = setTimeout(() => {
				this.diagnosticsWaiters.set(
					input.uri,
					(this.diagnosticsWaiters.get(input.uri) ?? []).filter(
						(item) => item.timer !== timer,
					),
				);
				reject(new Error("Language server diagnostics timed out."));
			}, this.timeoutMs);
			const waiter = { resolve, reject, timer };
			this.diagnosticsWaiters.set(input.uri, [
				...(this.diagnosticsWaiters.get(input.uri) ?? []),
				waiter,
			]);
		});
		...
		if (priorVersion === undefined)
			...
			await this.notify("textDocument/didChange", {
				textDocument: { uri: input.uri, version },
	...
	async close(): Promise<void> {
		try {
			...
			for (const pending of this.pending.values()) {
				...
				pending.reject(new Error("Language server client closed."));
			}
			this.pending.clear();
			for (const waiters of this.diagnosticsWaiters.values())
				for (const waiter of waiters) {
					clearTimeout(waiter.timer);
					waiter.reject(new Error("Language server client closed."));
				}
			...
			Promise.resolve(
				...
				reject(
					error instanceof Error
						? error
						: new Error("Language server send failed."),
				);
			});
		});
	}
	...
	private receive(message: LspMessage): void {
		if (message.id !== undefined && !message.method) {
			...
			if (message.error)
				pending.reject(
					new Error(
						`Language server error ${message.error.code}: ${message.error.message}`,
					),
				);
...
export function installCodeIntelligenceTools(
	runtime: AgentRuntime,
	client: LanguageServerClient,
	sessionId: string,
): void {
	const descriptor = (name: string, title: string, description: string) => ({
		name,
		title,
		description,
		...
		tags: ["lsp", "code", "untrusted-server"],
	});
	runtime.registerExternalTool({
		descriptor: descriptor(
			"code.diagnostics",
			"Get code diagnostics",
			"Open an in-memory document in a configured language server and return published diagnostics.",
		),
		inputSchema: {
			type: "object",
			properties: {
		...
		execute: async (_context, input) => ({
			...
			trust: "untrusted_language_server",
		}),
	});
	for (const [name, title, method] of [
		["code.definition", "Find definition", "definition"],
		["code.references", "Find references", "references"],
	] as const) {
		runtime.registerExternalTool({
			descriptor: descriptor(
				name,
				title,
				`Query a configured language server for ${method}.`,
			),
			inputSchema: {
				type: "object",
				properties: {
			...
			execute: async (_context, input) => ({
				locations: await client[method](String(input.uri), {
