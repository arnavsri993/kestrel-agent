...
describe("durable memory manager", () => {
  it("captures only explicit memories and keeps inferred user facts proposed until review", () => {
    ...
    expect(manager.captureExplicit("The sky is blue", "message-1").memory).toBeUndefined();
    const captured = manager.captureExplicit("Remember that I keep deployment notes in ops.md", "message-2");
    expect(captured.memory).toMatchObject({ content: "I keep deployment notes in ops.md", userConfirmed: true, inferred: false, sourceIds: ["message-2"] });
    const proposed = manager.captureExplicit("I prefer compact status updates", "message-3").userModelFacts[0]!;
  ...
  it("exposes approval-gated memory mutations and ranked cross-session transcript search", async () => {
    ...
    runtime.appendMessage({ sessionId: other.id, role: "user", content: "Deployed the kestrel service successfully" });
    expect(runtime.searchMessages("deploy kestrel")).toMatchObject([{ sessionId: other.id }]);
    expect(runtime.searchMessages("deploying kestrels")).toMatchObject([{ sessionId: other.id }]);
    ...
    const input = { type: "project", content: "Release checklist", sourceIds: ["message-source"] };
    expect((await runtime.callTool(session.id, "memory.remember", input, { idempotencyKey: "remember-1" })).status).toBe("blocked");
    const stored = await runtime.callTool(session.id, "memory.remember", input, { approvalStatus: "approved", idempotencyKey: "remember-1" });
    expect(stored).toMatchObject({ status: "verified", output: { memory: { content: "Release checklist" } } });
    const proposal = await runtime.callTool(session.id, "memory.user-model-propose", { kind: "relationship", key: "release-reviewer", value: "Sam reviews production releases", sourceIds: ["message-source"], confidence: 0.8 }, { approvalStatus: "approved", idempotencyKey: "propose-1" });
    expect(proposal).toMatchObject({ status: "verified", output: { fact: { status: "proposed", kind: "relationship" } } });
    expect(manager.userModel.promptContext()).toBe("");
    database.close();
  });
