...
import { McpClient, StdioMcpTransport, StreamableHttpMcpTransport, bridgeMcpTools } from "./extensions/mcp";
import { textContent, type ModelContentPart } from "./providers";
...
function modelParts(blocks: ContentBlock[]): ModelContentPart[] {
  ...
  for (const block of blocks) {
    ...
    else if (block.type === "image") parts.push({ type: "image", data: block.data, mediaType: block.mimeType, source: "base64" });
    else if (block.type === "audio") parts.push({ type: "audio", data: block.data, mediaType: block.mimeType, source: "base64" });
    else if (block.type === "resource_link") parts.push(...textContent(`[Resource link: ${block.name} ${block.uri}]`));
...
export function createKestrelAcpAgent(options: KestrelAcpOptions): AgentApp {
  ...
  const mcpClients = new Map<string, Array<{ client: McpClient; tools: string[] }>>();
  let editorToolsRegistered = false;
  const editorPath = (sessionId: string, requested: string, write = false) => {
    const root = realpathSync(options.runtime.getSession(sessionId).workspaceRoot!);
    if (!isAbsolute(requested)) throw new Error("Editor-delegated paths must be absolute.");
    const candidate = write && !existsSync(requested) ? resolve(requested) : realpathSync(requested);
    const parent = write && !existsSync(candidate) ? realpathSync(dirname(candidate)) : candidate;
    if ((candidate !== root && !candidate.startsWith(`${root}${sep}`)) || (parent !== root && !parent.startsWith(`${root}${sep}`))) throw new Error("Editor-delegated path escapes the session workspace.");
    return candidate;
  };
  const installEditorTools = (sessionId: string) => {
    if (!editorToolsRegistered) {
      ...
      options.runtime.registerExternalTool({ descriptor: { name: "editor.fs.write_text", title: "Write text through editor", description: "Write a workspace text file through the connected ACP editor client.", category: "workspace", riskLevel: "sensitive", readOnly: false, requiresWorkspace: true, source: "connector", tags: ["editor", "acp", "filesystem", "write"] }, inputSchema: { type: "object", properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] }, execute: async ({ session }, input) => {
        const editor = editorClients.get(session.id); if (!editor) throw new Error("The ACP editor client is no longer connected."); const content = String(input.content); if (content.length > 1_000_000) throw new Error("Editor write exceeds 1 MB.");
        const path = editorPath(session.id, String(input.path), true); await editor.request("fs/write_text_file", { sessionId: session.id, path, content }); return { path, bytes: Buffer.byteLength(content), delegated: true };
      } });
  ...
  return agent({ name: "Kestrel" })
    .onRequest("initialize", ({ params }) => ({
      ...
      agentCapabilities: {
        ...
        sessionCapabilities: { list: {}, resume: {}, close: {}, additionalDirectories: {} }
      }
    }))
    .onRequest("session/new", async ({ params, client }) => {
      const session = options.runtime.createSession({ title: "ACP editor session", workspaceRoot: realpathSync(params.cwd) });
      editorClients.set(session.id, client); installEditorTools(session.id);
      const connected: Array<{ client: McpClient; tools: string[] }> = [];
      try {
        for (const [index, server] of params.mcpServers.entries()) {
          let transport;
          if ("type" in server && server.type === "http") {
            const unsupported = server.headers.filter((header) => header.name.toLowerCase() !== "authorization"); if (unsupported.length) throw new Error("ACP MCP HTTP handoff supports only the Authorization header.");
            transport = new StreamableHttpMcpTransport(server.url, { ...(server.headers.find((header) => header.name.toLowerCase() === "authorization")?.value ? { authorization: server.headers.find((header) => header.name.toLowerCase() === "authorization")!.value } : {}) });
          } else if (!("type" in server)) {
            const command = realpathSync(server.command); const environment = Object.fromEntries(server.env.map(({ name, value }) => [name, value]));
            transport = new StdioMcpTransport({ command, args: server.args, cwd: session.workspaceRoot!, environment });
          } else throw new Error(`ACP MCP transport ${"type" in server ? server.type : "unknown"} is not supported by the stable bridge.`);
          const mcp = new McpClient(transport); const tools = await bridgeMcpTools(mcp, options.runtime, session.id, `acp-${session.id}-${index}-${server.name}`); connected.push({ client: mcp, tools });
        }
        mcpClients.set(session.id, connected);
      } catch (error) { await Promise.all(connected.map(({ client }) => client.close().catch(() => undefined))); editorClients.delete(session.id); options.runtime.cancelSession(session.id); throw error; }
    ...
    .onRequest("session/close", ({ params }) => {
      ...
      for (const connection of mcpClients.get(params.sessionId) ?? []) { for (const tool of connection.tools) options.runtime.unregisterExternalTool(tool); void connection.client.close(); }
    ...
    .onRequest("session/prompt", async ({ params, client, signal }) => {
      ...
      try {
        let result: AgentLoopResult = await options.loop.run({
          ...
          onTextDelta: (delta) => { void client.notify("session/update", { sessionId: params.sessionId, update: { sessionUpdate: "agent_message_chunk", content: { type: "text", text: delta } } }); }
        });
        while (result.run.status === "waiting_approval" && result.pendingExecution) {
          ...
          await client.notify("session/update", { sessionId: params.sessionId, update: { sessionUpdate: "tool_call", ...toolCall } });
          const permission = await client.request<RequestPermissionResponse>("session/request_permission", {
            sessionId: params.sessionId,
            toolCall,
            options: [
            ...
            _meta: { kestrel: { runId: result.run.id, riskLevel: pending.riskLevel } }
          });
          if (permission.outcome.outcome === "cancelled") return { stopReason: "cancelled", _meta: { kestrel: { status: "waiting_approval", runId: result.run.id } } };
          const approved = permission.outcome.optionId === "allow-once";
          await client.notify("session/update", { sessionId: params.sessionId, update: { sessionUpdate: "tool_call_update", toolCallId, status: approved ? "in_progress" : "failed", rawOutput: approved ? undefined : { error: "Rejected by user" } } });
          ...
          if (approved) await client.notify("session/update", { sessionId: params.sessionId, update: { sessionUpdate: "tool_call_update", toolCallId, status: "completed" } });
        }
        if (result.run.status === "cancelled") return { stopReason: "cancelled" };
        if (result.run.status === "failed") return { stopReason: "refusal" };
        return { stopReason: "end_turn", _meta: { kestrel: { runId: result.run.id } } };
      } finally {
        signal.removeEventListener("abort", abort);
        active.delete(params.sessionId);
      }
    })
    .onNotification("session/cancel", ({ params }) => { active.get(params.sessionId)?.abort(new Error("Cancelled by ACP client.")); });
}
