...
const REMOTE_BACKEND_KINDS = new Set<RemoteBackendKind>([
	"docker",
	"ssh",
	"cluster",
	"serverless",
...
function isRemoteTarget(value: unknown): value is RemoteTarget {
	...
	return (
		...
		REMOTE_BACKEND_KINDS.has(target.kind as RemoteBackendKind) &&
		typeof target.backendId === "string" &&
		Array.isArray(target.allowedCommands) &&
		target.allowedCommands.every((command) => typeof command === "string") &&
	...
	return new Promise((resolve, reject) => {
		if (signal.aborted) {
			reject(
				signal.reason instanceof Error
					? signal.reason
					: new Error("Remote execution cancelled."),
			);
			return;
		}
		...
		const capture =
			(chunks: Buffer[], kind: "stdout" | "stderr") => (chunk: Buffer) => {
				...
				if (stdoutBytes > 1_000_000 || stderrBytes > 1_000_000) {
					child.kill("SIGKILL");
					finish(new Error("Remote adapter output exceeded 1 MB per stream."));
					return;
				}
				chunks.push(chunk);
				onOutput?.(kind, chunk.toString("utf8"));
			};
		...
		child.once("error", (error) =>
			finish(
				new Error(
					`Remote adapter could not start ${executable}: ${error.message}`,
				),
			),
		);
		...
		timeout = setTimeout(() => {
			child.kill("SIGTERM");
			setTimeout(() => child.kill("SIGKILL"), 1_000).unref();
			finish(new Error("Remote execution timed out."));
		}, timeoutMs);
		timeout.unref();
	});
}
...
function commandResult(result: ProcessResult): RemoteExecutionResult {
	return { ...result, remoteExecutionId: `remote-${randomUUID()}` };
}
...
export class DockerCliRemoteBackend implements RemoteExecutionBackend {
	...
	async execute({
		...
		RemoteExecutionBackend["execute"]
	>[0]): Promise<RemoteExecutionResult> {
		if (target.kind !== "docker")
			throw new Error("Docker backend received a non-Docker target.");
		const image = stringConfig(target, "image")!;
		if (!/^[A-Za-z0-9][A-Za-z0-9._/:@-]{0,500}$/.test(image))
		...
		const dockerArgs = [
			...
			"--init",
			"--label",
			`kestrel.execution=${randomUUID()}`,
		];
		const workspaceRoot = stringConfig(target, "workspaceRoot", false);
		if (workspaceRoot) {
			const root = realpathSync(workspaceRoot);
			if (!statSync(root).isDirectory())
				throw new Error("Docker workspace root must be a directory.");
			dockerArgs.push(
				"--mount",
				`type=bind,src=${root},dst=/workspace`,
				"--workdir",
				stringConfig(target, "workdir", false) ?? "/workspace",
			);
		}
		if (target.configuration?.network !== true)
			dockerArgs.push("--network", "none");
		dockerArgs.push(image, command, ...args);
		return commandResult(
			await runBounded("docker", dockerArgs, timeoutMs, signal, onOutput),
		);
	}
}
...
export class SshCliRemoteBackend implements RemoteExecutionBackend {
	...
	>[0]): Promise<RemoteExecutionResult> {
		if (target.kind !== "ssh")
			throw new Error("SSH backend received a non-SSH target.");
		const host = stringConfig(target, "host")!;
		if (!/^[A-Za-z0-9][A-Za-z0-9.-]{0,252}$/.test(host))
			throw new Error("SSH host is invalid.");
		const user = stringConfig(target, "user", false);
		if (user && !/^[A-Za-z_][A-Za-z0-9_-]{0,63}$/.test(user))
		...
		const identityFile = stringConfig(target, "identityFile", false);
		if (identityFile) {
			const identity = realpathSync(identityFile);
			if (!statSync(identity).isFile())
				throw new Error("SSH identity must be a regular file.");
		...
		return commandResult(
			await runBounded("ssh", sshArgs, timeoutMs, signal, onOutput),
		);
	}
}
...
export class KubernetesCliRemoteBackend implements RemoteExecutionBackend {
	...
	>[0]): Promise<RemoteExecutionResult> {
		if (target.kind !== "cluster")
			throw new Error("Kubernetes backend received a non-cluster target.");
		const image = stringConfig(target, "image")!;
		if (!/^[A-Za-z0-9][A-Za-z0-9._/:@-]{0,500}$/.test(image))
			throw new Error("Kubernetes image reference is invalid.");
		const pod = `kestrel-${randomUUID().slice(0, 12)}`;
		const prefix: string[] = [];
		const context = stringConfig(target, "context", false);
		const namespace = stringConfig(target, "namespace", false);
		if (context) prefix.push("--context", context);
		if (namespace) prefix.push("--namespace", namespace);
		const runArgs = [
			...prefix,
			"run",
			pod,
		...
		try {
			return commandResult(
				await runBounded("kubectl", runArgs, timeoutMs, signal, onOutput),
			);
		} finally {
			if (signal.aborted)
				void runBounded(
...
export class ServerlessHttpRemoteBackend implements RemoteExecutionBackend {
	...
	>[0]): Promise<RemoteExecutionResult> {
		if (target.kind !== "serverless")
			throw new Error("Serverless backend received a non-serverless target.");
		let endpoint: URL;
		try {
			endpoint = new URL(stringConfig(target, "endpoint")!);
		} catch {
			throw new Error("Serverless endpoint must be a valid URL.");
		}
		if (
			...
			throw new Error("Serverless endpoint must be credential-free HTTPS.");
		...
		const response = await this.fetcher(endpoint, {
			...
			body: JSON.stringify({
				executionId: `kestrel-${randomUUID()}`,
				command,
				args,
				timeoutMs,
			}),
		...
		const bytes = await readBoundedResponseBytes(
			response,
			36_000_000,
			"Serverless response exceeds 36 MB.",
		);
		const text = new TextDecoder().decode(bytes);
		if (!response.ok)
			throw new Error(
				`Serverless backend returned HTTP ${response.status}: ${text.slice(0, 2_000)}`,
			);
		...
		if (
			...
			throw new Error("Serverless backend response is invalid.");
		if (body.stdout) onOutput?.("stdout", body.stdout.slice(0, 16_000));
		if (body.stderr) onOutput?.("stderr", body.stderr.slice(0, 16_000));
		const artifacts = Array.isArray(body.artifacts)
			? (body.artifacts as RemoteArtifact[])
			: undefined;
		return {
			...
			stderr: body.stderr,
			remoteExecutionId:
				typeof body.remoteExecutionId === "string"
					? body.remoteExecutionId.slice(0, 200)
					: `remote-${randomUUID()}`,
			...(artifacts ? { artifacts } : {}),
		};
	}
...
export class RemoteBackendManager {
	...
	constructor(
		...
		backends: RemoteExecutionBackend[],
		artifactRoot?: string,
	) {
	...
	setTargets(targets: RemoteTarget[]): void {
		if (
			targets.length > 100 ||
			Buffer.byteLength(JSON.stringify(targets), "utf8") > 1_000_000
		)
			throw new Error("Remote target configuration exceeds limits.");
		if (new Set(targets.map((target) => target.id)).size !== targets.length)
			throw new Error("Remote target IDs must be unique.");
		for (const target of targets) {
			if (!this.backends.has(target.backendId))
				throw new Error(
					`Remote backend ${target.backendId} is not configured.`,
				);
			if (
				target.allowedCommands.length === 0 ||
				target.allowedCommands.length > 200 ||
				target.allowedCommands.some(
					(command) => !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(command),
				)
			)
				throw new Error(
					`Remote target ${target.id} needs a valid explicit command allowlist.`,
				);
		}
	...
	async execute(
		...
		const target = this.listTargets().find(
			(candidate) => candidate.id === targetId,
		);
		if (!target || !target.enabled)
			throw new Error("Remote target is not enabled.");
		if (
			!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(command) ||
			!target.allowedCommands.includes(command)
		)
			throw new Error("Remote command is outside the target allowlist.");
		if (args.length > 200 || args.some((arg) => arg.length > 10_000))
			throw new Error("Remote command arguments exceed limits.");
		...
		const result = await backend.execute({
			...
			signal: AbortSignal.any([signal, AbortSignal.timeout(boundedTimeout)]),
			...(onOutput ? { onOutput } : {}),
		});
		...
		return {
			...
			attestedBy: backend.id,
		};
	}
	private async storeArtifacts(artifacts: RemoteArtifact[]): Promise<
		Array<{
			filename: string;
			mediaType: string;
			bytes: number;
	...
	> {
		if (artifacts.length > 20)
			throw new Error("Remote execution returned more than 20 artifacts.");
		if (artifacts.length && !this.artifactRoot)
			throw new Error("Remote artifact storage is not configured.");
		const output: Array<{
			filename: string;
			mediaType: string;
			bytes: number;
		...
		for (const artifact of artifacts) {
			...
			if (
				...
				throw new Error("Remote artifact filename is invalid.");
			if (
				typeof artifact.mediaType !== "string" ||
				artifact.mediaType.length > 200 ||
				!artifact.sha256?.match(/^[a-f0-9]{64}$/)
			)
				throw new Error("Remote artifact metadata is invalid.");
			const data = Buffer.from(artifact.dataBase64, "base64");
			if (
				!data.byteLength ||
				data.toString("base64").replace(/=+$/, "") !==
					artifact.dataBase64.replace(/=+$/, "")
			)
				throw new Error("Remote artifact data is invalid.");
			total += data.byteLength;
			if (data.byteLength > 25_000_000 || total > 100_000_000)
				throw new Error("Remote artifacts exceed size limits.");
			const sha256 = createHash("sha256").update(data).digest("hex");
			if (sha256 !== artifact.sha256)
				throw new Error("Remote artifact hash verification failed.");
			const path = join(this.artifactRoot!, `${randomUUID()}-${filename}`);
...
export interface RemoteExecutionConfiguration {
	backends: RemoteExecutionBackend[];
	targets: RemoteTarget[];
	artifactRoot?: string;
}
export function environmentRemoteExecutionConfiguration(
	environment: NodeJS.ProcessEnv = process.env,
	artifactRoot?: string,
): RemoteExecutionConfiguration | undefined {
	if (!environment.KESTREL_REMOTE_TARGETS) return undefined;
	let targets: RemoteTarget[];
	try {
		targets = JSON.parse(environment.KESTREL_REMOTE_TARGETS) as RemoteTarget[];
	} catch {
		throw new Error("KESTREL_REMOTE_TARGETS must be valid JSON.");
	}
	if (!Array.isArray(targets) || targets.length > 100)
		throw new Error("KESTREL_REMOTE_TARGETS must contain at most 100 targets.");
	const backends: RemoteExecutionBackend[] = [
		new DockerCliRemoteBackend(),
		new SshCliRemoteBackend(),
		new KubernetesCliRemoteBackend(),
		new ServerlessHttpRemoteBackend(),
	];
	return { backends, targets, ...(artifactRoot ? { artifactRoot } : {}) };
}

export function installRemoteExecutionTool(
	runtime: AgentRuntime,
	manager: RemoteBackendManager,
	sessionId: string,
): void {
	runtime.registerExternalTool({
		descriptor: {
			name: "remote.execute",
			title: "Execute on remote target",
			description:
				"Run an argv-only allowlisted command through an explicitly enabled Docker, SSH, cluster, or serverless backend.",
			category: "execution",
			riskLevel: "high_consequence",
			readOnly: false,
			requiresWorkspace: false,
			source: "connector",
			tags: ["remote", "docker", "ssh", "cluster", "serverless"],
		},
		inputSchema: {
			type: "object",
			properties: {
		...
		execute: ({ signal, progress }, input) =>
			manager.execute(
				...
				signal,
				(stream, chunk) => progress({ stream, chunk: chunk.slice(0, 16_000) }),
			),
	});
...
export interface RemoteTrustedIdentity {
	kind: "trusted-proxy";
	identity: string;
	scopes: RemoteScope[];
}
...
function isPairingRecord(value: unknown): value is PairingRecord {
	...
	return (
		...
		pairing.label.length > 0 &&
		pairing.label.length <= MAX_REMOTE_PAIRING_LABEL_LENGTH &&
		typeof pairing.codeHash === "string" &&
		/^[a-f0-9]{64}$/.test(pairing.codeHash) &&
		Array.isArray(pairing.scopes) &&
		pairing.scopes.length > 0 &&
		pairing.scopes.every(isRemoteScope) &&
		typeof pairing.expiresAt === "string" &&
		Number.isFinite(Date.parse(pairing.expiresAt)) &&
		isInteger(pairing.attempts) &&
		pairing.attempts >= 0 &&
		pairing.attempts <= 5 &&
...
export class RemoteControl {
	...
	beginPairing(
		...
		if (
			!label.trim() ||
			label.length > MAX_REMOTE_PAIRING_LABEL_LENGTH ||
			scopes.length === 0
		)
			throw new Error("Remote pairing requires a valid label and scopes.");
		if (scopes.some((scope) => !["read", "tasks", "approve"].includes(scope)))
			throw new Error("Remote pairing scope is invalid.");
		const pairings = this.prunePairings();
		if (pairings.length >= MAX_REMOTE_PAIRINGS)
			throw new Error("Remote pairing limit reached.");
		...
		const pairing: PairingRecord = {
			...
			expiresAt: new Date(
				this.now().getTime() + Math.max(30_000, Math.min(lifetimeMs, 600_000)),
			).toISOString(),
			attempts: 0,
			status: "pending",
		};
	...
	completePairing(
		pairingId: string,
		code: string,
	): { deviceId: string; token: string; scopes: RemoteScope[] } {
		const pairings = this.prunePairings();
		...
		if (
			...
			throw new Error("Remote pairing is invalid or expired.");
		...
		if (!timingSafeEqual(supplied, expected)) {
			pairings[index] = {
				...
				...(pairing.attempts + 1 >= 5 ? { status: "locked" as const } : {}),
			};
	...
	listSessions(token: RemoteCredential): RemoteSessionSummary[] {
		...
		return this.runtime
			.listSessions()
			.map(({ id, title, status, parentSessionId, updatedAt }) => ({
				id,
				title,
				status,
	...
	): Array<Omit<ScheduledAgentJob, "prompt" | "instructions">> {
		this.authorize(token, "read");
		return this.orchestrator
			.listJobs()
			.map(({ prompt: _prompt, instructions: _instructions, ...job }) => job);
	}
	submitJob(
		token: RemoteCredential,
		input: Omit<ScheduledAgentJob, "id" | "status" | "createdAt" | "updatedAt">,
	): Omit<ScheduledAgentJob, "prompt" | "instructions"> {
		this.authorize(token, "tasks");
		const {
			prompt: _prompt,
			instructions: _instructions,
			...job
		} = this.orchestrator.schedule(input);
		return job;
	}
	async resumeJob(
		token: RemoteCredential,
		jobId: string,
	): Promise<Omit<ScheduledAgentJob, "prompt" | "instructions">> {
	...
	assertAuthorized(token: RemoteCredential, scope: RemoteScope): void {
	...
	private authorize(
		token: RemoteCredential,
		scope: RemoteScope,
	): DeviceRecord | RemoteTrustedIdentity {
		if (typeof token !== "string") {
			if (!token.scopes.includes(scope))
				throw new Error("Trusted proxy identity lacks the required scope.");
			return token;
		}
		const hash = digest(token);
		const device = this.devices().find(
			(candidate) => candidate.tokenHash === hash && !candidate.revokedAt,
		);
		if (!device || !device.scopes.includes(scope))
			throw new Error("Remote token is invalid or lacks the required scope.");
		return device;
	}
	private prunePairings(): PairingRecord[] {
		...
		if (active.length !== pairings.length)
			this.database.setPrivateState(this.pairingKey, active);
		return active;
	}
