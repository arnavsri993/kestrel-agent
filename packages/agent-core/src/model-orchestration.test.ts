...
describe("adaptive model orchestration", () => {
  ...
  it("rolls back outcome learning when persistence fails", () => {
    ...
    item.database.db.exec(`
      CREATE TRIGGER reject_model_outcome_persistence
      BEFORE UPDATE ON private_runtime_state
      WHEN NEW.key = 'orchestration.model-registry.v1'
      BEGIN
        SELECT RAISE(ABORT, 'forced model outcome persistence failure');
      END
    `);

    expect(() => item.registry.recordOutcome({ modelId: "fixture:base", capabilities: { coding: 1 }, succeeded: false, validationPassed: false, observedAt: "2026-07-29T12:00:00.000Z" })).toThrow("forced model outcome persistence failure");
