...
describe("reviewable user model", () => {
	it("keeps inferences out of context until confirmed and preserves provenance", () => {
		...
		const proposed = store.propose({
			kind: "preference",
			key: "response_style",
			value: "Lead with the outcome",
			sourceIds: ["message-1"],
			confidence: 0.9,
			sensitivity: "normal",
		});
		expect(store.promptContext()).toBe("");
		expect(store.review(proposed.id, "confirm").sourceIds).toEqual([
			"message-1",
		]);
		...
		const privateFact = store.propose({
			kind: "profile",
			key: "private_note",
			value: "secret detail",
			sourceIds: ["message-3"],
		...
		expect(store.promptContext({ includeSensitive: true })).toContain(
			"secret detail",
		);
		const ciphertext = database.db
			...
			.get("memory.user-model") as { value_ciphertext: string };
