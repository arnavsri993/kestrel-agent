...
describe("SandboxedCommandRunner", () => {
	...
	it("throws error if platform is not darwin", () => {
		...
		expect(() => runner.start(defaultInput, { onProgress: vi.fn() })).toThrow(
			"implemented only for macOS",
		);
	});
	...
	it.each([
		...
		0,
		1.5,
	])("rejects invalid timeout %s before spawning", (timeoutMs) => {
		vi.mocked(fs.accessSync).mockImplementation(() => {});

		expect(() =>
			runner.start({ ...defaultInput, timeoutMs }, { onProgress: vi.fn() }),
	...
	it("throws error if command is not in allowlist", () => {
		...
		expect(() => runner.start(input, { onProgress: vi.fn() })).toThrow(
			"not in the Kestrel executable allowlist",
		);
	});
	...
	it("throws error if allowed command is not found in PATH", () => {
		...
		expect(() => runner.start(defaultInput, { onProgress: vi.fn() })).toThrow(
			"Allowed command ls is not installed",
		);
	});
	...
	it("cancels execution if output exceeds 1MB", async () => {
		...
		await expect(handle.completion).rejects.toThrow(
			"exceeded the 1 MB safety limit",
		);
	});
	...
	it("handles interactive mode correctly", async () => {
		...
		const child = mockSpawn();
		const handle = runner.start(defaultInput, {
			interactive: true,
			onProgress: vi.fn(),
		});
