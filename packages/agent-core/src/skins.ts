...
const workstrand: SkinDefinition = {
	id: "workstrand",
	name: "Kestrel",
	description: "Mac-native graphite, aluminum text, and a quiet grayscale signal.",
	mode: "dark",
	builtin: true,
	colors: {
		canvas: "#17181b",
	...
	terminal: {
		...
		error: 245,
		promptSymbol: "›",
		responseLabel: "Kestrel",
		toolPrefix: "┊",
		thinkingVerbs: ["working", "reviewing", "verifying"],
	},
};
...
const daylight: SkinDefinition = {
	...
	name: "Daylight",
	description:
		"A bright paper workspace with cool graphite text and russet focus.",
	mode: "light",
	builtin: true,
	colors: {
		canvas: "#f5f2ea",
	...
	terminal: {
		...
		responseLabel: "Kestrel",
		toolPrefix: "│",
		thinkingVerbs: ["working", "reading", "checking"],
	},
};
...
const mono: SkinDefinition = {
	...
	name: "Mono",
	description:
		"Quiet grayscale for recording, focus, and low-distraction work.",
	mode: "dark",
	builtin: true,
	colors: {
		canvas: "#202020",
	...
	terminal: {
		...
		responseLabel: "Kestrel",
		toolPrefix: "|",
		thinkingVerbs: ["working", "reviewing", "checking"],
	},
};
...
const slate: SkinDefinition = {
	...
	name: "Slate",
	description:
		"Cool blue-gray surfaces with a clear sky-blue interaction signal.",
	mode: "dark",
	builtin: true,
	colors: {
		canvas: "#1e252d",
	...
	terminal: {
		...
		responseLabel: "Kestrel",
		toolPrefix: "│",
		thinkingVerbs: ["tracing", "reviewing", "verifying"],
	},
};
...
export const BUILTIN_SKINS: readonly SkinDefinition[] = [
	...
	mono,
	slate,
].map((skin) => SkinDefinitionSchema.parse(skin));
...
function validateContrast(skin: SkinDefinition): void {
	const checks: Array<[string, string, string, number]> = [
		...
		[
			...
			skin.colors.solidText,
			skin.colors.solidHover,
			4.5,
		],
	...
	for (const [label, foreground, background, minimum] of checks) {
		if (contrast(foreground, background) < minimum)
			throw new Error(`Skin ${label} contrast must be at least ${minimum}:1.`);
	}
}
...
export class SkinManager {
	...
	constructor(private readonly database: KestrelDatabase) {
		...
		this.selectedId =
			database.getPrivateState<string>(this.selectedKey) ?? "workstrand";
		if (!this.all().some((skin) => skin.id === this.selectedId))
			this.selectedId = "workstrand";
	}
	...
	select(id: string): SkinStatus {
		if (!this.all().some((skin) => skin.id === id))
			throw new Error(`Skin ${id} is not installed.`);
		this.selectedId = id;
		this.persist();
		return this.status();
	}
	...
	import(source: string): SkinStatus {
		if (Buffer.byteLength(source) > 65_536)
			throw new Error("Skin document exceeds 64 KB.");
		let raw: unknown;
		try {
			raw = JSON.parse(source);
		} catch {
			throw new Error("Skin document must be valid JSON.");
		}
		const input = SkinImportSchema.parse(raw);
		if (BUILTIN_SKINS.some((skin) => skin.id === input.id))
			throw new Error("A custom skin cannot replace a built-in skin.");
		...
		if (!exists && this.custom.length >= 20)
			throw new Error("At most 20 custom skins can be installed.");
		this.custom = [
			...this.custom.filter((candidate) => candidate.id !== skin.id),
			skin,
		];
	...
	remove(id: string): SkinStatus {
		if (BUILTIN_SKINS.some((skin) => skin.id === id))
			throw new Error("Built-in skins cannot be removed.");
		if (!this.custom.some((skin) => skin.id === id))
			throw new Error(`Custom skin ${id} is not installed.`);
