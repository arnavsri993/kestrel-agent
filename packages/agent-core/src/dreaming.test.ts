...
describe("review-gated memory dreaming", () => {
  it("keeps preview non-mutating, excludes restricted memory, and promotes only after review", () => {
    ...
    expect(database.listMemories().find((item) => item.id === eligible.id)?.userConfirmed).toBe(false);

    const applied = dreaming.run(false);
    expect(applied.candidates[0]).toMatchObject({ memoryId: eligible.id, status: "review" });
    expect(applied.diary[0]).toMatchObject({ preview: false, lightCandidates: 1, deepCandidates: 1 });
