...
describe("privacy-safe external observability", () => {
	...
	it("renders bounded Prometheus text without content, paths, session IDs, or secret values", () => {
		...
		runtime.appendMessage({
			...
			role: "user",
			content: "private prompt with sk-do-not-export",
		});
	...
	it("pushes real OTLP/HTTP protobuf metrics and traces with an encrypted custom header and no raw content", async () => {
		...
		runtime.appendMessage({
			sessionId: session.id,
			role: "user",
			content: "never export this prompt sk-private",
		});
		database.saveAgentRun({
		...
		expect(
			received.every(
				(request) => request.authorization === "Bearer collector-secret",
			),
		).toBe(true);
		...
		for (const secret of [
			"never export this title",
			"never export this prompt",
			"sk-private",
			session.id,
			"audit-raw-id-must-not-export",
			"collector-secret",
		]) {
	...
	it("releases exported audit IDs after database retention removes them", async () => {
		...
		const internals = manager as unknown as {
			tracerProvider: {
				getTracer(): { startSpan(_name: string): { end(): void } };
