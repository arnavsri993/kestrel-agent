...
<body><header><div><span class="eyebrow">Kestrel Remote</span><h1>Supervise the work.</h1></div><span id="status" class="status">Not paired</span></header><main>
<section id="pairing" class="panel"><h2>Pair this device</h2><p>Enter the one-time pairing details shown by the trusted Kestrel host.</p><form id="pair-form"><label>Pairing ID<input id="pairing-id" required autocomplete="off"></label><label>One-time code<input id="pairing-code" required autocomplete="one-time-code"></label><button>Pair securely</button></form><p id="pair-error" class="error" role="alert"></p></section>
<div id="workspace" hidden><section class="panel"><div class="section-head"><div><span class="eyebrow">Live tasks</span><h2>Review queue</h2></div><button id="refresh" class="secondary">Refresh</button></div><div id="jobs" class="cards" aria-live="polite"></div></section>
<section class="panel"><h2>Start a task</h2><form id="task-form"><label>Session<select id="session" required></select></label><label>Title<input id="title" maxlength="200" required></label><label>Prompt<textarea id="prompt" maxlength="1000000" required></textarea></label><div class="grid"><label>Model<input id="model" value="auto" required></label><label>Providers<input id="providers" value="auto" required aria-describedby="providers-help"><small id="providers-help">Comma separated</small></label></div><button>Queue task</button></form><p id="task-error" class="error" role="alert"></p></section>
<section class="panel"><div class="section-head"><div><span class="eyebrow">Presence</span><h2>Recent activity</h2></div><button id="forget" class="ghost">Forget device</button></div><ol id="events" class="events" aria-live="polite"></ol></section></div>
</main><script src="/app/app.js" defer></script></body></html>`;

const css = `:root{color-scheme:dark;--bg:#0d100d;--panel:#151a14;--line:#2a3327;--text:#f2f3ea;--muted:#9ca594;--accent:#c9ff52;--danger:#ff8b7d;font:16px/1.5 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 80% -10%,#263522 0,transparent 35rem),var(--bg);color:var(--text);min-height:100vh}header,main{width:min(920px,calc(100% - 2rem));margin:auto}header{display:flex;align-items:end;justify-content:space-between;padding:3rem 0 1.5rem;gap:1rem}h1,h2,p{margin-top:0}h1{font-size:clamp(2.2rem,7vw,4.5rem);line-height:.95;letter-spacing:-.055em;margin-bottom:0;max-width:12ch}h2{font-size:1.35rem;letter-spacing:-.02em}.eyebrow{display:block;color:var(--accent);font:700 .72rem/1.2 ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase;letter-spacing:.14em;margin-bottom:.7rem}.status{border:1px solid var(--line);border-radius:999px;padding:.45rem .75rem;color:var(--muted);font-size:.82rem}.status.live{border-color:#526d35;color:var(--accent)}main{display:grid;gap:1rem;padding-bottom:5rem}.panel{background:color-mix(in srgb,var(--panel) 92%,transparent);border:1px solid var(--line);border-radius:18px;padding:clamp(1rem,4vw,2rem);box-shadow:0 18px 60px #0004}.section-head{display:flex;justify-content:space-between;align-items:start;gap:1rem}form{display:grid;gap:1rem}label{display:grid;gap:.4rem;color:var(--muted);font-size:.84rem;font-weight:650}input,textarea,select,button{font:inherit}input,textarea,select{width:100%;border:1px solid var(--line);border-radius:10px;background:#0e120e;color:var(--text);padding:.78rem .85rem;outline:none}textarea{min-height:9rem;resize:vertical}input:focus,textarea:focus,select:focus{border-color:var(--accent);box-shadow:0 0 0 3px #c9ff5225}button{border:0;border-radius:10px;background:var(--accent);color:#11160d;font-weight:800;padding:.8rem 1rem;cursor:pointer}button:disabled{opacity:.45;cursor:not-allowed}.secondary,.ghost{background:#252d22;color:var(--text)}.ghost{background:transparent;border:1px solid var(--line)}.grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.cards{display:grid;gap:.75rem}.card{border-top:1px solid var(--line);padding:1rem 0;display:grid;grid-template-columns:1fr auto;gap:1rem}.card h3{margin:.1rem 0;font-size:1rem}.meta,small{color:var(--muted);font-size:.78rem}.pill{display:inline-block;border:1px solid var(--line);border-radius:999px;padding:.18rem .5rem}.events{list-style:none;padding:0;margin:0;display:grid;gap:.6rem}.events li{border-left:2px solid var(--line);padding-left:.8rem;color:var(--muted);font:400 .78rem/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.error{color:var(--danger);min-height:1.5rem;margin:.5rem 0 0}@media(max-width:620px){header{padding-top:max(1.5rem,env(safe-area-inset-top));align-items:start;flex-direction:column}.grid{grid-template-columns:1fr}.panel{border-radius:14px}.card{grid-template-columns:1fr}.card button{width:100%}}@media(prefers-reduced-motion:no-preference){.panel{animation:rise .35s ease both}@keyframes rise{from{opacity:0;transform:translateY(8px)}}}`;
