...
describe("stable ACP v1 editor bridge", () => {
	it("negotiates the official SDK, creates/lists sessions, streams prompts, and closes", async () => {
		...
		const provider: ModelProvider = {
			...
			complete: async (request, call) => {
				call?.onEvent?.({ type: "text_delta", delta: "Editor result" });
				return {
					providerId: "fake",
					model: request.model,
					text: "Editor result",
		...
		const app = createKestrelAcpAgent({
			runtime,
			loop: new AgentLoop(database, runtime, new ProviderPool([provider])),
			model: "fake",
			providerIds: ["fake"],
		});
		const updates: string[] = [];
		const editor = client({ name: "Test editor" }).onNotification(
			"session/update",
			({ params }) => {
				if (
		...
		await editor.connectWith(app, async (acp) => {
			...
			expect(await acp.request("session/list", { cwd: root })).toMatchObject({
				sessions: [{ sessionId: session.sessionId, cwd: realpathSync(root) }],
			});
			expect(
	...
	it("requests native ACP permission and resumes the same run after approval", async () => {
		...
		await editor.connectWith(app, async (acp) => {
			await acp.request("initialize", {
				protocolVersion: PROTOCOL_VERSION,
				clientInfo: { name: "Permission editor", version: "1" },
			});
			const session = await acp.request("session/new", {
				cwd: root,
				mcpServers: [],
			...
			expect(
				await acp.request("session/prompt", {
					sessionId: session.sessionId,
					prompt: [{ type: "text", text: "Delete remove.txt" }],
				}),
			).toMatchObject({ stopReason: "end_turn" });
		});
	...
	it("delegates contained filesystem and terminal operations to the connected editor", async () => {
		const root = mkdtempSync(join(tmpdir(), "kestrel-acp-delegation-"));
		directories.push(root);
		const path = join(root, "editor.txt");
		writeFileSync(path, "editor-owned content\n");
		const database = new KestrelDatabase(":memory:", createEncryptionKey());
		const runtime = new AgentRuntime(database, [root]);
		const provider: ModelProvider = {
			id: "fake",
		...
		const editor = client({ name: "Delegating editor" })
			...
			.onRequest("fs/write_text_file", ({ params }) => {
				written.push(`${params.path}:${params.content}`);
				return {};
			})
		...
		await editor.connectWith(app, async (acp) => {
			await acp.request("initialize", {
				protocolVersion: PROTOCOL_VERSION,
				clientInfo: { name: "Delegating editor", version: "1" },
				clientCapabilities: {
			...
			expect(
				await runtime.callTool(session.sessionId, "editor.fs.read_text", {
					path,
				}),
			).toMatchObject({
				status: "verified",
				output: { content: "editor-owned content\n" },
			});
			expect(
				await runtime.callTool(
					...
					{ approvalStatus: "approved", idempotencyKey: "editor-write" },
				),
			).toMatchObject({ status: "verified", output: { delegated: true } });
			expect(
				await runtime.callTool(
					session.sessionId,
					"editor.terminal.run",
					{ command: "git", args: ["status"], cwd: root },
					{ approvalStatus: "approved", idempotencyKey: "editor-terminal" },
				),
			).toMatchObject({
				status: "verified",
				output: {
			...
			expect(written).toEqual([`${realpathSync(path)}:replacement`]);
			expect(terminalRequests).toEqual(["git status"]);
			expect(
				await runtime.callTool(session.sessionId, "editor.fs.read_text", {
					path: "/etc/hosts",
				}),
				...
				status: "failed",
				error: "Editor-delegated path escapes the session workspace.",
			});
			await acp.request("session/close", { sessionId: session.sessionId });
		});
		database.close();
	...
	it("hands ACP-provided stdio MCP servers into the session tool catalog", async () => {
		const root = mkdtempSync(join(tmpdir(), "kestrel-acp-mcp-"));
		directories.push(root);
		const serverPath = join(root, "mcp-fixture.mjs");
		writeFileSync(
			serverPath,
		...
		const editor = client({ name: "MCP editor" });
		await editor.connectWith(app, async (acp) => {
			await acp.request("initialize", {
				protocolVersion: PROTOCOL_VERSION,
				clientInfo: { name: "MCP editor", version: "1" },
			});
			const session = await acp.request("session/new", {
				cwd: root,
				mcpServers: [
			...
			const tool = runtime
				.discoverTools(session.sessionId, "echo")
				.find((candidate) => candidate.source === "mcp");
			expect(tool?.name).toContain(".echo");
			expect(
				await runtime.callTool(
					session.sessionId,
					tool!.name,
					{ text: "hello" },
					{ approvalStatus: "approved", idempotencyKey: "mcp-echo" },
				),
			).toMatchObject({
				status: "verified",
				output: { content: [{ type: "text", text: "mcp:hello" }] },
			});
			await acp.request("session/close", { sessionId: session.sessionId });
		});
		database.close();
