...
export class PresenceManager {
  ...
  beacon(input: PresenceBeacon): PresenceEntry {
    if (!/^[A-Za-z0-9._-]{1,128}$/.test(input.instanceId)) throw new Error("Presence instance ID is invalid.");
    if (!["ui", "webchat", "node", "test"].includes(input.mode)) throw new Error("Presence mode is invalid.");
    if (input.version && input.version.length > 100) throw new Error("Presence version is invalid.");
    if (input.reason && input.reason.length > 200) throw new Error("Presence reason is invalid.");
    this.prune();
    ...
    if (this.entries.size > MAX_ENTRIES) {
      const oldest = [...this.entries.values()].sort((left, right) => left.lastSeenAt.localeCompare(right.lastSeenAt))[0];
  ...
  list(): PresenceEntry[] {
    ...
    return [...this.entries.values()]
      .map((entry) => ({ ...entry, status: now - new Date(entry.lastSeenAt).getTime() >= IDLE_AFTER_MS ? "idle" as const : "active" as const }))
