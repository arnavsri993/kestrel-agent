...
describe("custom personality bounds", () => {
	it("limits custom personality registrations while retaining built-ins", () => {
		...
		const personality = (index: number) => ({
			id: `custom-${index}`,
			name: `Custom ${index}`,
			description: "A bounded custom personality.",
			instructions: "Keep the task within scope.",
			memoryScope: "shared" as const,
		});
		for (let index = 0; index < 100; index += 1)
			registry.register(personality(index));

		expect(registry.list()).toHaveLength(103);
		expect(() => registry.register(personality(100))).toThrow(
			"100 custom personalities",
		);
...
describe("teacher scheduling vertical slice", () => {
	it("uses calendar evidence to recommend Monday and waits for approval", () => {
		...
		expect(
			snapshot.approvals[0]?.evidence.some((item) =>
				item.value.includes("Swim"),
			),
		).toBe(true);
		...
		expect(snapshot.modelRouting.currentDecision).toMatchObject({
			model: "local-rules",
	...
	it("executes once after approval, verifies providers, and records memory", () => {
		...
		expect(
			second.memories.some((item) => item.id === "memory-test-date-decision"),
		).toBe(true);
		expect(
			second.activity.filter((item) => item.id === "activity-email-sent"),
		).toHaveLength(1);
		core.close();
	});
...
describe("core agent request path", () => {
	it("turns model and provider auto-selection into an audited routed run", async () => {
		...
		const response = await core.handle({
			...
			message: "Review this repository architecture",
			model: "auto",
			providerIds: ["auto"],
		});
	...
	it("routes automatic image work only to providers with image capability", async () => {
		...
		const makeProvider = (id: string, images: boolean): ModelProvider => ({
			...
			...(images
				? {
						profileHints: {
							capabilities: { image_understanding: 0.98, reliability: 0.95 },
						},
					}
				: {}),
			complete: async (request) => {
		...
		const core = new AgentCore({
			...
			workspaceRoots: [root],
			modelProviders: [
				makeProvider("text", false),
				makeProvider("vision", true),
			],
			now: () => "2026-07-22T15:00:00.000Z",
		});
		const session = core.runtime.ensureMainSession();
		...
		expect(response).toMatchObject({
			ok: true,
			run: { model: "vision-model", providerIds: ["vision"] },
		});
			...
			jobs: [
				{
					title: "Morning review",
					status: "pending",
					schedule: { kind: "once", nextRunAt: "2026-07-23T09:00:00.000Z" },
				},
			],
		});
		expect(await core.handle({ type: "orchestration-list" })).toMatchObject({
			ok: true,
			jobs: [{ title: "Morning review" }],
		});
		await core.close();
	});
	...
	it("exposes explicit desktop memory correction and user-model review controls", async () => {
		const { core } = createCore();
		const remembered = await core.handle({
			type: "memory-remember",
			memoryType: "project",
			content: "Deploy from ops.md",
			sensitivity: "personal",
			sourceId: "desktop-user",
		});
		expect(remembered).toMatchObject({
			ok: true,
			memories: [{ content: "Deploy from ops.md", userConfirmed: true }],
		});
		if (!remembered.ok || !remembered.memories?.[0])
			throw new Error("Memory was not returned.");
		expect(
			await core.handle({
				type: "memory-correct",
				id: remembered.memories[0].id,
				content: "Deploy from RELEASE.md",
				memoryType: "project",
			...
			ok: true,
			memories: [
				{ content: "Deploy from RELEASE.md", sourceType: "user-correction" },
			],
		});
		const proposed = core.userModel.propose({
			kind: "boundary",
			key: "external-messages",
			value: "Always ask first",
			sourceIds: ["desktop-user"],
			confidence: 1,
			sensitivity: "normal",
		});
		expect(await core.handle({ type: "memory-user-model-list" })).toMatchObject(
			{ ok: true, userModelFacts: [{ id: proposed.id, status: "proposed" }] },
		);
		expect(
			await core.handle({
		...
		).toMatchObject({ ok: true, userModelFacts: [{ status: "confirmed" }] });
	...
	it("runs a configured provider through the durable agent loop", async () => {
		...
		expect(response).toMatchObject({
			...
			messages: [
				{
					role: "assistant",
					content: "The configured provider path is working.",
				},
			],
		});
		expect(
		...
		const policyResponse = await core.handle({
			type: "runtime-get-usage-policy",
		});
		expect(policyResponse).toMatchObject({
			ok: true,
			usagePolicy: { dailyBudgetUsd: 25, maximumConcurrentCalls: 4 },
		});
		if (!policyResponse.ok || !policyResponse.usagePolicy)
			throw new Error("Usage policy missing");
		expect(
			await core.handle({
				type: "runtime-set-usage-policy",
				policy: { ...policyResponse.usagePolicy, dailyBudgetUsd: 12 },
			}),
		).toMatchObject({ ok: true, usagePolicy: { dailyBudgetUsd: 12 } });
		core.close();
		rmSync(root, { recursive: true, force: true });
	});
	...
	it("ignores malformed persisted compaction counts in session usage", async () => {
		...
		database.setPrivateState("agent-run-compaction.run-malformed-compaction", {
	...
	it("persists a user-selected communication personality without changing safety policy", async () => {
		...
		expect(response).toMatchObject({
			ok: true,
			snapshot: { personality: { selectedId: "concise" } },
		});
		expect(
			core.snapshot().personality.available.map((item) => item.id),
		).toEqual(["pragmatic", "friendly", "concise"]);
		core.close();
	});
	...
	it("persists custom agents and enforces their model, provider, tool, and memory scopes", async () => {
		...
		const fact = core.userModel.propose({
			kind: "preference",
			key: "private",
			value: "private shared preference",
			sourceIds: ["user"],
			confidence: 1,
			sensitivity: "normal",
		});
		...
		const created = await core.handle({
			...
			personality: {
				id: "reader",
				name: "Reader",
				description: "Read-only isolated agent",
				instructions: "Inspect precisely.",
				preferredModel: "scoped-model",
				providerIds: ["scoped-provider"],
		...
		expect(
			created.ok &&
				created.snapshot?.personality.available.find(
					(item) => item.id === "reader",
				),
		).toMatchObject({ id: "reader", builtin: false, memoryScope: "isolated" });
		...
		const encrypted = database.db
			...
			.get("runtime.custom-personalities") as { value_ciphertext: string };
		expect(encrypted.value_ciphertext).not.toContain("Inspect precisely");
		expect(
			await core.handle({
				type: "remove-personality",
				personalityId: "reader",
			}),
		).toMatchObject({
			ok: true,
			snapshot: { personality: { selectedId: "pragmatic" } },
		});
		core.close();
		rmSync(root, { recursive: true, force: true });
	});
	...
	it("streams a configured provider and contains selected attachments to the task workspace", async () => {
		...
		const core = new AgentCore({
			...
			onAgentTextDelta: (event) =>
				deltas.push(`${event.streamId}:${event.delta}`),
		});
		const session = core.runtime.ensureMainSession();
		expect(await core.handle({ type: "runtime-list-providers" })).toMatchObject(
			{
				ok: true,
				providers: [
					{
						id: "attachment-provider",
						capabilities: { documents: true, video: true },
					},
					{ id: "auto", capabilities: { documents: true, video: true } },
				],
			},
		);
		expect(
		...
		expect(
			...
			ok: false,
			error: "Attachment escapes the task workspace.",
		});
		await core.close();
		rmSync(root, { recursive: true, force: true });
	...
	it("cancels an active desktop model stream by its stream ID", async () => {
		...
		).toMatchObject({ ok: true, answer: "Cancellation requested." });
	...
	it("cancels an active desktop stream before restoring its session history", async () => {
		...
		expect(await running).toMatchObject({
			ok: false,
			error: expect.stringContaining("session history was rolled back"),
		});
		expect(database.listAgentRuns(session.id)).toMatchObject([
			{
				status: "cancelled",
	...
	it("transcribes bounded voice data without exposing provider credentials to the request surface", async () => {
		...
		const core = new AgentCore({
			...
			transcriptionProvider: {
				id: "voice-test",
				transcribe: async ({ data, mediaType }) => ({
					text: `${mediaType}:${data.byteLength}`,
					model: "voice-model",
	...
	it("queues session-owned steering into an active run at the next model boundary", async () => {
		...
		const provider: ModelProvider = {
			...
			complete: async (request) => {
				...
				expect(request.messages.at(-1)).toMatchObject({
					role: "user",
					content: [{ type: "text", text: "Focus on the release path." }],
				});
				return {
					providerId: "steering-provider",
					model: request.model,
		...
		expect(
			...
			ok: false,
			error: "Agent stream is not active for this session.",
		});
		expect(
			await core.handle({
				type: "runtime-steer-agent",
				streamId: "steer-me",
				sessionId: session.id,
				message: "Focus on the release path.",
			}),
		).toMatchObject({ ok: true, answer: "Steering message queued." });
		release();
		expect(await running).toMatchObject({
			ok: true,
			run: { status: "completed", turn: 2 },
			messages: [{ content: "Updated answer" }],
		});
		expect(
			core.runtime
		...
		).toEqual(["Start", "Focus on the release path."]);
		await core.close();
	});
	...
	it("retries the exact last agent turn from its transcript and mutation baseline", async () => {
		...
		expect(
			await core.handle({
				type: "runtime-run-agent",
				sessionId: session.id,
				message: "Try this once",
				model: "fixture",
				providerIds: ["retry-provider"],
			}),
		...
		expect(
			core.runtime
				...
				.filter((message) => message.role === "user")
				.map((message) => message.content),
		).toEqual(["Try this once"]);
		expect(
			core.runtime
				.listMessages(session.id)
				.filter((message) => message.role === "assistant")
				.map((message) => message.content),
		).toEqual(["Retried answer"]);
		await core.close();
	});
...
describe("opportunity governance", () => {
	it("stops work at the configured autonomous depth", () => {
		...
		const scored = {
			...teacherOpportunity,
			priority: engine.score(teacherOpportunity),
		};
		expect(
			engine.canLaunch(
				scored,
	...
	it("fails closed when an opportunity score is non-finite", () => {
		const engine = new OpportunityEngine();
		expect(
			engine.score({ ...teacherOpportunity, expectedUtility: Number.NaN }),
		).toBe(0);
		expect(
			engine.score({
				...teacherOpportunity,
				expectedUtility: Number.POSITIVE_INFINITY,
			}),
		).toBe(0);
	});
});
