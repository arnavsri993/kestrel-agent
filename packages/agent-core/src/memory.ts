...
export class MemoryManager {
	...
	constructor(
		...
		this.userModel = new UserModelStore(database, now);
	}
	list(): MemoryRecord[] {
	...
	correct(
		...
		input: {
			...
			layer?: NonNullable<MemoryRecord["layer"]>;
		},
	): MemoryRecord {
		const memory = this.database.getMemory(id);
		if (!memory) throw new Error("Memory not found.");
		...
		if (!content || content.length > 100_000)
			throw new Error("Corrected memory content is required.");
		this.saveVersion(memory, "user");
		const corrected: MemoryRecord = {
			...
			structuredData: {
				...
				correctionCount: Number(memory.structuredData.correctionCount ?? 0) + 1,
			},
	...
	captureExplicit(
		...
		const existing = explicit
			? this.list().find(
					(memory) =>
						memory.content.toLowerCase() === explicit.toLowerCase() &&
		...
		return {
			...(memory ? { memory } : {}),
			userModelFacts: this.userModel.proposeFromText(text, sourceId),
		};
	}
...
export function installMemoryTools(
	...
	const register = (
		...
		runtime.registerExternalTool({
			descriptor: {
				...
				source: "builtin",
				tags: ["memory", "private", "provenance"],
			},
			inputSchema,
			execute,
		});
	...
	register(
		"memory.remember",
		"Store a provenance-backed memory",
		false,
		{
			type: "object",
			properties: {
	...
	register(
		...
		"Propose a provenance-backed user-model fact for review",
		false,
		{
			type: "object",
			properties: {
				kind: { enum: ["preference", "profile", "relationship", "boundary"] },
	...
	register(
		...
		"Confirm or reject a proposed user-model fact",
		false,
		{
			type: "object",
			properties: {
