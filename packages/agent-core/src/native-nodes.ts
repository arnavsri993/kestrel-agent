...
export type NativeNodeCapability =
	| "location"
	| "talk"
	| "voiceWake"
	| "activePresence";
...
const CAPABILITIES = new Set<NativeNodeCapability>([
	"location",
	"talk",
	"voiceWake",
	"activePresence",
...
function cleanTriggers(input: unknown): string[] {
	if (!Array.isArray(input))
		throw new Error("Voice-wake triggers must be an array.");
	const raw = input.length === 0 ? DEFAULT_TRIGGERS : input;
	if (raw.length > 32)
		throw new Error("Voice-wake triggers exceed 32 entries.");
	const normalized = raw.map((value) => {
		if (typeof value !== "string")
			throw new Error("Voice-wake trigger is invalid.");
		const trigger = value.trim().replace(/\s+/g, " ").toLocaleLowerCase();
		if (!trigger || trigger.length > 64)
			throw new Error("Voice-wake trigger is invalid.");
		return trigger;
	});
...
export class NativeNodeManager {
	...
	beacon(input: NativeNodeBeacon): NativeNodeRecord {
		if (!/^[A-Za-z0-9._-]{1,128}$/.test(input.nodeId))
			throw new Error("Node ID is invalid.");
		if (!input.label.trim() || input.label.length > 100)
			throw new Error("Node label is invalid.");
		if (!["ios", "android", "macos"].includes(input.platform))
			throw new Error("Node platform is invalid.");
		if (
			input.capabilities.length > 8 ||
			input.capabilities.some((item) => !CAPABILITIES.has(item))
		)
			throw new Error("Node capabilities are invalid.");
		if (
			input.idleSeconds !== undefined &&
			(!Number.isInteger(input.idleSeconds) ||
				input.idleSeconds < 0 ||
				input.idleSeconds > 31_536_000)
		)
			throw new Error("Node idle seconds are invalid.");
		...
		return {
			...record,
			status: (input.idleSeconds ?? 0) >= 60 ? "idle" : "active",
		};
	}
	...
	list(): NativeNodeRecord[] {
		...
		return [...this.nodes.values()].map((node) => ({
			...node,
			status:
				(node.idleSeconds ?? 0) >= 60 ? ("idle" as const) : ("active" as const),
		}));
	}
	...
	enqueueTalk(
		...
		if (!text.trim() || text.length > 10_000)
			throw new Error("Talk text is invalid.");
		if (sessionId && (sessionId.length > 200 || !sessionId.trim()))
			throw new Error("Talk session is invalid.");
		return this.enqueue(
			node.nodeId,
			"talk.speak",
	...
	poll(nodeId: string): { commands: NativeNodeCommand[]; voiceWake: string[] } {
		if (!this.nodes.has(nodeId))
			throw new Error("Node must beacon before polling.");
		const now = this.now().getTime();
		const pending = (this.commands.get(nodeId) ?? [])
			.filter((command) => new Date(command.expiresAt).getTime() > now)
			.slice(0, 20);
	...
	complete(nodeId: string, result: NativeNodeResult): void {
		...
		if (!/^node-command-[A-Za-z0-9-]+$/.test(result.commandId))
			throw new Error("Node command ID is invalid.");
		if (JSON.stringify(result).length > 100_000)
			throw new Error("Node result exceeds limits.");
	...
	setVoiceWake(input: unknown): string[] {
		this.triggers = cleanTriggers(input);
		return this.getVoiceWake();
	}

	private requireCapability(nodeId: string, capability: NativeNodeCapability) {
		const node = this.nodes.get(nodeId);
		if (!node || !node.capabilities.includes(capability))
			throw new Error(`Node does not provide ${capability}.`);
		return node;
	}

	private enqueue(
		nodeId: string,
		kind: NativeNodeCommand["kind"],
		input: Record<string, unknown>,
		lifetimeMs: number,
	): NativeNodeCommand {
		const created = this.now();
		const command: NativeNodeCommand = {
			id: `node-command-${randomUUID()}`,
			kind,
