...
import { BUILTIN_SKINS, contrast, SkinManager } from "./skins";
...
describe("visual skin registry", () => {
	it("ships complete accessible built-ins and persists selection independently", () => {
		...
		expect(BUILTIN_SKINS.map((skin) => skin.id)).toEqual([
			"workstrand",
			"daylight",
			"mono",
			"slate",
		...
		for (const skin of BUILTIN_SKINS) {
			expect(
				contrast(skin.colors.ink, skin.colors.canvas),
				skin.id,
			).toBeGreaterThanOrEqual(4.5);
			expect(
				contrast(skin.colors.signal, skin.colors.canvas),
				skin.id,
			).toBeGreaterThanOrEqual(3);
		}
		manager.select("slate");
		expect(new SkinManager(database).status()).toMatchObject({
			selectedId: "slate",
		});
		database.close();
	});
	...
	it("imports strict inherited JSON, rejects unsafe contrast, and recovers after removal", () => {
		...
		const source = JSON.stringify({
			...
			terminal: {
				promptSymbol: "»",
				thinkingVerbs: ["noting", "cross-checking"],
			},
		});
		expect(manager.import(source)).toMatchObject({
		...
		expect(() =>
			manager.import(
				JSON.stringify({
					version: 1,
					id: "hover-invisible",
					name: "Hover invisible",
					description: "Bad hover contrast.",
					base: "daylight",
					colors: { solidHover: "#ffffff" },
				}),
			),
		...
		expect(() =>
			manager.import(
				JSON.stringify({
					...
					description: "Bad status contrast.",
					base: "daylight",
					colors: { statusInk: "#f1d8ca" },
				}),
			),
		).toThrow("status text contrast");
		expect(() =>
			manager.import(
				JSON.stringify({
					version: 1,
					id: "surface-invisible",
					name: "Surface invisible",
					description: "Bad surface contrast.",
					base: "workstrand",
					colors: { muted: "#888888" },
				}),
			),
		).toThrow("secondary text on surfaces contrast");
		expect(() =>
			manager.import(
				JSON.stringify({
					version: 1,
					id: "scripted",
					name: "Scripted",
					description: "Unknown executable input.",
					script: "alert(1)",
				}),
			),
		).toThrow();
		expect(() =>
			manager.import(
				JSON.stringify({
					...
					id: "remote",
					name: "Remote",
					description: "Remote stylesheet.",
					colors: { signal: "https://example.com/theme.css" },
				}),
			),
		).toThrow();
		expect(() => manager.import(" ".repeat(65_537))).toThrow("64 KB");
		expect(manager.status().selectedId).toBe("field-notes");
		expect(manager.remove("field-notes")).toMatchObject({
			selectedId: "workstrand",
		});
	...
	it("recovers when the persisted custom-skin state is not an array", () => {
		...
		expect(manager.status()).toMatchObject({
			selectedId: "workstrand",
			skins: expect.arrayContaining([
				expect.objectContaining({ id: "workstrand" }),
			]),
		});
