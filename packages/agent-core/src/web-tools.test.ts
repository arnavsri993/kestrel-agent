...
describe("network-policy web tools", () => {
	...
	it("fetches bounded readable text and labels it untrusted", async () => {
		...
		const client = new NetworkPolicyWebClient({
			...
			fetcher: async () => {
				fetches += 1;
				return new Response(
					"<html><title>Kestrel guide</title><script>bad()</script><body><h1>Safe docs</h1></body></html>",
					{ headers: { "content-type": "text/html" } },
				);
			},
			cache: new EncryptedDatabaseWebCache(
				database,
		...
		expect(
			await client.fetch("https://docs.example.test/guide#section"),
		).toMatchObject({
			content: "Kestrel guide Safe docs",
			trust: "untrusted_external",
			url: "https://docs.example.test/guide",
			cached: false,
			citation: { title: "Kestrel guide" },
		});
		expect(
			await client.fetch("https://docs.example.test/guide#section"),
		).toMatchObject({
			cached: true,
			citation: { url: "https://docs.example.test/guide" },
		});
		...
		const ciphertext = database.db
			...
			.get("web.result-cache") as { value_ciphertext: string };
	...
	it("rejects non-HTTPS, unlisted hosts, private DNS, unsafe redirects, and oversized bodies", async () => {
		const client = new NetworkPolicyWebClient({
			...
			resolveHost: async (host) =>
				host === "safe.example.test" ? ["203.0.113.21"] : ["127.0.0.1"],
			fetcher: async () =>
		...
		await expect(client.fetch("http://safe.example.test/")).rejects.toThrow(
			"HTTPS",
		);
		await expect(client.fetch("https://other.example.test/")).rejects.toThrow(
			"not allowlisted",
		);
		await expect(client.fetch("https://safe.example.test/")).rejects.toThrow(
			"byte limit",
		);
	});
	...
	it("rejects non-finite search result counts before invoking the provider", async () => {
		...
		await expect(
			client.search("safe", Number.NaN, new AbortController().signal),
	...
	it("rejects an already-aborted fetch before validation or network access", async () => {
		...
		await expect(
			client.fetch("https://safe.example.test/", controller.signal),
		).rejects.toThrow("Fetch cancelled before start.");
	...
	it("exposes approval-gated runtime fetch and search tools", async () => {
		...
		const client = new NetworkPolicyWebClient({
			allowedHosts: ["safe.example.test"],
			resolveHost: async () => ["203.0.113.22"],
			fetcher: async () =>
		...
		const blocked = await runtime.callTool(session.id, "web.fetch", {
			url: "https://safe.example.test/",
		});
		expect(blocked.status).toBe("blocked");
		const fetched = await runtime.callTool(
			session.id,
			"web.fetch",
			{ url: "https://safe.example.test/" },
		...
		expect(fetched).toMatchObject({
			status: "verified",
			output: { content: "safe", trust: "untrusted_external" },
		});
		...
		expect(searched).toMatchObject({
			...
			output: {
				results: [
					{ title: "Safe", citation: { url: "https://safe.example.test/" } },
				],
				trust: "untrusted_external",
				cached: false,
			},
	...
	it("uses the bounded official Brave Search API contract and explicit public-host opt-in", async () => {
		...
		expect(request?.url).toContain(
			"api.search.brave.com/res/v1/web/search?q=kestrel",
		);
		expect(request?.headers.get("x-subscription-token")).toBe("search-secret");
		await expect(
			provider.search("kestrel", {
				maximumResults: Number.NaN,
				signal: new AbortController().signal,
			}),
		).rejects.toThrow("finite");
		...
		).toMatchObject({ allowPublicHosts: true });
		expect(
			environmentWebAccessOptions({ BRAVE_SEARCH_API_KEY: "key" }),
		).toBeUndefined();
		expect(
			environmentWebAccessOptions({
				BRAVE_SEARCH_API_KEY: "key",
		...
		await expect(
			provider.search("kestrel", { maximumResults: 5, signal: aborted.signal }),
		).rejects.toThrow("Search cancelled before start.");
		expect(resolutions).toBe(1);
		expect(fetches).toBe(1);
	});
});
