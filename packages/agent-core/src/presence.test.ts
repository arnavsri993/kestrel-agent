...
describe("ephemeral client presence", () => {
  it("deduplicates stable identities, marks idle clients, and expires them without network metadata", () => {
    ...
    presence.beacon({ instanceId: "ui-stable", mode: "ui", version: "0.1.0", reason: "desktop window" });
    now = new Date("2026-07-23T10:00:30.000Z");
    presence.beacon({ instanceId: "ui-stable", mode: "ui", version: "0.1.0", reason: "desktop window" });
    expect(presence.list()).toMatchObject([{ instanceId: "ui-stable", status: "active", firstSeenAt: "2026-07-23T10:00:00.000Z" }]);
  ...
  it("rejects churn modes and invalid identities", () => {
    ...
    expect(() => presence.beacon({ instanceId: "bad id", mode: "ui" })).toThrow("instance ID");
    expect(() => presence.beacon({ instanceId: "cli-1", mode: "cli" as "ui" })).toThrow("mode");
  });
});
