...
describe("usage governor", () => {
	...
	it("calculates configurable token cost and enforces concurrency", () => {
		...
		governor.setPolicy({
			...
			defaultRate: {
				...
				reasoningPerMillionUsd: 4,
			},
			rates: {},
		});
		expect(
			governor.estimateCost("provider", "model", {
		...
		const ciphertext = database.db
			...
			.get("runtime.usage-policy") as { value_ciphertext: string };
	...
	it("reserves budget before calls and blocks daily or monthly overspend", () => {
		...
		const session = new AgentRuntime(database).createSession({
		...
		database.saveAgentRun({
			...
			updatedAt: "2026-07-22T17:00:00.005Z",
		});
		database.saveModelCallAudit({
			...
			completedAt: "2026-07-22T17:00:00.005Z",
		});
