...
export function selectBrowserContext(
  ...
  if (!Number.isInteger(maximumCharacters) || maximumCharacters < 1_000)
    throw new Error("Browser context budget must be at least 1,000 characters.");
  const metadata = [
    ...
    `URL: ${context.url}`,
    context.description ? `Description: ${bounded(context.description, 1_000)}` : "",
  ...
  if (remaining > 300 && context.forms.length) {
    const forms = context.forms
      .slice(0, 20)
      .map((form) => `${bounded(form.label || form.name || "Unlabelled", 120)} [${bounded(form.type, 60)}]`)
      .join("\n");
