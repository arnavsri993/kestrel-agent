...
export function createEnvironmentModelProviders(environment: NodeJS.ProcessEnv = process.env): ModelProvider[] {
  ...
  const compatible = [
    ["nous", "NOUS_API_KEY", "NOUS_MODEL", "NOUS_BASE_URL", "stepfun/step-3.7-flash:free", "https://inference-api.nousresearch.com/v1", true],
    ["groq", "GROQ_API_KEY", "GROQ_MODEL", "GROQ_BASE_URL", "openai/gpt-oss-20b", "https://api.groq.com/openai/v1", false],
    ["mistral", "MISTRAL_API_KEY", "MISTRAL_MODEL", "MISTRAL_BASE_URL", "mistral-small-latest", "https://api.mistral.ai/v1", false],
    ["openrouter", "OPENROUTER_API_KEY", "OPENROUTER_MODEL", "OPENROUTER_BASE_URL", "openrouter/free", "https://openrouter.ai/api/v1", true],
    ["xai", "XAI_API_KEY", "XAI_MODEL", "XAI_BASE_URL", "grok-3-mini", "https://api.x.ai/v1", true],
    ["deepseek", "DEEPSEEK_API_KEY", "DEEPSEEK_MODEL", "DEEPSEEK_BASE_URL", "deepseek-chat", "https://api.deepseek.com/v1", false],
    ["together", "TOGETHER_API_KEY", "TOGETHER_MODEL", "TOGETHER_BASE_URL", "meta-llama/Llama-3.3-70B-Instruct-Turbo", "https://api.together.xyz/v1", true],
    ["fireworks", "FIREWORKS_API_KEY", "FIREWORKS_MODEL", "FIREWORKS_BASE_URL", "accounts/fireworks/models/llama-v3p3-70b-instruct", "https://api.fireworks.ai/inference/v1", true],
  ...
  for (const [id, keyName, modelName, baseName, defaultModel, defaultBase, images] of compatible) {
    ...
    if (!apiKey) continue;
    providers.push(new OpenAIChatCompletionsProvider({
      id,
      apiKey,
  ...
  if (environment.CLOUDFLARE_API_KEY && environment.CLOUDFLARE_ACCOUNT_ID) {
    providers.push(new OpenAIChatCompletionsProvider({
      ...
      baseUrl: environment.CLOUDFLARE_BASE_URL ?? `https://api.cloudflare.com/client/v4/accounts/${environment.CLOUDFLARE_ACCOUNT_ID}/ai/v1`
    }));
  }
