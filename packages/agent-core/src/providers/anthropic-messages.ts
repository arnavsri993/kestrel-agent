...
function contentPart(part: ModelContentPart): Record<string, unknown> {
	...
	if (part.type === "audio")
		throw new Error(
			"Anthropic Messages does not support audio input in this adapter.",
		);
	if (part.type === "video")
		throw new Error(
			"Anthropic Messages does not support video input in this adapter.",
		);
	const source =
		part.source === "url"
	...
	return part.type === "image"
		? { type: "image", source }
		: { type: "document", source, ...(part.name ? { title: part.name } : {}) };
}
	...
	for (const message of messages) {
		...
		if (message.role === "tool") {
			if (!message.toolCallId)
				throw new Error("Anthropic tool results require toolCallId.");
			output.push({
				role: "user",
				content: [
					{
	...
	async complete(
		...
		await readServerSentEvents(response, this.id, ({ data }) => {
			...
			if (event.type === "error") {
				...
				throw new ModelProviderError(
					`Anthropic stream error: ${String(error.message ?? "unknown error")}`,
					this.id,
					retryable,
				);
			}
