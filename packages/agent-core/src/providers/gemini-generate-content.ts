...
function part(input: ModelContentPart): Record<string, unknown> {
	...
	if (input.source === "url")
		return { fileData: { fileUri: input.data, mimeType: input.mediaType } };
...
function content(message: ModelMessage): Record<string, unknown> {
	if (message.role === "tool") {
		if (!message.toolName)
			throw new Error("Gemini tool results require toolName.");
		return {
			role: "user",
			parts: [
				{
					functionResponse: {
						...
						response: { output: contentText(message.content) },
					},
				},
			],
		};
...
export class GeminiGenerateContentProvider implements ModelProvider {
	...
	async complete(
		...
		const candidateContent = candidate?.content as
			...
			| undefined;
		const responseParts = Array.isArray(candidateContent?.parts)
		...
		const result: ModelResult = {
			...
			toolCalls,
			usage: {
				inputTokens: usageCount(usage.promptTokenCount),
				outputTokens: usageCount(usage.candidatesTokenCount),
				cachedInputTokens: usageCount(usage.cachedContentTokenCount),
