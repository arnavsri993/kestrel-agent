...
function inputPart(part: ModelContentPart): Record<string, unknown> {
	...
	if (part.type === "audio") {
		if (part.source === "url")
			throw new Error(
				"OpenAI audio input requires base64 data in this adapter.",
			);
	...
	if (part.type === "video")
		throw new Error(
			"OpenAI Responses does not support direct video input in this adapter.",
		);
	...
	for (const message of messages) {
		...
		if (message.role === "tool") {
			if (!message.toolCallId)
				throw new Error("OpenAI tool results require toolCallId.");
			items.push({
	...
	async complete(
		...
		await readServerSentEvents(response, this.id, ({ data }) => {
			...
			if (event.type === "response.output_item.done") {
				...
				if (
					...
					toolCalls.set(item.call_id, {
						id: item.call_id,
						name: item.name,
						arguments: safeJsonObject(String(item.arguments ?? "{}")),
					});
				}
			}
			...
			if (event.type === "error") {
				...
				throw new ModelProviderError(
					`OpenAI stream error: ${String(error?.message ?? "unknown error")}`,
					this.id,
					true,
				);
			}
		...
		const usage =
			(completed.usage as Record<string, unknown> | undefined) ?? {};
		const inputDetails =
		...
		const result: ModelResult = {
			providerId: this.id,
			model:
				typeof completed.model === "string" ? completed.model : request.model,
