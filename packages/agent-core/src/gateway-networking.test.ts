...
describe("trusted proxy gateway authentication", () => {
	...
	it("enforces source CIDRs, required headers, users, local-interface spoof guards, and scope caps", () => {
		const authorizer = new TrustedProxyAuthorizer(
			{ ...configuration, maximumScopes: [...configuration.maximumScopes] },
			() => new Set(["192.168.1.4"]),
		);
		const headers = {
			"x-auth-user": "Operator@Example.Test",
			"x-forwarded-proto": "https",
			"x-forwarded-host": "workstrand.example",
			"x-workstrand-scopes": "read",
		};
		expect(
			authorizer.authorize({ remoteAddress: "10.0.0.8", headers }),
		).toEqual({
			kind: "trusted-proxy",
		...
		expect(() =>
			authorizer.authorize({ remoteAddress: "10.0.1.8", headers }),
		).toThrow("untrusted source");
		expect(() =>
			authorizer.authorize({
				remoteAddress: "10.0.0.8",
				headers: { ...headers, "x-auth-user": "other@example.test" },
			}),
		).toThrow("not allowed");
		expect(() =>
			authorizer.authorize({
		...
		expect(() =>
			authorizer.authorize({
				remoteAddress: "10.0.0.8",
				headers: { ...headers, "x-workstrand-scopes": "approve" },
			}),
		...
		expect(() =>
			new TrustedProxyAuthorizer(
				{
					...configuration,
					trustedSources: ["192.168.1.4"],
					maximumScopes: [...configuration.maximumScopes],
				},
				() => new Set(["192.168.1.4"]),
			).authorize({ remoteAddress: "192.168.1.4", headers }),
		).toThrow("local host interface");
	});
	...
	it("requires deliberate loopback proxy trust", () => {
		...
		const denied = new TrustedProxyAuthorizer({
			...configuration,
			trustedSources: ["127.0.0.1"],
			maximumScopes: [...configuration.maximumScopes],
		});
		expect(() =>
			denied.authorize({ remoteAddress: "127.0.0.1", headers }),
		).toThrow("loopback source");
		expect(() =>
			denied.authorize({ remoteAddress: "::FFFF:127.0.0.1", headers }),
		).toThrow("loopback source");
		const allowed = new TrustedProxyAuthorizer({
			...configuration,
			trustedSources: ["127.0.0.1"],
			allowLoopback: true,
			maximumScopes: [...configuration.maximumScopes],
		...
		expect(
			allowed.authorize({ remoteAddress: "127.0.0.1", headers }),
		).toMatchObject({
			identity: "operator@example.test",
			scopes: ["read", "tasks"],
		});
	...
	it.each([["10.0.0.0/"], ["10.0.0.0/24/extra"], ["10.0.0.0/ 24"]])(
		"rejects malformed trusted source CIDR %s",
		(trustedSource) => {
			expect(
				() =>
					new TrustedProxyAuthorizer({
...
describe("managed Tailscale exposure", () => {
	it("publishes loopback through tailnet-only Serve and resets only what it applied", async () => {
				...
				return args[0] === "status"
					? {
							...
							stdout: JSON.stringify({
								...
								Self: { Online: true, DNSName: "agent.example.ts.net." },
							}),
							stderr: "",
						}
		...
		const manager = new TailscaleExposureManager(
			{
				mode: "serve",
				serviceName: "svc:workstrand",
				resetOnExit: true,
	...
	it("requires an explicit public acknowledgement for Funnel and refuses non-loopback targets", async () => {
		expect(
			() =>
				new TailscaleExposureManager({
					mode: "funnel",
					resetOnExit: true,
					publicExposureApproved: false,
				}),
		).toThrow("explicit public exposure approval");
		const runner: GatewayCommandRunner = {
			run: async () => ({
				exitCode: 0,
				stdout: JSON.stringify({
		...
		const manager = new TailscaleExposureManager(
			{ mode: "serve", resetOnExit: false, publicExposureApproved: false },
			runner,
		);
		await expect(manager.apply("not a URL")).rejects.toThrow("loopback");
		await expect(manager.apply("http://0.0.0.0:18789")).rejects.toThrow(
			"loopback",
		);
	});
	...
	it("throws when Tailscale status returns invalid JSON or is missing MagicDNS", async () => {
		const runStatus = async (stdout: string) => {
			...
			await expect(manager.apply("http://127.0.0.1:18789")).rejects.toThrow(
		...
		await runStatus(
			JSON.stringify({
				BackendState: "Running",
				Self: { Online: false, DNSName: "agent.example.ts.net." },
			}),
		);
		await runStatus(
			JSON.stringify({
				BackendState: "Running",
				Self: { Online: true, DNSName: null },
			}),
		);
...
describe("Bonjour gateway discovery", () => {
	it("publishes bounded non-secret hints and stops the one long-lived registration", async () => {
		...
		let stopped = false;
		const runner: GatewayProcessRunner = {
		...
		const advertiser = new BonjourAdvertiser(
			{
				...
				displayName: "Kestrel",
				tlsEnabled: true,
				tlsSha256: "a".repeat(64),
				cliPath: "/private/path",
			},
			runner,
			"darwin",
		);
		expect(await advertiser.start("https://0.0.0.0:18789")).toMatchObject({
			active: true,
			mode: "minimal",
				...
				new BonjourAdvertiser(
					{
						...
						displayName: "Kestrel",
						tlsEnabled: false,
						cliPath: `/${"a".repeat(255)}`,
					},
					runner,
					"darwin",
				),
		).toThrow("CLI path hint");
	});
	...
	it("keeps discovery off without starting a process and rejects unsupported hosts", async () => {
		...
		expect(
			await new BonjourAdvertiser(
				{ mode: "off", displayName: "Kestrel", tlsEnabled: false },
				runner,
				"linux",
		...
		await expect(
			new BonjourAdvertiser(
				{ mode: "minimal", displayName: "Kestrel", tlsEnabled: false },
				runner,
				"linux",
			).start("http://127.0.0.1:1"),
	...
	it("rejects malformed gateway origins before starting discovery", async () => {
		...
		await expect(advertiser.start("not a URL")).rejects.toThrow(
			"Bonjour gateway origin is invalid.",
		);
	});
});
