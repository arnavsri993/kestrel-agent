...
export class LifeContextService {
	...
	upsertPerson(input: PersonInput): PersonRecord {
		...
		const person = PersonRecordSchema.parse({
			...
			nicknames: [
				...new Set(
					input.nicknames?.map((value) => value.trim()).filter(Boolean) ??
						existing?.nicknames ??
						[],
				),
			],
			...
			facts,
			sourceIds: [...new Set([...(existing?.sourceIds ?? []), input.sourceId])],
			confidence: 1,
	...
	providerStatuses(): CalendarProviderStatus[] {
		const googleState = this.database.getCalendarSyncState<{
			lastSyncedAt?: string;
		}>("google");
		return [
			{
				...
				state: "connected",
				detail:
					"Explicit, inferred, and suggested blocks stay encrypted on this Mac.",
				readOnly: false,
			},
			{
				id: "google",
	...
	async syncGoogleIfStale(at = this.now()): Promise<void> {
		...
		const state = this.database.getCalendarSyncState<{ lastSyncedAt?: string }>(
			"google",
		);
		const lastSynced = state?.lastSyncedAt ? Date.parse(state.lastSyncedAt) : 0;
		if (Number.isFinite(lastSynced) && at.getTime() - lastSynced < 15 * 60_000)
			return;
	...
	assembleContext(input: {
		...
		const people = [
			...
			.filter(
				(person, index, all) =>
					all.findIndex((item) => item.id === person.id) === index,
			)
		...
		const influences = [
			...
			...people.map((person) => ({
				...
				id: person.id,
				reason: directlyNamedPeople.some(
	...
	captureConversation(text: string, sourceId: string): UnifiedCalendarEvent[] {
		...
		if (dayOverride) {
			...
			if (existing && endClock) {
				const timestamp = this.now().toISOString();
				const remainingDays = existing.recurrenceDays!.filter(
					(day) => day !== 5,
				);
				this.database.upsertCalendarEvent({
					...existing,
	...
	private upsertPersonFact(facts: PersonFact[], incoming: PersonFact): void {
		...
		if (
			active.some(
				(fact) => normalized(fact.value) === normalized(incoming.value),
			)
		)
			return;
