...
function groupCharacters(group: MessageGroup): number {
	return group.entries.reduce(
		(sum, entry) => sum + messageCharacters(entry.message),
		0,
	);
}
...
function atomicConversationGroups(messages: IndexedMessage[]): MessageGroup[] {
	...
	for (let index = 0; index < messages.length; ) {
		...
		if (
			...
			!entry.message.toolCalls?.length
		) {
...
function boundedText(value: string, maximumCharacters: number): string {
	...
	const suffix = "\n[…truncated]";
	if (maximumCharacters <= suffix.length + 16)
...
function fitMessage(
	...
	return {
		...message,
		content: textContent(
			boundedText(contentText(message.content), contentCharacters),
		),
	};
}
...
export class ContextCompactor {
	compact(
		...
		const maximumCharacters = normalizedMaximum(
			options.maximumCharacters,
			120_000,
		);
		const preserveRecentMessages = normalizedMaximum(
		...
		const latestUserGroup = [...groups]
			.reverse()
			.find((group) =>
				group.entries.some((entry) => entry.message.role === "user"),
			);
		...
		const digestReserve =
			possibleDigestGroups.length > 0
				? Math.min(
						messageCharacters({
							role: "system",
							content: textContent(possibleDigest),
						}),
		...
		const recent = groups
			...
			.sort(
				(left, right) =>
					left.entries[0]!.sourceIndex - right.entries[0]!.sourceIndex,
			)
			.flatMap(
				(group) =>
					groupOverrides.get(group) ??
					group.entries.map((entry) => entry.message),
			);
		const messages = [
			...compactedSystems,
