...
export const CoreRequestSchema = z.discriminatedUnion("type", [
 z.object({ type: z.literal("source-run-review"), retry: z.boolean().default(false), sessionId: z.string().min(1), observationId: z.string().min(1).max(200), model: z.string().min(1).default("auto"), providerIds: z.array(z.string().min(1)).min(1).default(["auto"]) }),
...
export const BrokeredCredentialIdSchema = z.enum([
	"openai",
	"openai-secondary",
	"anthropic",
	"anthropic-secondary",
...
export const RendererRequestSchema = z.union([
 ...
 z.object({ type: z.literal("whatsapp-sync"), sessionId: z.string().min(1), resourceId: z.string().min(1) }),
	...
	z.object({
		...
		credentialId: BrokeredCredentialIdSchema,
