...
import { createEncryptionKey, decryptText, encryptText } from "./index.js";
...
describe("encryption", () => {
	describe("encryptText", () => {
		...
		it("should throw an error if the key is not 32 bytes", () => {
			...
			expect(() => encryptText("test", invalidKey)).toThrowError(
				"Encryption key must be 32 bytes",
			);

			const invalidKey2 = Buffer.alloc(33);
			expect(() => encryptText("test", invalidKey2)).toThrowError(
				"Encryption key must be 32 bytes",
			);
		});
		...
		it("should be successfully decrypted by decryptText", () => {
			const key = createEncryptionKey();
			const plaintext =
				"hello secret world, this is a longer string to test decryption.";
			const payload = encryptText(plaintext, key);
