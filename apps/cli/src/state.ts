...
function encryptionKey(directory: string): Buffer {
	...
	if (existsSync(path)) {
		...
		if (!metadata.isFile() || metadata.isSymbolicLink())
			throw new Error("Kestrel data key must be a regular non-symlink file.");
		if (metadata.size > MAX_ENCRYPTION_KEY_FILE_BYTES)
			throw new Error("Kestrel data key is invalid.");
	...
	try {
		writeFileSync(descriptor, `${key.toString("base64")}\n`);
	} finally {
...
export function openKestrel(workspaceRoots: string[] = []): AgentCore {
	...
	const activePersistedWorkspaceRoots = persistedWorkspaceRoots.flatMap(
		(root) => {
			try {
	...
	const explicitWorkspaceRoots = workspaceRoots.map((root) =>
		realpathSync(root),
	);
	const grantedWorkspaceRoots = [
		...new Set([...explicitWorkspaceRoots, ...activePersistedWorkspaceRoots]),
	];
	const configuredWorkspaceRoots = [
		...new Set([...explicitWorkspaceRoots, ...persistedWorkspaceRoots]),
	];
	...
	const remoteExecution = environmentRemoteExecutionConfiguration(
		process.env,
		join(directory, "artifacts", "remote"),
	);
	const managedPolicy =
		process.env.KESTREL_MANAGED_POLICY && process.env.KESTREL_MANAGED_POLICY_KEY
			? loadSignedManagedPolicy(
	...
	return new AgentCore({
		...
		seedDevelopmentFixtures:
			process.env.KESTREL_ENABLE_DEVELOPMENT_FIXTURES === "1",
...
export function resolveModelConfig(input: {
	...
	const providers = input.providers?.length
		? input.providers
		: (process.env.KESTREL_PROVIDERS ?? "")
				.split(",")
				.map((value) => value.trim())
				.filter(Boolean);
	if (!model)
		throw new Error("A model is required. Pass --model or set KESTREL_MODEL.");
	if (providers.length === 0)
		throw new Error(
			"At least one provider is required. Pass --providers or set KESTREL_PROVIDERS.",
		);
