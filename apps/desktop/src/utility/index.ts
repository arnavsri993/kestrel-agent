...
function browserRequest<T>(request: Record<string, unknown>, signal?: AbortSignal): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    const cancelled = () => signal?.reason instanceof Error ? signal.reason : new Error("Browser operation cancelled.");
    if (signal?.aborted) {
      reject(cancelled());
      return;
    }
...
const browserBackend: BrowserAutomationBackend = {
  createSession: async ({ allowedOrigins }) => browserRequest<string>({ operation: "create", allowedOrigins }),
  navigate: async (sessionId, url, signal) => { await browserRequest({ operation: "navigate", sessionId, url }, signal); },
  act: async (sessionId, action: BrowserAction, signal) => { await browserRequest({ operation: "act", sessionId, action }, signal); },
  snapshot: (sessionId, signal) => browserRequest<BrowserSnapshot>({ operation: "snapshot", sessionId }, signal),
  screenshot: async (sessionId, signal) => {
    const result = await browserRequest<ScreenshotFrame>({ operation: "screenshot", sessionId }, signal);
    return { ...result, rgba: new Uint8Array(result.rgba), ...(result.png ? { png: new Uint8Array(result.png) } : {}) };
  },
  setViewport: async (sessionId, viewport: BrowserViewport, signal) => { await browserRequest({ operation: "viewport", sessionId, viewport }, signal); },
  diagnostics: (sessionId, signal) => browserRequest<BrowserDiagnostic[]>({ operation: "diagnostics", sessionId }, signal),
  authHandoff: async (sessionId, visible, signal) => { await browserRequest({ operation: "auth-handoff", sessionId, visible }, signal); },
  upload: async (sessionId, selector, paths, signal) => { await browserRequest({ operation: "upload", sessionId, selector, paths }, signal); },
...
port.on("message", async ({ data }) => {
  ...
  if (message.type === "bootstrap" && message.config) {
    try {
      ...
      const transcriptionProvider = createEnvironmentTranscriptionProvider(message.config.secureEnvironment);
      ...
      if (configuredLanguageServer) {
        ...
        for (const session of core.runtime.listSessions()) installCodeIntelligenceTools(core.runtime, languageServer, session.id);
      }
      core.runtime.on("event", (event) => port.postMessage({ type: "runtime-event", event }));
      ...
      port.postMessage({ type: "start-error", error: error instanceof Error ? error.message : "Core bootstrap failed" });
    }
    return;
  }
  if (message.type === "request" && message.requestId) {
    const response = core ? await core.handle(CoreRequestSchema.parse(message.request)) : { ok: false as const, error: "Agent Core is not initialized." };
