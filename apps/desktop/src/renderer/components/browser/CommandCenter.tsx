...
export function CommandCenter({
  ...
  const visible = useMemo(() => {
    ...
    return destinations.filter((destination) =>
      !needle || `${destination.label} ${destination.detail} ${destination.group}`.toLowerCase().includes(needle),
    );
  }, [destinations, query]);
  useEffect(() => {
  ...
  return (
    <main className="command-center" aria-labelledby="command-center-title">
      ...
      <label className="command-search">
        ...
        <input autoFocus value={query} placeholder="Search actions and settings" onChange={(event) => setQuery(event.target.value)} />
        <kbd>⌘ K</kbd>
      </label>
      {visible.length === 0 ? (
        ...
        <div className="command-groups">
          {(["Browse", "Agent", "Context", "Build", "System"] as const).map((group) => {
            const items = visible.filter((destination) => destination.group === group);
              ...
              <section key={group} aria-labelledby={`command-${group}`}>
                ...
                <div>
                  {items.map((destination) => (
                    <button type="button" key={destination.id} onClick={() => onSelect(destination.id)}>
