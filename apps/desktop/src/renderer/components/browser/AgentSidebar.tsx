    ...
    >
      <div className="agent-sidebar-header">
        ...
        <section className={`agent-session-context state-${agentState}`} aria-label="Current task and agent state">
          <div className="agent-context-line" title={activeTab?.url || "No page open"}>
            <Icon name="context" />
          ...
          <div className="agent-sidebar-history-list">
              {sortedSessions.slice(0, 3).map((session) => (
                ...
                >
                  <Icon name="chat" />
                  <span><strong>{sessionTitleForDisplay(session.title)}</strong></span>
                  <time dateTime={session.updatedAt}>
					{new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric" }).format(new Date(session.updatedAt))}
				  </time>
                </button>
              ))}
          </div>
