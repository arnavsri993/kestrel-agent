...
export function ObservabilitySettings() {
	const [configuration, setConfiguration] =
		useState<ObservabilityConfiguration>(() => structuredClone(EMPTY));
	...
	function receive(
		...
		if (
			"observabilityConfiguration" in response &&
			response.observabilityConfiguration &&
			response.observabilityStatus
		) {
	...
	useEffect(() => {
		void window.kestrel
			.request({ type: "observability-get" })
			.then(receive)
			.catch((cause) =>
				setError(errorMessage(cause, "Observability status failed.")),
			);
	}, []);
		...
		try {
			...
			setNotice(
				configuration.enabled
					? "Observability configuration saved and applied."
					: "External observability disabled.",
			);
		} catch (cause) {
			setError(
				errorMessage(cause, "Observability configuration could not be saved."),
			);
		} finally {
			setBusy("");
		}
	...
	async function testCollector() {
		...
		setNotice("");
		try {
			const checked = await window.kestrel.request({
	...
	const canTest =
		configuration.enabled &&
		configuration.otlp.enabled &&
		Boolean(configuration.otlp.endpoint) &&
		(configuration.otlp.metrics || configuration.otlp.traces);

	return (
		<article className="setting-row observability-setting">
			<div>
				...
				<label className="checkbox-label">
					...
					/>
					<span>Enable content-free diagnostics</span>
				</label>
				{configuration.enabled && (
					<div className="observability-controls">
						<details open={configuration.otlp.enabled}>
							<summary>
								...
								<span
									className={`external-secret-state ${configuration.otlp.enabled ? "ready" : "disabled"}`}
								>
									{configuration.otlp.enabled ? "enabled" : "disabled"}
								</span>
							</summary>
							<div className="observability-form">
								<label className="checkbox-label">
									<input
										type="checkbox"
										checked={configuration.otlp.enabled}
										onChange={(event) =>
											setConfiguration((current) => ({
												...current,
												otlp: {
									...
									/>
									<span>Enable OTLP push</span>
								</label>
								<div className="external-secret-grid">
									<label className="wide">
										Collector base URL{" "}
										<input
											value={configuration.otlp.endpoint}
									...
									<label>
										Service name{" "}
										<input
											value={configuration.otlp.serviceName}
											autoComplete="off"
											spellCheck={false}
											onChange={(event) =>
												setConfiguration((current) => ({
									...
									<label>
										Export interval{" "}
										<select
											value={configuration.otlp.exportIntervalMs}
											onChange={(event) =>
												setConfiguration((current) => ({
													...current,
													otlp: {
														...current.otlp,
														exportIntervalMs: Number(event.target.value),
													},
												}))
											}
										>
									...
									<label>
										Auth header name{" "}
										<input
											value={configuration.otlp.headerName}
											autoComplete="off"
											spellCheck={false}
											onChange={(event) =>
												setConfiguration((current) => ({
									...
									<label>
										...
										<input
											...
											value={headerValue}
											placeholder={
												status?.hasHeaderValue
													? "Stored; enter a replacement"
													: "Optional collector credential"
											}
									...
									<label>
										...
										<input
											...
											step={0.05}
											inputMode="decimal"
											value={configuration.otlp.sampleRate}
											onChange={(event) =>
												setConfiguration((current) => ({
													...current,
													otlp: {
								...
								<div className="observability-signal-row">
									<label className="checkbox-label">
										<input
											type="checkbox"
											checked={configuration.otlp.metrics}
											onChange={(event) =>
												setConfiguration((current) => ({
													...current,
													otlp: {
									...
									<label className="checkbox-label">
										<input
											type="checkbox"
											checked={configuration.otlp.traces}
											onChange={(event) =>
												setConfiguration((current) => ({
													...current,
													otlp: {
														...current.otlp,
														traces: event.target.checked,
													},
												}))
											}
										/>
						...
						<details open={configuration.prometheus.enabled}>
							<summary>
								...
								<span
									className={`external-secret-state ${configuration.prometheus.enabled ? "ready" : "disabled"}`}
								>
									{configuration.prometheus.enabled ? "enabled" : "disabled"}
								</span>
							</summary>
							<div className="observability-form">
								<label className="checkbox-label">
									<input
										type="checkbox"
										checked={configuration.prometheus.enabled}
										onChange={(event) =>
											setConfiguration((current) => ({
												...current,
												prometheus: { enabled: event.target.checked },
											}))
										}
									/>
				...
				<div className="button-row observability-actions">
					...
					>
						{busy === "test" ? "Sending test…" : "Test collector"}
					</button>
				</div>
				<div className="observability-feedback" aria-live="polite">
					{status && (
						<small>
							{status.detail}
							{status.lastExportAt
								? ` Last ${status.lastExportState} test: ${new Date(status.lastExportAt).toLocaleString()}.`
								: ""}
						</small>
					)}
