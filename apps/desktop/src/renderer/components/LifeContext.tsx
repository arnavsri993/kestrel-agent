...
function memoryState(memory: MemoryRecord): string {
  return (
    ...
    (memory.userConfirmed ? "confirmed" : memory.inferred ? "inferred" : "suggested")
  ).replaceAll("_", " ");
}
            ...
            return (
              <section className="calendar-day" key={day.toISOString()}>
                ...
                <div>
                  ...
                  {items.length === 0 && <span className="calendar-open">Open</span>}
                </div>
              </section>
            );
          })}
          ...
          {selected ? (
            <>
              <span
                ...
                data-origin={selected.event.origin}
              >
...
function PeopleView() {
  ...
  const [formality, setFormality] =
    useState<"casual" | "neutral" | "professional" | "formal">("professional");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Could not save person.");
      setBusy(false);
    }
  }
  ...
  async function removePerson(id: string) {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Could not delete this person.",
      );
      setBusy(false);
    }
  }
        ...
        {people.map((person) => (
          ...
          >
            ...
            <div>
              ...
              <small>
                {person.relationship ?? person.organization ?? "Relationship not set"}
              </small>
            </div>
          </button>
        ))}
          ...
          <>
            ...
            {deleteConfirmId === selected.id ? (
              <div className="destructive-confirmation" role="alert">
                <p>
                  Delete this person and every memory directly attached to
                  them? Unrelated memories remain.
                </p>
                <div>
                  <button
                    className="button danger"
        ...
        <form onSubmit={(event) => void save(event)}>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setFormality(
                  event.target.value as typeof formality,
                )
              }
            >
...
function MemoryView({
  ...
  const [layer, setLayer] = useState<"all" | NonNullable<MemoryRecord["layer"]>>(
    "all",
  );
  ...
  async function forgetSelected() {
    ...
    window.setTimeout(() => {
      const target = next
        ...
        : undefined;
      (target ?? memoryListRef.current?.querySelector<HTMLButtonElement>("button"))?.focus();
    }, 0);
  }
        ...
        <section className="memory-inspector">
          {selected ? (
            ...
            <div className="memory-inspector-empty">
              ...
              <p>Inspect its source, authority, confidence, use, and controls.</p>
            </div>
          )}
        </section>
      </div>
      ...
      <details className="life-create">
        ...
        <form onSubmit={(event) => void createMemory(event)}>
          ...
          <label>
            ...
            >
              {["semantic", "episodic", "procedural", "project", "relationship"].map(
                (type) => (
      ...
      <section className="context-explainer">
        <header>
          <div>
            ...
            <p>Preview the smallest non-sensitive context a task would receive.</p>
          </div>
        </header>
...
export function LifeContext({
  ...
  return (
    <div className="life-page">
      ...
      {view === "memory" && (
        <MemoryView snapshot={snapshot} update={update} />
      )}
    </div>
  );
}
