...
export function EventApplications({
	...
	useEffect(() => {
		void refresh().catch((cause) =>
			setError(
				cause instanceof Error ? cause.message : "Applications could not load.",
			),
		);
	}, []);
	...
	async function create(event: FormEvent) {
		...
		try {
			...
			if (!response.ok || !response.eventApplications?.[0])
				throw new Error(
					response.ok ? "Application was not created." : response.error,
				);
			setForm({ title: "", organizer: "", url: "", deadline: "" });
			await refresh(response.eventApplications[0].id);
		} catch (cause) {
			setError(
				cause instanceof Error ? cause.message : "Application was not created.",
			);
		} finally {
			setBusy("");
		}
	...
	async function update(
		application: EventApplicationContract,
		patch: {
		...
		try {
			...
			setError(
				cause instanceof Error ? cause.message : "Application was not saved.",
			);
		} finally {
			setBusy("");
		}
	...
	async function prepare(application: EventApplicationContract) {
		...
		try {
			...
			if (!created.ok || !created.session)
				throw new Error(
					created.ok ? "Agent session was not created." : created.error,
				);
			...
			const run = (await window.kestrel.request({
			...
			setError(
				cause instanceof Error
					? cause.message
					: "The preparation agent could not start.",
			);
		} finally {
			setBusy("");
		}
	...
	async function submitWithAgent(application: EventApplicationContract) {
		...
		try {
			...
			if (!created.ok || !created.session)
				throw new Error(
					created.ok ? "Submission session was not created." : created.error,
				);
			...
			setError(
				cause instanceof Error
					? cause.message
					: "The browser submission agent could not start.",
			);
		} finally {
			setBusy("");
		}
		...
		<section className="page-frame event-applications-page">
			<header className="event-applications-hero">
				...
				<div className="event-safety-note">
					...
					<span>
						The agent may research and draft. It cannot turn a draft into a
						submitted application from this screen.
					</span>
				</div>
			</header>
			<div className="event-applications-layout">
				<aside
					className="event-application-rail"
				...
				<div className="event-application-main">
					<form className="event-import-form" onSubmit={create}>
						<div>
							<span className="eyebrow">Import official page</span>
							<h2>Start a review workspace</h2>
						</div>
						<label>
							<span>Event or hackathon</span>
							<input
								required
								maxLength={200}
								value={form.title}
						...
						<label>
							...
							<input
								...
								onChange={(event) =>
									setForm({ ...form, organizer: event.target.value })
								}
							/>
						</label>
						<label className="wide">
							<span>Official HTTPS application URL</span>
							<input
								required
								type="url"
								inputMode="url"
						...
						<button className="button primary" disabled={Boolean(busy)}>
							{busy === "create" ? "Importing…" : "Import application"}
						</button>
					</form>
					{selected && (
						<article className="event-review">
							...
							<div className="event-review-actions">
								<button
									...
									disabled={Boolean(busy) || selected.status === "submitted"}
									onClick={() => void prepare(selected)}
								>
									{busy === "prepare"
										? "Starting agent…"
										: selected.answers.length
											? "Prepare again with agent"
							...
							<section>
								<div className="event-section-heading">
									...
									<span>
										{
											selected.eligibility.filter((item) => item.met === true)
												.length
										}
										/{selected.eligibility.length} confirmed
									</span>
								</div>
								{selected.eligibility.length === 0 ? (
									...
									<ul className="eligibility-list">
										{selected.eligibility.map((item, index) => (
											<li key={item.id}>
												<div>
												...
												<select
													...
													aria-label={`${item.label} eligibility`}
													value={
														item.met === null
															? "unknown"
															: item.met
													...
													onChange={(event) => {
														const eligibility = selected.eligibility.map(
															(entry, itemIndex) =>
																itemIndex === index
																	? {
																			...entry,
							...
							<section>
								<div className="event-section-heading">
									<h3>Answers for review</h3>
									<span>
										{selected.answers.filter((item) => item.reviewed).length}/
										{selected.answers.length} reviewed
									</span>
								</div>
								{selected.answers.length === 0 ? (
									...
									<div className="event-answer-list">
										{selected.answers.map((answer, index) => (
											<label
												key={answer.id}
												className={`event-answer sensitivity-${answer.sensitivity}`}
											>
												<span>
													<b>
														{answer.label}
													...
													<small>
														{answer.sensitivity} · drafted by {answer.source}
													</small>
												</span>
												<textarea
													disabled={Boolean(busy)}
													value={answer.value}
													onChange={(event) => {
														const answers = selected.answers.map(
															(entry, itemIndex) =>
																itemIndex === index
																	? {
																			...entry,
													...
													onBlur={() =>
														void update(selected, { answers: selected.answers })
													}
												/>
							...
							<footer>
								...
								{selected.status === "approved" ? (
									<button
										className="button primary"
										disabled={Boolean(busy)}
										onClick={() => void submitWithAgent(selected)}
									>
										{busy === "submit"
											? "Starting secure browser…"
											: "Continue with browser agent"}
									</button>
								) : (
									<button
										className="button primary"
										disabled={Boolean(busy) || selected.status === "submitted"}
										onClick={() =>
											void update(selected, { status: "approved" })
										}
									>
										{selected.status === "submitted"
											? "Submission receipt saved"
											: "Approve reviewed application"}
									</button>
								)}
							</footer>
						</article>
