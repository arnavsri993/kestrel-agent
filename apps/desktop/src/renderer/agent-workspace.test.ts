...
describe("agent workspace presentation", () => {
  ...
  it("filters by task title or project and keeps recent work first", () => {
    expect(agentSessionsForWorkspace(sessions, "", "all").map(({ id }) => id))
      .toEqual(["session-new", "session-old"]);
    expect(agentSessionsForWorkspace(sessions, "launch", "all")).toHaveLength(1);
  ...
  it("keeps project and recency labels compact", () => {
    ...
    expect(agentSessionRecency("2026-08-11T13:59:30.000Z", now)).toBe("Just now");
