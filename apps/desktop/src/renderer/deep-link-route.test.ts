...
describe("desktop deep-link routing", () => {
  ...
  it("does not infer arbitrary session or parameterized navigation", () => {
    expect(desktopDeepLinkAction("kestrel://session/private-id")).toBeUndefined();
    expect(desktopDeepLinkAction("kestrel://settings?section=privacy")).toBeUndefined();
    ...
    expect(desktopDeepLinkAction("https://example.com/settings")).toBeUndefined();
  });
});
