...
export function useUserBrowser(): UserBrowserController {
  ...
  const requestState = useCallback(async (request: Parameters<typeof window.kestrel.request>[0]) => {
    try {
      ...
      const message = cause instanceof Error ? cause.message : "The browser request failed.";
      setError(message);
      throw cause;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    void window.kestrel
      ...
      .catch((cause) => {
        if (active)
          setError(cause instanceof Error ? cause.message : "The browser could not start.");
      });
    return () => {
      active = false;
      unsubscribe();
  ...
  const setContentBounds = useCallback(async (
    bounds: { x: number; y: number; width: number; height: number },
    visible: boolean,
  ) => {
