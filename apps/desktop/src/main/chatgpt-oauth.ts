...
export class ChatGptOAuthManager {
	...
	private readStdout(chunk: Buffer): void {
		...
		if (Buffer.byteLength(this.stdoutBuffer) > MAX_LINE_BYTES) {
			this.failAll(
				new Error("Codex account response exceeded the safety limit."),
			);
			void this.close();
			return;
		}
		while (this.stdoutBuffer.includes("\n")) {
			...
			try {
				...
				this.failAll(
					new Error("Codex account service emitted malformed JSON."),
				);
				void this.close();
			}
		}
	...
	private waitForLogin(loginId: string, signal?: AbortSignal): Promise<void> {
