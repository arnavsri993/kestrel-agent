...
describe("plugin publisher trust store", () => {
  it("imports, persists, deduplicates, and removes Ed25519 publisher keys", async () => {
    ...
    writeFileSync(document, JSON.stringify({ keyId: "publisher.test", publicKey: publicKey.export({ type: "spki", format: "pem" }).toString() }));
    const store = new PluginTrustStore(join(root, "trust.json"));
    const imported = await store.importDocument(document);
    expect(imported).toMatchObject({ keyId: "publisher.test", fingerprint: expect.stringMatching(/^[a-f0-9]{64}$/) });
    expect(await store.importDocument(document)).toEqual(imported);
    expect(await store.trustKeys()).toMatchObject([{ keyId: "publisher.test" }]);
  ...
  it("rejects non-Ed25519 documents and key-ID replacement", async () => {
    ...
    writeFileSync(invalid, JSON.stringify({ keyId: "publisher.test", publicKey: rsa.export({ type: "spki", format: "pem" }).toString() }));
    await expect(store.importDocument(invalid)).rejects.toThrow("must be Ed25519");

    const first = generateKeyPairSync("ed25519").publicKey;
    const second = generateKeyPairSync("ed25519").publicKey;
    const document = join(root, "publisher.json");
    writeFileSync(document, JSON.stringify({ keyId: "publisher.test", publicKey: first.export({ type: "spki", format: "pem" }).toString() }));
  ...
  it("rejects oversized persisted trust state before reading its contents", async () => {
    const root = mkdtempSync(join(tmpdir(), "kestrel-plugin-trust-state-large-"));
    directories.push(root);
    writeFileSync(join(root, "trust.json"), Buffer.alloc(1_000_001));

    await expect(new PluginTrustStore(join(root, "trust.json")).list()).rejects.toThrow("trust store exceeds 1 MB");
  });
});
