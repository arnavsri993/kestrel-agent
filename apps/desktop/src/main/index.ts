...
import { ChatGptOAuthManager, type ChatGptOAuthStatus } from "./chatgpt-oauth";
...
const petOverlayAccess = new WeakMap<BrowserWindow, PetOverlayRequestAccess>();
					...
					: path
						? id === "codex"
							? chatGptStatus?.connected
								...
								: chatGptStatus &&
										"accountType" in chatGptStatus &&
										chatGptStatus.accountType === "apiKey"
...
async function createVerifiedLocalBackup(destinationParent: string): Promise<{
	path: string;
	createdAt: string;
	files: number;
	bytes: number;
...
async function listLocalModels(
	...
	const bytes = await readBoundedResponseBytes(
		response,
		1_000_000,
		"Ollama model list response exceeds 1 MB.",
	);
...
async function pullLocalModel(
	...
	await readBoundedResponseBytes(
		response,
		1_000_000,
		"Ollama model pull response exceeds 1 MB.",
	);
...
supervisor.on(
	"crash",
	(event: { restarts: number; recovering: boolean; delayMs?: number }) => {
		if (
			!event.recovering ||
			event.restarts !== 1 ||
...
if (
	...
	const heartbeatMonitor = setInterval(() => {
		...
		if (!developmentHeartbeatIsStale(lastHeartbeatAt, Date.now(), 1_000))
			return;
		clearInterval(heartbeatMonitor);
		process.exit(0);
	}, 250);
...
function createMainWindow(): BrowserWindow {
	...
	if (DEVELOPMENT_RENDERER_URL) void window.loadURL(DEVELOPMENT_RENDERER_URL);
...
async function initializeCore(
	...
	try {
		...
		if (localModels.length > 0) {
			...
			secureEnvironment.KESTREL_OLLAMA_MODEL ??=
				(await localRuntime.preferredModel(localModels)) ??
				localModels[0]!.name;
		}
	} catch {
	...
	const configuredWorkspaceRoots = await workspaceGrantStore.configuredPaths();
...
function registerIpc(): void {
	...
	ipcMain.handle("kestrel:request", async (event, raw) => {
		...
		if (request.type === "browser-set-content-bounds") {
			if (!userBrowserService)
				throw new Error("The visible user browser is unavailable.");
			await userBrowserService.setContentBounds(
				request.bounds,
				request.visible,
			);
			return { ok: true };
