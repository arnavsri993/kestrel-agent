...
const entryPath = "/Applications/Kestrel.app/Contents/Resources/app/renderer/index.html";

describe("renderer URL trust boundary", () => {
  it("accepts only the packaged renderer file", () => {
    expect(isTrustedRendererUrl(pathToFileURL(entryPath).toString(), entryPath)).toBe(true);
    expect(isTrustedRendererUrl(`${pathToFileURL(entryPath).toString()}?petOverlay=1`, entryPath)).toBe(true);
    expect(isTrustedRendererUrl("file:///tmp/untrusted.html", entryPath)).toBe(false);
  ...
  it("accepts only the configured loopback development page", () => {
    ...
    expect(isTrustedRendererUrl("http://localhost:5173/", entryPath, developmentUrl)).toBe(true);
    expect(isTrustedRendererUrl("http://localhost:5173/?petOverlay=1", entryPath, developmentUrl)).toBe(true);
    expect(isTrustedRendererUrl("http://localhost:5173/other", entryPath, developmentUrl)).toBe(false);
    expect(isTrustedRendererUrl("http://localhost.evil.example/", entryPath, developmentUrl)).toBe(false);
    expect(isTrustedRendererUrl("http://localhost@evil.example/", entryPath, developmentUrl)).toBe(false);
    expect(
      isTrustedRendererUrl(
        "http://[::1]:5173/",
  ...
  it("rejects a non-loopback configured development origin", () => {
    expect(
      isTrustedRendererUrl("https://example.com/", entryPath, "https://example.com/"),
    ).toBe(false);
    expect(trustedDevelopmentRendererUrl("https://example.com/")).toBeUndefined();
    expect(
      trustedDevelopmentRendererUrl("http://user:secret@localhost:5173/"),
    ).toBeUndefined();
    expect(
      trustedDevelopmentRendererUrl("http://127.0.0.1:5173/"),
  ...
  it("accepts privileged IPC only from the trusted top-level frame", () => {
    ...
    const trust = (url: string) =>
      isTrustedRendererUrl(url, entryPath);
