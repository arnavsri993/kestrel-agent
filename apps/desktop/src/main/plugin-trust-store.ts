...
function parsePublisher(value: unknown): StoredPublisher {
	...
	if (
		...
		record.publicKey.length > 32_000
	)
		throw new Error("Plugin publisher key document is invalid.");
...
export class PluginTrustStore {
	...
	private async stored(): Promise<StoredPublisher[]> {
		...
		if (bytes.byteLength > 1_000_000)
			throw new Error("Plugin publisher trust store exceeds 1 MB.");
		const parsed = JSON.parse(bytes.toString("utf8")) as unknown;
		if (!Array.isArray(parsed))
			throw new Error("Plugin publisher trust store is invalid.");
		const publishers = parsed.map(parsePublisher);
		if (
			new Set(publishers.map((publisher) => publisher.keyId)).size !==
			publishers.length
		)
			throw new Error(
				"Plugin publisher trust store contains duplicate key IDs.",
			);
		return publishers;
	}
	...
	async list(): Promise<TrustedPluginPublisher[]> {
		return (await this.stored()).map(({ keyId, fingerprint }) => ({
			keyId,
			fingerprint,
		}));
	}
	...
	async trustKeys(): Promise<PluginTrustKey[]> {
		return (await this.stored()).map(({ keyId, publicKey }) => ({
			keyId,
			publicKey,
		}));
	}
	...
	async importDocument(path: string): Promise<TrustedPluginPublisher> {
		...
		if (bytes.byteLength > 64_000)
			throw new Error("Plugin publisher key document exceeds 64 KB.");
		const publisher = parsePublisher(
			JSON.parse(bytes.toString("utf8")) as unknown,
		);
		return this.mutate(async () => {
			const publishers = await this.stored();
			const existing = publishers.find(
				(item) => item.keyId === publisher.keyId,
			);
			if (existing && existing.fingerprint !== publisher.fingerprint)
				throw new Error(
					`Publisher key ID ${publisher.keyId} is already trusted with a different key.`,
				);
			if (!existing)
				await this.save(
					[...publishers, publisher].sort((left, right) =>
	...
	async remove(keyId: string): Promise<void> {
		await this.mutate(async () => {
			const publishers = await this.stored();
			if (!publishers.some((publisher) => publisher.keyId === keyId))
				throw new Error(`Plugin publisher key ${keyId} is not trusted.`);
			await this.save(
				publishers.filter((publisher) => publisher.keyId !== keyId),
			);
		});
	}
	...
	private async mutate<T>(operation: () => Promise<T>): Promise<T> {
		...
		try {
			...
			release();
			if (trustMutationQueues.get(this.path) === queued)
				trustMutationQueues.delete(this.path);
		}
	}
}
