...
export class ProviderAuthMonitor {
	...
	start(): void {
		...
		const initialDelay = boundedTimer(
			this.options.initialDelayMs,
			2 * 60_000,
			1_000,
		);
		const interval = boundedTimer(
	...
	async check(): Promise<ProviderVerification[]> {
		...
		this.checking = true;
		try {
			const listed = await this.options.request({
			...
			for (const verification of verifications) {
				...
				if (!verification.ok && previous !== false) {
					this.options.notify({
						...
						status: "failed",
						detail:
							verification.error?.slice(0, 500) ||
							"The provider rejected its saved authentication.",
					});
				} else if (verification.ok && previous === false) {
					this.options.notify({
						providerId: key,
						status: "recovered",
						detail: "The provider accepted its saved authentication again.",
					});
				}
			}
			for (const key of [...this.status.keys()])
				if (!observed.has(key)) this.status.delete(key);
