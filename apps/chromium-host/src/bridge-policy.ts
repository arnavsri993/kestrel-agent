...
export const HostCommandSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("send"), message: z.string().trim().min(1).max(20_000), provider: z.string().min(1).max(100), model: z.string().trim().min(1).max(200), readBrowser: z.boolean().default(false) }).strict(),
