...
export const HostCommandSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("send"), message: z.string().trim().min(1).max(20_000), provider: z.string().min(1).max(100), model: z.string().trim().min(1).max(200) }).strict(),
  z.object({ type: z.literal("cancel") }).strict(),
  z.object({ type: z.literal("open-tab"), url: z.string().max(8_000) }).strict(),
  z.object({ type: z.literal("focus-tab"), id: z.string().uuid() }).strict(),
  z.object({ type: z.literal("close-tab"), id: z.string().uuid() }).strict(),
]);
