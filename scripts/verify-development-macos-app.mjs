...
const expectedBundles = [
	...
	[
		join(helperApps[0], "Contents", "Info.plist"),
		"com.kestrel.desktop.dev.helper",
	],
	[
		join(helperApps[1], "Contents", "Info.plist"),
		"com.kestrel.desktop.dev.helper.Renderer",
	],
	[
		join(helperApps[2], "Contents", "Info.plist"),
		"com.kestrel.desktop.dev.helper.GPU",
	],
	[
		join(helperApps[3], "Contents", "Info.plist"),
...
if (architecture.status !== 0 || architecture.stdout.trim() !== "arm64") {
	const detail =
		`${architecture.stdout ?? ""}${architecture.stderr ?? ""}`.trim();
	throw new Error(
