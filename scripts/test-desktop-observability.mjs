...
const screenshotPath = resolve(
	"artifacts/screenshots/desktop/setup-revised/settings-observability.png",
);
...
await new Promise((resolvePromise) =>
	collector.listen(0, "127.0.0.1", resolvePromise),
);
...
try {
	...
	await observability
		.getByLabel("Collector base URL")
		.fill(`http://127.0.0.1:${collectorAddress.port}`);
	await observability
		.getByLabel("Service name")
		.fill("workstrand-desktop-test");
	await observability
		.getByLabel("Auth header value")
		.fill("Bearer desktop-collector-secret");
	await observability.getByLabel("Trace sample rate").fill("1");
	await observability.getByText("Prometheus", { exact: true }).click();
	await observability
		.getByLabel("Expose metrics when remote serve is running")
		.check();
	await observability
		.getByRole("button", { name: "Save observability" })
		.click();
	await observability
		.getByText("Observability configuration saved and applied.", {
			exact: true,
		})
		.waitFor();
	const session = await page.evaluate(() =>
		window.kestrel.request({
			...
			title: "private telemetry test title",
		}),
	);
	assert.equal(session.ok, true);
	await observability.getByRole("button", { name: "Test collector" }).click();
	await observability
		.getByText("Collector accepted content-free OTLP metrics and traces.", {
			exact: true,
		})
		.waitFor();
	assert.deepEqual(received.map((request) => request.path).sort(), [
		"/v1/metrics",
		"/v1/traces",
	]);
	assert.ok(
	...
	assert.ok(
		received.every(
			(request) => request.authorization === "Bearer desktop-collector-secret",
		),
	);
	const wire = Buffer.concat(received.map((request) => request.body)).toString(
		"utf8",
	);
	assert.match(wire, /workstrand-desktop-test/);
	assert.doesNotMatch(
		wire,
		/private telemetry test title|desktop-collector-secret/,
	);
	const state = await page.evaluate(() =>
		window.kestrel.request({ type: "observability-get" }),
	);
	assert.equal(state.ok, true);
	assert.ok(
		"observabilityStatus" in state && state.observabilityStatus?.hasHeaderValue,
	);
	...
	process.stdout.write(
		`Desktop OTLP protobuf setup/test, encrypted write-only auth header, Prometheus opt-in, compact reflow, and screenshot passed. Screenshot: ${screenshotPath}\n`,
	);
} finally {
	await application?.close();
	await new Promise((resolvePromise, reject) =>
		collector.close((error) => (error ? reject(error) : resolvePromise())),
	);
