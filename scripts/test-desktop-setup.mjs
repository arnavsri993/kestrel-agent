...
try {
  ...
  await cohereMethods.locator('input[type="password"]').fill("test-cohere-account");
  ...
  const removedCohere = await page.evaluate(async () =>
    window.kestrel.request({ type: "credential-remove", credentialId: "cohere" }),
  );
  ...
  const daylightContrast = await page.evaluate(() => {
    ...
    const primary = getComputedStyle(document.querySelector("#contrast-primary"));
    const composer = getComputedStyle(document.querySelector("#contrast-composer"));
  ...
  await page.locator("#daylight-contrast-fixture").evaluate((fixture) => fixture.remove());
