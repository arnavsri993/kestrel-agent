...
const screenshotPath = resolve("artifacts/screenshots/desktop/setup-revised/settings-external-secrets.png");
...
try {
  ...
  await external.getByRole("button", { name: "Sync and verify" }).last().click();
  ...
  const providers = await page.evaluate(() => window.kestrel.request({ type: "runtime-list-providers" }));
  assert.equal(providers.ok, true);
  assert.ok("providers" in providers && providers.providers?.some((provider) => provider.id === "openai"));

  if (process.env.WORKSTRAND_TEST_REAL_BWS === "1") {
    await external.getByText("Bitwarden Secrets Manager", { exact: true }).click();
    await external.getByRole("button", { name: "Install verified CLI" }).click();
    await external.getByRole("button", { name: "Verified CLI installed" }).waitFor({ timeout: 150_000 });
    await external.getByText(/Verified Bitwarden CLI installed locally/).waitFor();
  }
  ...
  assert.ok(await page.evaluate(() => document.activeElement instanceof HTMLElement && document.activeElement.matches("button, input, textarea, select, summary, a")));
  ...
  const providersAfterRemoval = await page.evaluate(() => window.kestrel.request({ type: "runtime-list-providers" }));
  assert.equal(providersAfterRemoval.ok, true);
  assert.ok("providers" in providersAfterRemoval && !providersAfterRemoval.providers?.some((provider) => provider.id === "openai"));
  assert.deepEqual(runtimeErrors, []);
  process.stdout.write(`External secret Settings flow, command allowlist, isolated-core restart, removal, compact reflow, and screenshot passed${process.env.WORKSTRAND_TEST_REAL_BWS === "1" ? " with a real pinned Bitwarden CLI install" : ""}.\n`);
} finally {
