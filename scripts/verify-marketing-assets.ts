...
function publicAssetPath(
  ...
  if (withinPublic.startsWith("..") || resolve(publicRoot, withinPublic) !== path) {
...
async function verifyRegistryAssetFiles(
  ...
  if (asset.muted !== true)
    problems.push(`${assetLabel} is not marked muted`);
...
export async function verifyMarketingAssets(
  ...
  for (const asset of registry) {
    ...
    if (!asset.sourceManifestId)
      problems.push(`registry entry ${asset.id || "(missing id)"} has no source manifest`);
  }
  ...
  for (const file of await readdir(manifestDir)) {
    ...
    if (generatedStatuses.has(manifest.status)) {
      ...
      if (original) {
        ...
        if (originalSize !== undefined && originalSize <= 0)
          problems.push(`${label}: local original is empty or not a regular file`);
      }
    }

    if (manifest.status !== "published") continue;
    publishedManifestIds.add(manifest.id);
    if (!Array.isArray(manifest.processedOutputPaths) || !manifest.processedOutputPaths.length) {
    ...
    const assets = registry.filter((asset) => asset.sourceManifestId === manifest.id);
    if (!assets.length) {
