...
describe("public release artifact coherence", () => {
  ...
  it("rejects malformed semantic versions", () => {
    expect(() =>
      validateReleaseManifest(fixtureManifest(), "1.2.3-."),
    ).toThrow(/Invalid public release version/);
  });
  ...
  it("rejects ambiguous or unsupported updater YAML", () => {
    ...
    expect(() =>
      parseLatestMac(`${valid}version: ${version}\n`),
    ).toThrow(/repeats top-level version/);
    expect(() =>
      parseLatestMac(valid.replace("    size:", "      size:")),
    ).toThrow(/unsupported file content or indentation/);
    expect(() =>
      parseLatestMac(`${valid}unexpected: value\n`),
    ).toThrow(/unsupported top-level content/);
  });
  ...
  it("verifies the configured distribution metadata and public artifact bytes", async () => {
    ...
    const urls = {
      ...
      checksums: new URL(
        "https://downloads.example.test/SHA256SUMS",
      ),
