...
const installRoot = resolve(
	process.env.KESTREL_MACOS_INSTALL_ROOT ?? "/Applications",
);
const destination = join(installRoot, "Kestrel.app");
const trashRoot = resolve(
	process.env.KESTREL_MACOS_TRASH_ROOT ?? join(home, ".Trash"),
);
...
function isKestrelBundle(bundlePath) {
	if (!bundlePath.endsWith(".app") || !statIsDirectory(bundlePath))
		return false;
	...
	return (
		(identifier === "com.kestrel.desktop" ||
...
function unregister(path) {
	if (process.env.KESTREL_SKIP_LSREGISTER === "1" || !existsSync(lsregister))
		return;
	try {
...
function moveDuplicatesToTrash(excludedPaths) {
	...
	for (const root of searchRoots) {
		for (const candidate of appCandidates(root)) {
			if (
				excludedPaths.some((excludedPath) => samePath(candidate, excludedPath))
			)
				continue;
...
function install() {
	...
	if (!samePath(source, destination)) {
		...
		if (existsSync(destination))
			moved.push({
				from: destination,
				to: moveToTrash(destination, "previous"),
			});
