import { createHash } from "node:crypto";
import { readdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { basename, join, resolve } from "node:path";

const root = resolve(process.argv[2] ?? "release");
const artifacts = readdirSync(root).map((name) => join(root, name)).filter((path) => statSync(path).isFile() && /\.(?:dmg|zip)$/.test(path)).sort();
if (artifacts.length === 0) throw new Error("No DMG or ZIP release artifacts were found.");
const records = artifacts.map((path) => ({ filename: basename(path), bytes: statSync(path).size, sha256: createHash("sha256").update(readFileSync(path)).digest("hex") }));
const manifest = { schemaVersion: 1, product: "Workstrand", version: process.env.GITHUB_REF_NAME ?? "development", commit: process.env.GITHUB_SHA ?? "unknown", artifacts: records };
writeFileSync(join(root, "release-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, { mode: 0o644 });
writeFileSync(join(root, "SHA256SUMS"), `${records.map((record) => `${record.sha256}  ${record.filename}`).join("\n")}\n`, { mode: 0o644 });
