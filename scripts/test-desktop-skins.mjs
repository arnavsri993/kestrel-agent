...
const daylightScreenshot = resolve(
	"artifacts/screenshots/desktop/setup-revised/settings-skin-daylight.png",
);
const customScreenshot = resolve(
	"artifacts/screenshots/desktop/setup-revised/settings-skin-custom.png",
);
...
try {
	...
	assert.equal(
		await skinSetting
			.getByRole("button", { name: /Kestrel/ })
			.getAttribute("aria-pressed"),
		"true",
	);
	await daylight.click();
	await page.waitForFunction(
		() => document.documentElement.dataset.skin === "daylight",
	);
	await page.waitForFunction(() =>
		[...document.querySelectorAll(".skin-setting button")].some(
			(button) =>
				button.textContent?.includes("Daylight") &&
	...
	await page
		.locator(".skin-setting")
		.getByRole("button", { name: "Install JSON skin" })
		.click();
	await page
		.locator(".skin-setting")
		.getByText("Custom skin installed and selected.", { exact: true })
		.waitFor();
	const fieldNotes = page
		.locator(".skin-setting")
	...
	await page
		.locator(".skin-setting")
		.getByRole("button", { name: "Remove Field Notes" })
		.click();
	await page
		.locator(".skin-setting")
		.getByText("Custom skin removed; Kestrel restored.", { exact: true })
		.waitFor();
	assert.equal(
		await page.evaluate(() => document.documentElement.dataset.skin),
		"workstrand",
	...
	process.stdout.write(
		`Desktop built-in selection, restart persistence, strict custom import, recovery, focus, compact reflow, and screenshots passed. Screenshots: ${daylightScreenshot}, ${customScreenshot}\n`,
	);
} finally {
