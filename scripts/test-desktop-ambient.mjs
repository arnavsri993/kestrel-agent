...
const memoryScreenshot = resolve(
	"artifacts/screenshots/desktop/setup-revised/memory-dreaming.png",
);
const presenceScreenshot = resolve(
	"artifacts/screenshots/desktop/setup-revised/settings-presence.png",
);
...
try {
	...
	const automatic = dreaming.getByRole("switch", {
	...
	await dreaming
		.getByText("Preview complete. Nothing was stored or promoted.")
		.waitFor();
	...
	assert.match(
		await presence.textContent(),
		/never stores client IP addresses, hostnames, or probe traffic/,
	);
	const channelPolicy = page.locator(".channel-interaction-setting");
	await channelPolicy
		.getByText("Channel progress, typing, and reactions", { exact: true })
		.waitFor();
	...
	await channelPolicy
		.getByRole("button", { name: "Save channel policy" })
		.click();
	await channelPolicy
		.getByText("Channel interaction policy saved.", { exact: true })
		.waitFor();
	...
	process.stdout.write(
		`Review-gated dreaming, non-persistent preview, scheduled state, ephemeral presence, compact reflow, and screenshots passed. Screenshots: ${memoryScreenshot}, ${presenceScreenshot}\n`,
	);
} finally {
			...
			new Promise((resolvePromise, reject) => {
				...
				const poll = () => {
					if (element.getAttribute("aria-checked") === value)
						...
						reject(new Error(`Expected aria-checked=${value}.`));
