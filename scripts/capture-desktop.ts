...
const captureName = process.argv.includes("--setup-revised") ? "setup-revised" : process.argv.includes("--workstrand-pass1") ? "workstrand-pass1" : process.argv.includes("--workstrand-revised") ? "workstrand-revised" : process.argv.includes("--revised") ? "revised" : "initial";
...
await page.setViewportSize({ width: 1320, height: 860 });
await page.getByRole("button", { name: "Set up models later" }).click();
await page.getByRole("heading", { name: /Your foundation is set|Workstrand is ready for a model/ }).waitFor();
