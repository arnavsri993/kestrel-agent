...
export function parseSha256Sums(source) {
  ...
  for (const [index, rawLine] of source.replaceAll("\r", "").split("\n").entries()) {
    if (!rawLine.trim()) continue;
    const match = rawLine.match(/^([a-fA-F0-9]{64})  ([^\s/]+)$/);
    if (!match)
      throw new Error(`SHA256SUMS line ${index + 1} is malformed.`);
  ...
  for (const rawLine of source.replaceAll("\r", "").split("\n")) {
    ...
    if (indentation === 0) {
      ...
      if (line === "files:") {
        ...
        version = parseYamlScalar(line.slice("version:".length), "Updater version");
...
export function verifyReleaseMetadata({
  ...
  if (
    !updatePath ||
    updater.sha512 !== updatePath.sha512
  )
    throw new Error(
...
async function readMetadata(response, label) {
  ...
  if (
    ...
    declaredBytes > maximumMetadataBytes
  ) {
...
async function hashWebArtifact(fetchImpl, url, label, expected, signal) {
  ...
  if (!response.body)
    throw new Error(`${label} returned no artifact body.`);
...
export async function verifyPublicReleaseArtifacts({
  ...
  if (!commitPattern.test(expectedCommit))
    throw new Error("Expected public release commit must be a full Git commit SHA.");
