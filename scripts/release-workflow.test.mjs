...
const workflow = readFileSync(
  resolve(import.meta.dirname, "..", ".github", "workflows", "release-macos.yml"),
  "utf8",
);
const websiteWorkflow = readFileSync(
  resolve(import.meta.dirname, "..", ".github", "workflows", "deploy-website.yml"),
  "utf8",
);
...
describe("macOS release workflow security contract", () => {
  it("keeps verification read-only and release credentials step-local", () => {
    ...
    const verifyJobHeader = verifyJob.slice(0, verifyJob.indexOf("\n    steps:"));
  ...
  it("serializes each ref and never publishes a manual dispatch", () => {
    ...
    expect(workflow.split("node scripts/verify-release-bundle.mjs release"))
      .toHaveLength(3);
    expect(workflow).toContain(
